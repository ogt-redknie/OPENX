...
steps:
  - name: emits protected low-cardinality prometheus metrics
    actions:
      ...
      - set: gatewayToken
        value:
          expr: "String(env.gateway.token ?? env.gateway.runtimeEnv.OPNEX_GATEWAY_TOKEN ?? '')"
      - assert:
      ...
      - assert:
          expr: "prometheusText.includes('# TYPE opnex_run_completed_total counter')"
          message: "missing run completion counter"
      - assert:
          expr: "prometheusText.includes('# TYPE opnex_run_duration_seconds histogram')"
          message: "missing run duration histogram"
      - assert:
          expr: "prometheusText.includes('# TYPE opnex_model_call_total counter')"
          message: "missing model call counter"
      - assert:
          expr: "prometheusText.includes('# TYPE opnex_harness_run_total counter')"
      ...
      - assert:
          expr: "!prometheusText.includes('opnex.content.')"
          message: "prometheus output leaked content attributes"
      - assert:
          expr: "!/opnex_prometheus_series_dropped_total(?:\\{[^}]*\\})?\\s+(?!0(?:\\.0+)?(?:\\s|$))/.test(prometheusText)"
