...
const CHECK_SCRIPT = "scripts/check-opnex-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "opnex-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "opnex", version }));
    writeFileSync(
    ...
    const tarball = join(root, "opnex.tgz");
...
describe("check-opnex-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("OPNEX package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
