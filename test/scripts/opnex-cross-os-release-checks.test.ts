...
} from "../../scripts/opnex-cross-os-release-checks.ts";

describe("scripts/opnex-cross-os-release-checks", () => {
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "opnex-cross-os-agent-output-"));
    try {
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'opnex.cmd')");
    expect(script).toContain("$cmd = Get-Command opnex -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        OPNEX_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        OPNEX_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        OPNEX_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("opnex-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://opnex.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://opnex.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://opnex.ai/install.ps1");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "opnex/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "OPNEX_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/opnex/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/opnex/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/opnex-cross-os-release-checks.ts", "utf8")).toContain(
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/opnex", "darwin"),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("opnex@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("opnex@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "opnex-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "opnex-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects bundled runtime-deps staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "opnex-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".OPNEX-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".OPNEX-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "opnex-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "opnex-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
