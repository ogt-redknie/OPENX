...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.OPNEX_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "OPNEX_TEST_FAST", value: process.env.OPNEX_TEST_FAST },
    {
      key: "OPNEX_STRICT_FAST_REPLY_CONFIG",
      value: process.env.OPNEX_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "OPNEX_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.OPNEX_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "OPNEX_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.OPNEX_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "OPNEX_STATE_DIR", value: process.env.OPNEX_STATE_DIR },
    { key: "OPNEX_CONFIG_PATH", value: process.env.OPNEX_CONFIG_PATH },
    { key: "OPNEX_GATEWAY_PORT", value: process.env.OPNEX_GATEWAY_PORT },
    { key: "OPNEX_BRIDGE_ENABLED", value: process.env.OPNEX_BRIDGE_ENABLED },
    { key: "OPNEX_BRIDGE_HOST", value: process.env.OPNEX_BRIDGE_HOST },
    { key: "OPNEX_BRIDGE_PORT", value: process.env.OPNEX_BRIDGE_PORT },
    { key: "OPNEX_CANVAS_HOST_PORT", value: process.env.OPNEX_CANVAS_HOST_PORT },
    { key: "OPNEX_TEST_HOME", value: process.env.OPNEX_TEST_HOME },
    { key: "OPNEX_AGENT_DIR", value: process.env.OPNEX_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPNEX_TEST_HOME = tempHome;
  process.env.OPNEX_TEST_FAST = "1";
  process.env.OPNEX_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.OPNEX_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.OPNEX_BRIDGE_ENABLED;
  delete process.env.OPNEX_BRIDGE_HOST;
  delete process.env.OPNEX_BRIDGE_PORT;
  delete process.env.OPNEX_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.OPNEX_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.OPNEX_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".opnex");
  const priorIsolatedHome = params.env.OPNEX_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".opnex")
  ) {
    realStateDir = path.join(params.realHome, ".opnex");
  }
  const tempStateDir = path.join(params.tempHome, ".opnex");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.OPNEX_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.OPNEX_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "opnex.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "opnex.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.OPNEX_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.OPNEX_LIVE_USE_REAL_HOME);
