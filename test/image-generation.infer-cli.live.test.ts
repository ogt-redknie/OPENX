...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.OPNEX_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through opnex infer", () => {
    const outputBase = path.join(os.tmpdir(), `opnex-infer-image-${process.pid}.png`);
