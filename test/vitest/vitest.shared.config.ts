...
export type OPNEXVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: OPNEXVitestPool = resolveDefaultVitestPool(env),
): number {
...
function hasWorkerOverride(env: Record<string, string | undefined>): boolean {
  return Boolean((env.OPNEX_VITEST_MAX_WORKERS ?? env.OPNEX_TEST_WORKERS)?.trim());
}
...
export const sharedVitestConfig = {
  ...
  resolve: {
    alias: [
      {
        find: "opnex/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      {
        find: "@opnex/qa-channel/api.js",
