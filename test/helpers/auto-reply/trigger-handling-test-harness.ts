...
import type { OPNEXConfig } from "../../../src/config/types.opnex.js";
...
const piEmbeddedMocks = getSharedMocks("opnex.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("opnex.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("opnex.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("opnex.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    opnexHome: process.env.OPNEX_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPNEX_HOME", snapshot.opnexHome);
  restoreKey("OPNEX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPNEX_STATE_DIR = join(home, ".opnex");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "opnex-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".opnex", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OPNEXConfig {
