---
name: opnex-qa-testing
description: Run, watch, debug, extend, or explain OPNEX qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
OPNEX_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm opnex qa suite \
...
5. If the user wants to watch the live UI, find the current `opnex-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm opnex qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
OPNEX_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `OPNEX_QA_PROVIDER_MODE`
  - `OPNEX_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `OPNEX`
  - item: `OPNEX_QA_CONVEX_SITE_URL`
  - item: `OPNEX_QA_CONVEX_SECRET_MAINTAINER`
  - item: `OPNEX_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `OPNEX QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `OPNEX_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `OPNEX_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `OPNEX/OPNEX_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
OPNEX_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
OPNEX_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
OPNEX_QA_PROVIDER_MODE="mock-openai" \
OPNEX_NPM_TELEGRAM_PACKAGE_SPEC="opnex@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `OPNEX_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `OPNEX_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `OPNEX_QA_PROVIDER_MODE`, map it explicitly to `OPNEX_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    - `OPNEX_QA_CREDENTIAL_SOURCE=convex`
    - `OPNEX_QA_CREDENTIAL_ROLE=maintainer`
    - `OPNEX_QA_CONVEX_SITE_URL`
    - `OPNEX_QA_CONVEX_SECRET_MAINTAINER`
    - `OPNEX_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal OPNEX identity combines with the character.
...
```bash
pnpm opnex qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `OPNEX_HOME` sandboxed.
