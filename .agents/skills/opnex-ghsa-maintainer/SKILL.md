---
name: opnex-ghsa-maintainer
description: Inspect, patch, validate, publish, or confirm OPNEX GHSA security advisories and private-fork state.
---

# OPNEX GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `opnex-release-maintainer`.
...
```bash
fork=$(gh api /repos/opnex/opnex/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/opnex/opnex/security-advisories/<GHSA> \
