...
description: Use gitcrawl to search duplicate OPNEX PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local OPNEX history and clustering source.
...
gitcrawl neighbors opnex/opnex --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search opnex/opnex --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search opnex/opnex --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```
...
```bash
gh search prs --repo opnex/opnex --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo opnex/opnex --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo opnex/opnex --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R opnex/opnex "<problem phrase>" --types group --limit 10
prtags search similar -R opnex/opnex "<problem summary>" --types group --limit 10
prtags group list -R opnex/opnex
```
...
```bash
prtags group create -R opnex/opnex \
  --kind mixed \
...
```bash
prtags field ensure -R opnex/opnex --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R opnex/opnex --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R opnex/opnex --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R opnex/opnex --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R opnex/opnex --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R opnex/opnex --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R opnex/opnex --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R opnex/opnex --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R opnex/opnex --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R opnex/opnex <pr-number> \
...
```bash
prtags annotation issue set -R opnex/opnex <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R opnex/opnex
```
