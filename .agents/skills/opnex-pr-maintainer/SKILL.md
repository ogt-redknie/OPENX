---
name: opnex-pr-maintainer
description: Review, triage, close, label, comment on, or land OPNEX PRs/issues with maintainer evidence checks.
---
...
- Use `$gitcrawl` first anytime you inspect OPNEX issues or PRs.
...
gitcrawl search opnex/opnex --query "<scope or title keywords>" --mode hybrid --json
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete OPNEX problem/fix
...
- Use `--repo opnex/opnex` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo opnex/opnex --match title,body --limit 50 -- "auto-update"
gh search issues --repo opnex/opnex --match title,body --limit 50 -- "auto-update"
gh search issues --repo opnex/opnex --match title,body --limit 50 \
