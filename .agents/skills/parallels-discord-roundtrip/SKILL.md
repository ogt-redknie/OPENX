...
```bash
export OPNEX_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.opnex/opnex.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env OPNEX_PARALLELS_DISCORD_TOKEN \
...
- Use the `opnex` wrapper for guest `message send/read`; `node opnex.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/opnex-parallels-smoke.*`
...
- host inbound nonce appears in `opnex message read` output
