...
1. Download the runtime for your OS (extracts into `$OPNEX_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.opnex/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$OPNEX_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.opnex/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active OPNEX config file (`$OPNEX_CONFIG_PATH`, default `~/.opnex/opnex.json`):
