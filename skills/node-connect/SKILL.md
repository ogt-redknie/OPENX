---
name: node-connect
description: Diagnose OPNEX Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify OPNEX is advertising that route, then fix pairing/auth.
...
- whether `opnex devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `opnex qr --json`. It uses the same setup-code payload Android scans.

```bash
opnex config get gateway.mode
opnex config get gateway.bind
opnex config get gateway.tailscale.mode
opnex config get gateway.remote.url
opnex config get gateway.auth.mode
opnex config get gateway.auth.allowTailscale
opnex config get plugins.entries.device-pair.config.publicUrl
opnex qr --json
opnex devices list
opnex nodes status
```

If this OPNEX instance is pointed at a remote gateway, also run:

```bash
opnex qr --remote --json
```
...
If `opnex qr --json` says `Gateway is only bound to loopback`:
...
- `opnex devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run opnex qr again, rescan, then approve the pending device pairing.`
