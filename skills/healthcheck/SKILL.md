---
name: healthcheck
description: Audit and harden hosts running OPNEX for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# OPNEX Host Hardening

## Overview

Assess and harden the host running OPNEX, then align it to a user-defined risk tolerance without breaking access. Use OPNEX security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OPNEX changes the host firewall, SSH, or OS updates; it does not.
...
5. OPNEX gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OPNEX can access sensitive data.
...
### 2) Run OPNEX security audits (read-only)

As part of the default read-only checks, run `opnex security audit --deep`. Only offer alternatives if the user requests them:

1. `opnex security audit` (faster, non-probing)
2. `opnex security audit --json` (structured output)

Offer to apply OPNEX safe defaults (numbered):

1. `opnex security audit --fix`

Be explicit that `--fix` only tightens OPNEX defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OPNEX version/update status (read-only)

As part of the default read-only checks, run `opnex update status`.
...
- Credential hygiene notes (location of OPNEX creds, prefer disk encryption)
...
After OPNEX install or first hardening pass, run at least one baseline audit and version check:

- `opnex security audit`
- `opnex security audit --deep`
- `opnex update status`

Ongoing monitoring is recommended. Use the OPNEX cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opnex cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opnex update status`
...
Before creating, `opnex cron list` and match on exact `name`. If found, `opnex cron edit <id> ...`.
If not found, `opnex cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opnex update status` (preferred for source checkouts and channels)
2. `npm view opnex version` (published npm version)

## OPNEX command accuracy

Use only supported commands and flags:

- `opnex security audit [--deep] [--fix] [--json]`
- `opnex status` / `opnex status --deep`
- `opnex health --json`
- `opnex update status`
- `opnex cron add|list|runs|run`

Do not invent CLI flags or imply OPNEX enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OPNEX compaction:
