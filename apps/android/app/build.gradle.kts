...
val androidStoreFile = providers.gradleProperty("OPNEX_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("OPNEX_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("OPNEX_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("OPNEX_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set OPNEX_ANDROID_STORE_FILE, " +
      "OPNEX_ANDROID_STORE_PASSWORD, OPNEX_ANDROID_KEY_ALIAS, and " +
      "OPNEX_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/OPNEXKit/Sources/OPNEXKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.opnex.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "opnex-$versionName-$buildType.apk"
          } else {
            "opnex-$versionName-$flavorName-$buildType.apk"
          }
