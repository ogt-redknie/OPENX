...
internal fun opnexAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this OPNEX Android gateway connection failure.
    ...
    - tell me whether `opnex devices list` should show a pending pairing request
    - if more signal is needed, ask for `opnex qr --json`, `opnex devices list`, and `opnex nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${opnexAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("OPNEX gateway diagnostics", report))
