...
private const val NOTIFICATION_CHANNEL_BASE_ID = "opnex.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "OPNEX Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "OPNEX Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "OPNEX Active")
      }
