...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "OPNEX Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "OPNEX Node · Talk"
              state.connected -> "OPNEX Node · Connected"
              else -> "OPNEX Node"
            }
          val text =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "OPNEX node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.opnex.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.opnex.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.opnex.app.extra.VOICE_CAPTURE_MODE"
