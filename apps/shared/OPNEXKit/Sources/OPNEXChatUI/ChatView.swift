...
@MainActor
public struct OPNEXChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: OPNEXChatViewModel
    ...
    private var visibleMessages: [OPNEXChatMessage] {
        let base: [OPNEXChatMessage]
    ...
    private func mergeToolResults(in messages: [OPNEXChatMessage]) -> [OPNEXChatMessage] {
    ...
    private func isToolResultMessage(_ message: OPNEXChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: OPNEXChatMessage) -> Bool {
    ...
    private func primaryText(in message: OPNEXChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: OPNEXChatMessage) -> Bool {
    ...
    private func toolCalls(in message: OPNEXChatMessage) -> [OPNEXChatMessageContent] {
    ...
    private func inlineToolResults(in message: OPNEXChatMessage) -> [OPNEXChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: OPNEXChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OPNEXChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(OPNEXChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
