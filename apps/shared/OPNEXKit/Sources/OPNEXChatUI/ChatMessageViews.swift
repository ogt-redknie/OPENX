...
struct ChatMessageBubble: View {
    let message: OPNEXChatMessage
    let style: OPNEXChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OPNEXChatTheme.userText : OPNEXChatTheme.assistantText
    ...
    private var inlineAttachments: [OPNEXChatMessageContent] {
    ...
    private var toolCalls: [OPNEXChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OPNEXChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OPNEXChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OPNEXChatTheme.onboardingAssistantBubble
        }
        return OPNEXChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OPNEXChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OPNEXChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OPNEXChatTheme.userText : OPNEXChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OPNEXChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OPNEXChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OPNEXChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OPNEXChatTheme.assistantText)
            }
        }
    }
}
