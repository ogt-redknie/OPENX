...
private let chatUILogger = Logger(subsystem: "ai.opnex", category: "OPNEXChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class OPNEXChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [OPNEXChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [OPNEXChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [OPNEXPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OPNEXChatPendingToolCall] = []
    public private(set) var sessions: [OPNEXChatSessionEntry] = []
    private let transport: any OPNEXChatTransport
    private var sessionDefaults: OPNEXChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: OPNEXChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any OPNEXChatTransport,
    ...
    public var sessionChoices: [OPNEXChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [OPNEXChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OPNEXPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OPNEXChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OPNEXChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OPNEXChatMessage) -> OPNEXChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OPNEXChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OPNEXChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: OPNEXChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: OPNEXChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: OPNEXChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [OPNEXChatMessage],
        incoming: [OPNEXChatMessage]) -> [OPNEXChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OPNEXChatMessage]) -> [OPNEXChatMessage] {
        var result: [OPNEXChatMessage] = []
    ...
    private static func dedupeKey(for message: OPNEXChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OPNEXChatMessageContent] = [
            OPNEXChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OPNEXChatAttachmentPayload in
            OPNEXChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OPNEXChatSessionEntry {
        OPNEXChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = OPNEXChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: OPNEXChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OPNEXChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OPNEXAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OPNEXChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OPNEXPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
