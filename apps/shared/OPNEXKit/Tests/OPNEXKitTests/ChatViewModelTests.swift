...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OPNEXChatHistoryPayload
{
    OPNEXChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OPNEXChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> OPNEXChatSessionEntry
{
    OPNEXChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> OPNEXChatModelChoice {
    OPNEXChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OPNEXChatHistoryPayload],
    sessionsResponses: [OPNEXChatSessionsListResponse] = [],
    modelResponses: [[OPNEXChatModelChoice]] = [],
    ...
    -> (TestChatTransport, OPNEXChatViewModel)
{
...
private func sendUserMessage(_ vm: OPNEXChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OPNEXChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OPNEXChatHistoryPayload]
    private let sessionsResponses: [OPNEXChatSessionsListResponse]
    private let modelResponses: [[OPNEXChatModelChoice]]
    ...
    private let stream: AsyncStream<OPNEXChatTransportEvent>
    private let continuation: AsyncStream<OPNEXChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<OPNEXChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OPNEXChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OPNEXChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [OPNEXChatAttachmentPayload]) async throws -> OPNEXChatSendResponse
    {
        ...
        return OPNEXChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OPNEXChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OPNEXChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OPNEXChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = OPNEXChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = OPNEXChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: OPNEXChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = OPNEXChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OPNEXChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(OPNEXChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == OPNEXChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = OPNEXChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OPNEXChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = OPNEXChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = OPNEXChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = OPNEXChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { OPNEXChatViewModel(sessionKey: "main", transport: transport) }
