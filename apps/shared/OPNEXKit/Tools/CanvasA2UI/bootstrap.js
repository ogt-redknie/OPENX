...
import "@a2ui/lit/ui";
import { themeContext } from "@opnex/a2ui-theme-context";
...
class OPNEXA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.opnexA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["opnex:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.opnexCanvasA2UIAction ??
      globalThis.opnexCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.opnexCanvasA2UIAction) {
...
if (!customElements.get("opnex-a2ui-host")) {
  customElements.define("opnex-a2ui-host", OPNEXA2UIHost);
}
