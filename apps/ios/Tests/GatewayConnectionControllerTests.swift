...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": OPNEXLocationMode.always.rawValue,
            ...
            #expect(caps.contains(OPNEXCapability.canvas.rawValue))
            #expect(caps.contains(OPNEXCapability.screen.rawValue))
            #expect(caps.contains(OPNEXCapability.camera.rawValue))
            #expect(caps.contains(OPNEXCapability.location.rawValue))
            #expect(caps.contains(OPNEXCapability.voiceWake.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": OPNEXLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(OPNEXLocationCommand.get.rawValue))
        }
    }
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(!commands.contains(OPNEXSystemCommand.run.rawValue))
            #expect(!commands.contains(OPNEXSystemCommand.which.rawValue))
            #expect(!commands.contains(OPNEXSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(OPNEXSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.opnex.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.opnex.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.opnex.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.opnex.gateway", account: "lastConnection")
