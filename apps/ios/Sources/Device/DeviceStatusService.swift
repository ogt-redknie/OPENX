...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OPNEXDeviceStatusPayload {
        ...
        return OPNEXDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> OPNEXBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OPNEXBatteryState = switch device.batteryState {
        ...
        return OPNEXBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OPNEXThermalStatusPayload {
        let state: OPNEXThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OPNEXThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OPNEXStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OPNEXStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
