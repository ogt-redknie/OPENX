...
final class RemindersService: RemindersServicing {
    func list(params: OPNEXRemindersListParams) async throws -> OPNEXRemindersListPayload {
        ...
        let payload: [OPNEXReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return OPNEXRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OPNEXRemindersAddParams) async throws -> OPNEXRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OPNEXReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OPNEXRemindersAddPayload(reminder: payload)
    }
