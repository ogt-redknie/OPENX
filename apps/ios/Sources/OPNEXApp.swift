...
@MainActor
enum OPNEXAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class OPNEXAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.opnex.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.opnex.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.opnex.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? OPNEXAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "opnex.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "opnex.watch.promptId"
    static let sessionKeyKey = "opnex.watch.sessionKey"
    static let actionPrimaryIDKey = "opnex.watch.action.primary.id"
    static let actionPrimaryLabelKey = "opnex.watch.action.primary.label"
    static let actionSecondaryIDKey = "opnex.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "opnex.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "opnex.watch.action.primary"
    static let actionSecondaryIdentifier = "opnex.watch.action.secondary"
    static let actionIdentifierPrefix = "opnex.watch.action."
    static let actionIDKeyPrefix = "opnex.watch.action.id."
    static let actionLabelKeyPrefix = "opnex.watch.action.label."
    static let categoryPrefix = "opnex.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OPNEXWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OPNEXWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OPNEXWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "OPNEX" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OPNEXWatchAction]) -> [UNNotificationAction] {
...
struct OPNEXApp: App {
    ...
    @UIApplicationDelegateAdaptor(OPNEXAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        OPNEXAppModelRegistry.appModel = appModel
...
extension OPNEXApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("OPNEX: installing uncaught exception handler")
