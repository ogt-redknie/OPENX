...
private let webChatSwiftLogger = Logger(subsystem: "ai.opnex", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "opnex.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: OPNEXChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> OPNEXChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(OPNEXChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            OPNEXChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return OPNEXChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OPNEXChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OPNEXGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: OPNEXProtocol.ModelChoice) -> OPNEXChatModelChoice {
        OPNEXChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OPNEXChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OPNEXChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OPNEXChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: OPNEXChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OPNEXChatView>) -> NSViewController
    {
