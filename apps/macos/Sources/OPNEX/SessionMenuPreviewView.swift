...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opnex", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OPNEXSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OPNEXSessionPreviewEntry,
    ...
    private static func cache(payload: OPNEXSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OPNEXChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OPNEXKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OPNEXKit.AnyCodable]) -> [OPNEXChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OPNEXChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OPNEXChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OPNEXChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OPNEXChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OPNEXChatMessage) -> String? {
