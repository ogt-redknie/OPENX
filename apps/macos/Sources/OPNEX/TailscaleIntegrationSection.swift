...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opnex/opnex.json. Prefer OPNEX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opnex/opnex.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opnex/opnex.json. Restart the gateway to apply."
        }
