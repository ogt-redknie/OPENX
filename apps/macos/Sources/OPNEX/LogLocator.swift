...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPNEX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/opnex")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opnex-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("opnex-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/opnex/ (rolling or stdout), or nil if none exist.
