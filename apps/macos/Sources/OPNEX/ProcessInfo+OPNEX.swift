...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.opnex.mac`) even if the shipped
    ...
    {
        if environment["OPNEX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "opnex.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "opnex.nixMode") { return true }
