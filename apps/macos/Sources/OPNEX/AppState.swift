...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = OPNEXConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            OPNEXConfigFile.canonicalHostForComparison(host) ==
                OPNEXConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = OPNEXConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: OPNEXConfigFile.loadDict(),
            draft: .init(
        ...
        OPNEXConfigFile.saveDict(synced.root)
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/opnex"
        state.remoteCliPath = ""
        return state
    }
}
