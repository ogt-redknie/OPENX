...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OPNEXProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.opnex", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OPNEXKit.AnyCodable]()) {
                $0[$1.key] = OPNEXKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OPNEXProtocol.AnyCodable?) -> [String: OPNEXProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OPNEXProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OPNEXKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OPNEXProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("opnex.control.heartbeat")
    static let controlAgentEvent = Notification.Name("opnex.control.agent")
}
