...
enum CommandResolver {
    private static let projectRootDefaultsKey = "opnex.gatewayProjectRootPath"
    private static let helperName = "opnex"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let opnexEntry = root.appendingPathComponent("opnex.mjs").path
        if FileManager().isReadableFile(atPath: opnexEntry) { return opnexEntry }
        let binEntry = root.appendingPathComponent("bin/opnex.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/opnex")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let opnexPaths = self.opnexManagedPaths(home: home)
        if !opnexPaths.isEmpty {
            extras.insert(contentsOf: opnexPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + opnexPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func opnexManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".opnex"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func opnexExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOPNEXExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("opnex.mjs").path,
            root.appendingPathComponent("bin/opnex.js").path,
        ]
    ...
    static func hasAnyOPNEXInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.opnexExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let opnexPath = self.projectOPNEXExecutable(projectRoot: root) {
            return [opnexPath, subcommand] + extraArgs
        }
        if let opnexPath = self.opnexExecutable(searchPaths: searchPaths) {
            return [opnexPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "opnex", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            opnex entrypoint missing (looked for dist/index.js or opnex.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real opnex CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/opnex"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/opnex.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/opnex.mjs"
            node "$PRJ/opnex.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/opnex.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/opnex.js"
            node "$PRJ/bin/opnex.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent opnex"
          pnpm --silent opnex \(quotedArgs);
        else
          echo "opnex CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OPNEXConfigFile.loadDict()
