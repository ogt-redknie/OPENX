...
enum DebugActions {
    private static let verboseDefaultsKey = "opnex.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "opnex.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OPNEXPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OPNEX", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OPNEXPaths.configURL
        guard
