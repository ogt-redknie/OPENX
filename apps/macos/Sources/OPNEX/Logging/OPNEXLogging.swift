...
enum OPNEXLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OPNEXOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OPNEXFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OPNEXLogging.bootstrapIfNeeded()
        let label = OPNEXLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OPNEXOSLogHandler: AppLogLevelBackedHandler {
...
struct OPNEXFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OPNEXLogging.parseLabel(self.label)
