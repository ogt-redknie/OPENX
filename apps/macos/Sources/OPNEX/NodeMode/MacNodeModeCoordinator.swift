...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opnex", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = OPNEXConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OPNEXNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: OPNEXLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        var caps: [String] = [OPNEXCapability.canvas.rawValue, OPNEXCapability.screen.rawValue]
        if browserControlEnabled, connectionMode == .local {
            caps.append(OPNEXCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(OPNEXCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(OPNEXCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: OPNEXConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: OPNEXLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            OPNEXSystemCommand.which.rawValue,
            OPNEXSystemCommand.run.rawValue,
            OPNEXSystemCommand.execApprovalsGet.rawValue,
            OPNEXSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(OPNEXCapability.browser.rawValue) {
            commands.append(OPNEXBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(OPNEXCapability.camera.rawValue) {
        ...
        if capsSet.contains(OPNEXCapability.location.rawValue) {
