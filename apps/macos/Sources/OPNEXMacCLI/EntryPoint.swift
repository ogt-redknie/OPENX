...
struct OPNEXMacCLI {
    static func main() async {
        ...
        default:
            fputs("opnex-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      opnex-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      opnex-mac discover [--timeout <ms>] [--json] [--include-local]
      opnex-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      opnex-mac connect
      opnex-mac connect --url ws://127.0.0.1:18789 --json
      opnex-mac discover --timeout 3000 --json
      opnex-mac wizard --mode local
    """)
}
