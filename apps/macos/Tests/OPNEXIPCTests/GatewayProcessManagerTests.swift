...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["OPNEX_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "opnex-gateway",
                executablePath: "/tmp/opnex-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 opnex-gateway @ /tmp/opnex-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
