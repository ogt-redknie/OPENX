...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: OPNEXProtocol.AnyCodable(["ok": OPNEXProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OPNEXProtocol.AnyCodable(["ok": OPNEXProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = OPNEXProtocol.AnyCodable([
            "runId": OPNEXProtocol.AnyCodable("run-1"),
            "sessionKey": OPNEXProtocol.AnyCodable("main"),
            "state": OPNEXProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OPNEXProtocol.AnyCodable(["a": OPNEXProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
