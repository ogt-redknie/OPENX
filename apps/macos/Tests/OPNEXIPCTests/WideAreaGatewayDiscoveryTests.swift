...
import Foundation
import Testing
@testable import OPNEXDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("OPNEX_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("OPNEX_WIDE_AREA_DOMAIN", "opnex.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("OPNEX_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("OPNEX_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._opnex-gw._tcp.opnex.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.opnex.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (OPNEX)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/opnex/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (OPNEX)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/opnex/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._opnex-gw._tcp.opnex.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
