...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: OPNEXProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OPNEXProtocol.AnyCodable] = [
            "presence": .init([OPNEXProtocol.AnyCodable(entry)]),
        ]
        let payload = OPNEXProtocol.AnyCodable(payloadMap)
