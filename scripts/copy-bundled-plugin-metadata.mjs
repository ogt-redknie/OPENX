...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.OPNEX_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@opnex/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "opnex.plugin.json");
    ...
    if (packageJson.opnex && "extensions" in packageJson.opnex) {
      packageJson.opnex = {
        ...packageJson.opnex,
        extensions: rewritePackageExtensions(packageJson.opnex.extensions),
        ...(typeof packageJson.opnex.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.opnex.setupEntry) }
          : {}),
      };
    }
