...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.opnex?.extensions)
    ? packageJson.opnex.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.opnex?.setupEntry === "string" &&
    packageJson.opnex.setupEntry.trim().length > 0
      ? packageJson.opnex.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "opnex.plugin.json"));
    }
    if (hasPackageJson) {
