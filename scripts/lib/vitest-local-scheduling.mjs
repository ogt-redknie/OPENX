...
function isSystemThrottleDisabled(env) {
  const normalized = env.OPNEX_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.OPNEX_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.OPNEX_VITEST_MAX_WORKERS ?? env.OPNEX_TEST_WORKERS);
  if (override !== null) {
