...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "opnex-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "opnex.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".opnex", "workspace");
