...
BASE_IMAGE="${BASE_IMAGE:-opnex-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-opnex-sandbox-common:bookworm-slim}"
...
OPNEX_DOCKER_BUILD_USE_BUILDX="${OPNEX_DOCKER_BUILD_USE_BUILDX:-0}"
OPNEX_DOCKER_BUILD_CACHE_FROM="${OPNEX_DOCKER_BUILD_CACHE_FROM:-}"
OPNEX_DOCKER_BUILD_CACHE_TO="${OPNEX_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=opnex.sandbox=1)
NOTE
