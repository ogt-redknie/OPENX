...
#
# Suggested cron: */30 * * * * /home/admin/opnex/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OPNEX notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opnex-alerts)
...
STATE_FILE="$HOME/.opnex/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OPNEX if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opnex
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OPNEX to $NOTIFY_PHONE..."
            opnex send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OPNEX is down. Run: ssh l36 '~/opnex/scripts/mobile-reauth.sh'" "urgent"
    exit 1
