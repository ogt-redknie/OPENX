...
function usage() {
  return "Usage: node scripts/check-opnex-package-tarball.mjs <opnex.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`OPNEX package tarball does not exist: ${tarball}`);
}
...
if (errors.length > 0) {
  fail(`OPNEX package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`OPNEX package tarball integrity warning: ${warning}`);
}
console.log("OPNEX package tarball integrity passed.");
