#!/usr/bin/env bash
# ClawDock - Docker helpers for OPNEX
# Inspired by Simon Willison's "Running OPNEX in Docker"
# https://til.simonwillison.net/llms/opnex-docker
#
# Installation:
...
# Common paths to check for OPNEX
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/opnex"
  "${HOME}/workspace/opnex"
  "${HOME}/projects/opnex"
  "${HOME}/dev/opnex"
  "${HOME}/code/opnex"
  "${HOME}/src/opnex"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OPNEX at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/opnex"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OPNEX not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/opnex/opnex.git ~/opnex"
    echo "  cd ~/opnex && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPNEX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d opnex-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart opnex-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f opnex-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.opnex"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show opnex.json
  if [[ -f "${config_dir}/opnex.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/opnex.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/opnex.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No opnex.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec opnex-gateway \
    bash -c 'echo "alias opnex=\"./opnex.mjs\"" > /tmp/.bashrc_opnex && bash --rcfile /tmp/.bashrc_opnex'
}
...
clawdock-cli() {
  _clawdock_compose run --rm opnex-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build opnex-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d opnex-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build opnex-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPNEX_GATEWAY_TOKEN=$token" opnex-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" opnex-gateway \
    bash -c './opnex.mjs config set gateway.remote.token "$TOKEN" && ./opnex.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec opnex-gateway \
    bash -c "./opnex.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart opnex-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm opnex-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec opnex-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'opnex config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for OPNEX${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (opnex alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to opnex project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.opnex)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opnex channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opnex status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.opnex.ai${_CLR_RESET}"
  echo ""
}
