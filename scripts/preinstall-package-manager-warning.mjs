...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[opnex] warning: detected ${packageManager} for install lifecycle.`,
    "[opnex] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[opnex] prefer: corepack pnpm install",
  ].join("\n");
}
