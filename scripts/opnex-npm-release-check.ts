...
const EXPECTED_REPOSITORY_URL = "https://github.com/opnex/opnex";
...
const skipPackValidationEnv = "OPNEX_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "opnex") {
    errors.push(`package.json name must be "opnex"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.opnex !== "opnex.mjs") {
    errors.push(
      `package.json bin.opnex must be "opnex.mjs"; found "${pkg.bin?.opnex ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`opnex-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `opnex-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
