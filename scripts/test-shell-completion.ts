 ...
 * `opnex update`. Use it to verify the completion installation flow
...
\`opnex update\`, \`opnex doctor\`, and \`opnex onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OPNEX Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opnex completion --install\` later to enable.`));
    return;
  }
