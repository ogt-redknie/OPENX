...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.opnex?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.opnex?.channel?.order;
      const labelRaw = parsed.opnex?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".opnex-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "opnex-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "opnex-build",
    ...
    OPNEX_PLUGIN_MANIFEST_CACHE_MS: "0",
...
function renderSourceBrowserHelpText(
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        OPNEX_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
