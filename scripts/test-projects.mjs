...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.OPNEX_VITEST_MAX_WORKERS || env.OPNEX_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      OPNEX_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.OPNEX_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.OPNEX_TEST_PROJECTS_PARALLEL &&
        !baseEnv.OPNEX_VITEST_MAX_WORKERS &&
        !baseEnv.OPNEX_TEST_WORKERS &&
