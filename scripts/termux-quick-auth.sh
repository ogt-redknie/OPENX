...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/opnex/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OPNEX Re-Auth" -c "After getting key, run: ssh $SERVER '~/opnex/scripts/mobile-reauth.sh'" --id opnex-auth
        ;;
    *)
