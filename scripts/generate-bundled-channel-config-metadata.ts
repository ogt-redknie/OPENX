...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const opnexMeta =
    ...
    "opnex" in source.packageJson
      ? (source.packageJson.opnex as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    opnexMeta &&
    typeof opnexMeta.channel === "object" &&
    opnexMeta.channel &&
    !Array.isArray(opnexMeta.channel)
      ? (opnexMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
