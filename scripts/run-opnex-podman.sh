#!/usr/bin/env bash
# Rootless OPNEX in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-opnex-podman.sh launch        # Start gateway
#   ./scripts/run-opnex-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   opnex --container opnex dashboard --no-open
#   opnex --container opnex channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      OPNEX_GATEWAY_TOKEN|OPNEX_PODMAN_CONTAINER|OPNEX_PODMAN_IMAGE|OPNEX_IMAGE|OPNEX_PODMAN_PULL|OPNEX_PODMAN_GATEWAY_HOST_PORT|OPNEX_GATEWAY_PORT|OPNEX_PODMAN_BRIDGE_HOST_PORT|OPNEX_BRIDGE_PORT|OPNEX_GATEWAY_BIND|OPNEX_PODMAN_USERNS|OPNEX_BIND_MOUNT_OPTIONS|OPNEX_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-opnex-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${OPNEX_CONFIG_DIR:-$EFFECTIVE_HOME/.opnex}"
ENV_FILE="${OPNEX_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${OPNEX_CONFIG_DIR:-$EFFECTIVE_HOME/.opnex}"
ENV_FILE="${OPNEX_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${OPNEX_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${OPNEX_PODMAN_CONTAINER:-opnex}"
OPNEX_IMAGE="${OPNEX_PODMAN_IMAGE:-${OPNEX_IMAGE:-opnex:local}}"
PODMAN_PULL="${OPNEX_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${OPNEX_PODMAN_GATEWAY_HOST_PORT:-${OPNEX_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${OPNEX_PODMAN_BRIDGE_HOST_PORT:-${OPNEX_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${OPNEX_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$OPNEX_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  OPNEX_CONTAINER="" OPNEX_CONFIG_DIR="$config_dir" \
    opnex config get gateway.bind 2>/dev/null || true
}
...
# OPNEX_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${OPNEX_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v opnex >/dev/null 2>&1; then
    echo "Warning: opnex not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    OPNEX_CONTAINER="" OPNEX_CONFIG_DIR="$config_dir" \
      opnex config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    OPNEX_CONTAINER="" OPNEX_CONFIG_DIR="$config_dir" \
      opnex config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  OPNEX_CONTAINER="" OPNEX_CONFIG_DIR="$config_dir" \
    opnex config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${OPNEX_GATEWAY_TOKEN:-}" ]]; then
  export OPNEX_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "OPNEX_GATEWAY_TOKEN" "$OPNEX_GATEWAY_TOKEN"
  echo "Generated OPNEX_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/opnex.json"
...
PODMAN_USERNS="${OPNEX_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported OPNEX_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (OPNEX_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${OPNEX_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${OPNEX_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$OPNEX_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.opnex/.npm \
    -e OPNEX_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.opnex:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.opnex/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$OPNEX_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME opnex dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME opnex devices approve --latest  # if pairing required"
