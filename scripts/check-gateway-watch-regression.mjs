...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  OPNEX_SKIP_ACPX_RUNTIME: "1",
  OPNEX_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$OPNEX_WATCH_PID_FILE"',
    'mkdir -p "$OPNEX_HOME/.opnex"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$OPNEX_HOME/.opnex/opnex.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    OPNEX_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    OPNEX_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-gateway-watch-"));
