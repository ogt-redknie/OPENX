#!/usr/bin/env bash
# Verifies `opnex plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "opnex-plugin-update-e2e" OPNEX_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${OPNEX_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${OPNEX_CURRENT_PACKAGE_TGZ:-}")"
...
npm install -g --prefix /tmp/npm-prefix \"\$package_tgz\" --no-fund --no-audit >/tmp/opnex-install.log 2>&1
entry=\"/tmp/npm-prefix/lib/node_modules/opnex/dist/index.mjs\"
[ -f \"\$entry\" ] || entry=/tmp/npm-prefix/lib/node_modules/opnex/dist/index.js
package_version=\$(node -p \"require('/tmp/npm-prefix/lib/node_modules/opnex/package.json').version\")
...
mkdir -p \"\$HOME/.opnex/extensions/lossless-claw\"
cat > \"\$HOME/.opnex/extensions/lossless-claw/package.json\" <<'JSON'
{
  \"name\": \"@example/lossless-claw\",
  \"version\": \"0.9.0\"
}
JSON
if [ \"\$OPNEX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT\" = \"1\" ]; then
...
mkdir -p \"\$HOME/.opnex/plugins\"
cat > \"\$HOME/.opnex/plugins/installs.json\" <<'JSON'
{
  \"version\": 1,
  \"warning\": \"DO NOT EDIT. This file is generated by OPNEX plugin registry commands.\",
...
cat > /tmp/opnex-e2e-registry.mjs <<'NODE'
...
NODE
node /tmp/opnex-e2e-registry.mjs >/tmp/opnex-e2e-registry.log 2>&1 &
registry_pid=\$!
...
if [ \"\$registry_ready\" -ne 1 ]; then
  ...
  cat /tmp/opnex-e2e-registry.log || true
  exit 1
fi

before_config_hash=\"\"
if [ \"\$OPNEX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT\" != \"1\" ]; then
  before_config_hash=\$(sha256sum \"\$HOME/.opnex/opnex.json\" | awk '{print \$1}')
fi
...
node --input-type=module > /tmp/plugin-update-before.json <<'NODE'
  ...
  const config = readJson(path.join(home, \".opnex\", \"opnex.json\"));
  const index = readJson(path.join(home, \".opnex\", \"plugins\", \"installs.json\"));
...
if [ -n \"\$before_config_hash\" ]; then
  after_config_hash=\$(sha256sum \"\$HOME/.opnex/opnex.json\" | awk '{print \$1}')
