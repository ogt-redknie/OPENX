...
// against the OPNEX npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/opnex-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      OPNEX_STATE_DIR: "/tmp/opnex-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[opnex mcp] ${String(chunk)}`);
  });
