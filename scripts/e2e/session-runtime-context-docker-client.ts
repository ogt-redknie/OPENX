...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".opnex", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "opnex.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("OPNEX_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-session-runtime-context-"));
  process.env.HOME = root;
  process.env.OPNEX_STATE_DIR = path.join(root, ".opnex");
  process.env.OPNEX_CONFIG_PATH = path.join(process.env.OPNEX_STATE_DIR, "opnex.json");
  try {
    ...
    if (process.env.OPNEX_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
