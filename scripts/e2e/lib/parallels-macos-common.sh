...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/opnex-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/opnex-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${OPNEX_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${OPNEX_SECRET_ENV_NAME:-}" ] && [ -n "${OPNEX_SECRET_FILE:-}" ] && [ -f "$OPNEX_SECRET_FILE" ]; then
  secret_value="$(cat "$OPNEX_SECRET_FILE")"
  rm -f "$OPNEX_SECRET_FILE"
  export "${OPNEX_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "OPNEX_SECRET_ENV_NAME=$api_key_env" \
      "OPNEX_SECRET_FILE=$secret_path" \
      "OPNEX_WRAPPER_FILE=$wrapper_path" \
