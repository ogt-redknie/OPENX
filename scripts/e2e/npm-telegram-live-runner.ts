...
function resolveCredentialSource(env: NodeJS.ProcessEnv) {
  return env.OPNEX_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.OPNEX_QA_CREDENTIAL_SOURCE;
}

function resolveCredentialRole(env: NodeJS.ProcessEnv) {
  return env.OPNEX_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.OPNEX_QA_CREDENTIAL_ROLE;
}

async function resolveTrustedOPNEXCommand(rawCommand: string) {
  if (!path.isAbsolute(rawCommand)) {
    throw new Error("OPNEX_NPM_TELEGRAM_SUT_COMMAND must be an absolute path.");
  }
  const commandName = path.basename(rawCommand);
  if (commandName !== "opnex" && commandName !== "opnex.cmd") {
    throw new Error(
      `OPNEX_NPM_TELEGRAM_SUT_COMMAND must point to opnex; got: ${commandName}`,
    );
  }
  const npmPrefix = process.env.NPM_CONFIG_PREFIX?.trim();
  if (!npmPrefix) {
    throw new Error("Missing NPM_CONFIG_PREFIX for installed opnex command validation.");
  }
  ...
  if (realCommand !== realPrefix && !realCommand.startsWith(`${realPrefix}${path.sep}`)) {
    throw new Error("OPNEX_NPM_TELEGRAM_SUT_COMMAND must resolve inside NPM_CONFIG_PREFIX.");
  }
  return rawCommand;
}
...
async function main() {
  ...
  const rawSutOPNEXCommand = process.env.OPNEX_NPM_TELEGRAM_SUT_COMMAND?.trim();
  if (!rawSutOPNEXCommand) {
    throw new Error("Missing OPNEX_NPM_TELEGRAM_SUT_COMMAND.");
  }
  const sutOPNEXCommand = await resolveTrustedOPNEXCommand(rawSutOPNEXCommand);

  const repoRoot = path.resolve(process.env.OPNEX_NPM_TELEGRAM_REPO_ROOT ?? process.cwd());
  const outputDir =
    process.env.OPNEX_NPM_TELEGRAM_OUTPUT_DIR?.trim() ||
  ...
  const result = await runTelegramQaLive({
    ...
    providerMode: process.env.OPNEX_NPM_TELEGRAM_PROVIDER_MODE,
    primaryModel: process.env.OPNEX_NPM_TELEGRAM_MODEL,
    alternateModel: process.env.OPNEX_NPM_TELEGRAM_ALT_MODEL,
    fastMode: parseBoolean(process.env.OPNEX_NPM_TELEGRAM_FAST),
    scenarioIds: splitCsv(process.env.OPNEX_NPM_TELEGRAM_SCENARIOS),
    sutAccountId: process.env.OPNEX_NPM_TELEGRAM_SUT_ACCOUNT,
  ...
  if (
    !parseBoolean(process.env.OPNEX_NPM_TELEGRAM_ALLOW_FAILURES) &&
