...
import { applyDockerOpenAiProviderConfig, type OPNEXConfig } from "./docker-openai-seed.ts";
...
process.title = "opnex-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.OPNEX_STATE_DIR?.trim() || path.join(os.homedir(), ".opnex");
  const configPath =
    process.env.OPNEX_CONFIG_PATH?.trim() || path.join(stateDir, "opnex.json");
