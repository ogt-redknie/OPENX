#!/usr/bin/env bash
# Runs bundled plugin runtime-dependency Docker scenarios from a mounted OPNEX
...
IMAGE_NAME="$(docker_e2e_resolve_image "opnex-bundled-channel-deps-e2e" OPNEX_BUNDLED_CHANNEL_DEPS_E2E_IMAGE)"
UPDATE_BASELINE_VERSION="${OPNEX_BUNDLED_CHANNEL_UPDATE_BASELINE_VERSION:-2026.4.20}"
DOCKER_TARGET="${OPNEX_BUNDLED_CHANNEL_DOCKER_TARGET:-bare}"
HOST_BUILD="${OPNEX_BUNDLED_CHANNEL_HOST_BUILD:-1}"
PACKAGE_TGZ="${OPNEX_CURRENT_PACKAGE_TGZ:-}"
RUN_CHANNEL_SCENARIOS="${OPNEX_BUNDLED_CHANNEL_SCENARIOS:-1}"
RUN_UPDATE_SCENARIO="${OPNEX_BUNDLED_CHANNEL_UPDATE_SCENARIO:-1}"
RUN_ROOT_OWNED_SCENARIO="${OPNEX_BUNDLED_CHANNEL_ROOT_OWNED_SCENARIO:-1}"
RUN_SETUP_ENTRY_SCENARIO="${OPNEX_BUNDLED_CHANNEL_SETUP_ENTRY_SCENARIO:-1}"
RUN_LOAD_FAILURE_SCENARIO="${OPNEX_BUNDLED_CHANNEL_LOAD_FAILURE_SCENARIO:-1}"
RUN_DISABLED_CONFIG_SCENARIO="${OPNEX_BUNDLED_CHANNEL_DISABLED_CONFIG_SCENARIO:-1}"
CHANNEL_ONLY="${OPNEX_BUNDLED_CHANNEL_ONLY:-}"
DOCKER_RUN_TIMEOUT="${OPNEX_BUNDLED_CHANNEL_DOCKER_RUN_TIMEOUT:-900s}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${OPNEX_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "OPNEX_BUNDLED_CHANNEL_HOST_BUILD=0 requires OPNEX_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
