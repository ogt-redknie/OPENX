...
IMAGE_NAME="$(docker_e2e_resolve_image "opnex-plugins-e2e" OPNEX_PLUGINS_E2E_IMAGE)"

docker_e2e_build_or_reuse "$IMAGE_NAME" plugins

DOCKER_ENV_ARGS=(-e COREPACK_ENABLE_DOWNLOAD_PROMPT=0)
for env_name in \
  OPNEX_PLUGINS_E2E_CLAWHUB \
  OPNEX_PLUGINS_E2E_CLAWHUB_SPEC \
  OPNEX_PLUGINS_E2E_CLAWHUB_ID \
  OPNEX_CLAWHUB_URL \
  CLAWHUB_URL \
  OPNEX_CLAWHUB_TOKEN \
  CLAWHUB_TOKEN \
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/opnex-plugins-run.XXXXXX")"
...
home_dir=$(mktemp -d "/tmp/opnex-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
OPNEX_PLUGIN_HOME="$HOME/.opnex/$BUNDLED_PLUGIN_ROOT_DIR"
...
const ledgerPath = path.join(process.env.HOME, ".opnex", "plugins", "installs.json");
const ledger = fs.existsSync(ledgerPath)
  ? JSON.parse(fs.readFileSync(ledgerPath, "utf8"))
  : {
      version: 1,
      warning:
        "DO NOT EDIT. This file is generated by OPNEX plugin install/update/uninstall commands. Use `opnex plugins install/update/uninstall` instead.",
      records: {},
    };
...
run_logged() {
  local label="$1"
  shift
  local log_file="/tmp/opnex-plugins-e2e-${label}.log"
...
JS
  cat > "$dir/opnex.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/opnex.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$OPNEX_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/opnex.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins.json
node "$OPNEX_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/opnex-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@opnex/demo-plugin-tgz",
  "version": "0.0.1",
  "opnex": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/opnex.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
run_logged install-tgz node "$OPNEX_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins2.json
node "$OPNEX_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/opnex-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@opnex/demo-plugin-dir",
  "version": "0.0.1",
  "opnex": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/opnex.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
run_logged install-dir node "$OPNEX_ENTRY" plugins install "$dir_plugin"
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins3.json
node "$OPNEX_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/opnex-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@opnex/demo-plugin-file",
  "version": "0.0.1",
  "opnex": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/opnex.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
run_logged install-file node "$OPNEX_ENTRY" plugins install "file:$file_pack_dir/package"
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins4.json
node "$OPNEX_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$OPNEX_PLUGIN_HOME/$bundle_plugin_id"
...
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins-bundle-disabled.json
node - <<'NODE'
const fs = require("node:fs");
...
run_logged enable-claude-bundle node "$OPNEX_ENTRY" plugins enable claude-bundle-e2e
node "$OPNEX_ENTRY" plugins inspect claude-bundle-e2e --json > /tmp/plugins-bundle-inspect.json
node - <<'NODE'
const fs = require("node:fs");
...
slash_install_dir="$(mktemp -d "/tmp/opnex-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@opnex/slash-install-plugin",
  "version": "0.0.1",
  "opnex": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/opnex.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
run_logged install-slash-plugin node "$OPNEX_ENTRY" plugins install "$slash_install_dir"
node "$OPNEX_ENTRY" plugins inspect slash-install-plugin --json > /tmp/plugin-command-install-show.json
node - <<'NODE'
const fs = require("node:fs");
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "opnex/fixture-marketplace"
    }
  }
}
JSON

node "$OPNEX_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$OPNEX_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$OPNEX_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$OPNEX_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$OPNEX_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
const allowLegacyCompat = process.env.OPNEX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$OPNEX_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$OPNEX_ENTRY" plugins update marketplace-shortcut
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$OPNEX_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
...
if [ "${OPNEX_PLUGINS_E2E_CLAWHUB:-1}" = "0" ]; then
  echo "Skipping ClawHub plugin install and uninstall (OPNEX_PLUGINS_E2E_CLAWHUB=0)."
else
echo "Testing ClawHub plugin install and uninstall..."
CLAWHUB_PLUGIN_SPEC="${OPNEX_PLUGINS_E2E_CLAWHUB_SPEC:-clawhub:opnex-now4real}"
CLAWHUB_PLUGIN_ID="${OPNEX_PLUGINS_E2E_CLAWHUB_ID:-now4real}"
...
const packageName = "opnex-now4real";
...
NODE
  ...
  for _ in $(seq 1 100); do
    if [[ -s "$server_port_file" ]]; then
      export OPNEX_CLAWHUB_URL="http://127.0.0.1:$(cat "$server_port_file")"
...
if [[ -z "${OPNEX_CLAWHUB_URL:-}" && -z "${CLAWHUB_URL:-}" ]]; then
  # Keep the release-path smoke hermetic; live ClawHub can rate-limit CI.
  clawhub_fixture_dir="$(mktemp -d "/tmp/opnex-clawhub-fixture.XXXXXX")"
...
const baseUrl = (process.env.OPNEX_CLAWHUB_URL || process.env.CLAWHUB_URL || "https://clawhub.ai")
  .replace(/\/+$/, "");
const token =
  process.env.OPNEX_CLAWHUB_TOKEN ||
...
run_logged install-clawhub node "$OPNEX_ENTRY" plugins install "$CLAWHUB_PLUGIN_SPEC"
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins-clawhub-installed.json
node "$OPNEX_ENTRY" plugins inspect "$CLAWHUB_PLUGIN_ID" --json > /tmp/plugins-clawhub-inspect.json
...
const extensionsRoot = path.join(process.env.HOME, ".opnex", "extensions");
...
run_logged uninstall-clawhub node "$OPNEX_ENTRY" plugins uninstall "$CLAWHUB_PLUGIN_SPEC" --force
node "$OPNEX_ENTRY" plugins list --json > /tmp/plugins-clawhub-uninstalled.json
...
const configAfterUninstallPath = path.join(process.env.HOME, ".opnex", "opnex.json");
