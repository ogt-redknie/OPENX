...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${OPNEX_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${OPNEX_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${OPNEX_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${OPNEX_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
SMOKE_DIR=""
PACK_DIR=""
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "OPNEX_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (OPNEX_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set OPNEX_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/opnex-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack OPNEX tarball"
...
' "$pack_json_file" "$PACK_DIR"
  ...
  if [ -z "$PACKAGE_TGZ" ] || [ ! -f "$PACKAGE_TGZ" ]; then
    echo "missing packed OPNEX tarball" >&2
    exit 1
  fi
}
...
main() {
  ...
  echo "==> Bun global install packed OPNEX"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  opnex_bin="$BUN_INSTALL/bin/opnex"
  if [ ! -x "$opnex_bin" ]; then
    opnex_bin="$(command -v opnex || true)"
  fi
  if [ -z "$opnex_bin" ] || [ ! -x "$opnex_bin" ]; then
    echo "Bun global install did not create an executable opnex binary" >&2
    exit 1
  fi

  echo "==> OPNEX version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$opnex_bin" --version

  echo "==> OPNEX image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$opnex_bin" infer image providers --json)"
  OPNEX_IMAGE_PROVIDERS_JSON="$providers_json" node - <<'NODE'
const raw = process.env.OPNEX_IMAGE_PROVIDERS_JSON ?? "";
let parsed;
try {
