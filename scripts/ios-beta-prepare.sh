...
- regenerates apps/ios/OPNEX.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${OPNEX_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid OPNEX_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid OPNEX_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid OPNEX_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid OPNEX_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing OPNEX_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(OPNEX_URL_SLASH)$(OPNEX_URL_SLASH)#g'
)"
...
OPNEX_CODE_SIGN_STYLE = Automatic
OPNEX_DEVELOPMENT_TEAM = ${TEAM_ID}
OPNEX_IOS_SELECTED_TEAM = ${TEAM_ID}
OPNEX_APP_BUNDLE_ID = ai.opnex.client
OPNEX_SHARE_BUNDLE_ID = ai.opnex.client.share
OPNEX_ACTIVITY_WIDGET_BUNDLE_ID = ai.opnex.client.activitywidget
OPNEX_WATCH_APP_BUNDLE_ID = ai.opnex.client.watchkitapp
OPNEX_WATCH_EXTENSION_BUNDLE_ID = ai.opnex.client.watchkitapp.extension
OPNEX_APP_PROFILE =
OPNEX_SHARE_PROFILE =
OPNEX_PUSH_TRANSPORT = relay
OPNEX_PUSH_DISTRIBUTION = official
OPNEX_URL_SLASH = /
OPNEX_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
OPNEX_PUSH_APNS_ENVIRONMENT = production
EOF
