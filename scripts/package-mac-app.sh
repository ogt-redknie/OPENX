...
# Build and bundle OPNEX into a minimal .app we can open.
# Outputs to dist/OPNEX.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OPNEX.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OPNEX"
MLX_TTS_HELPER_PRODUCT="opnex-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.opnex.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opnex/opnex/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OPNEX/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OPNEXBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OPNEXGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OPNEX"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OPNEX"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OPNEX"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OPNEX" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OPNEX/Resources/OPNEX.icns" "$APP_ROOT/Contents/Resources/OPNEX.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OPNEX/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OPNEXKit resources"
OPNEXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OPNEXKit_OPNEXKit.bundle"
if [ -d "$OPNEXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OPNEXKit_OPNEXKit.bundle"
  cp -R "$OPNEXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OPNEXKit_OPNEXKit.bundle"
else
  echo "WARN: OPNEXKit resource bundle not found at $OPNEXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OPNEX"
killall -q OPNEX 2>/dev/null || true
