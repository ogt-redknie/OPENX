...
type DriverMode = "token" | "webhook" | "opnex";
...
function resolveStateDir(): string {
  const override = process.env.OPNEX_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPNEX_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".opnex");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver opnex] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OPNEX spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|opnex> Driver transport mode (default: token)\n" +
    ...
    "  --opnex-bin <path>        OPNEX CLI binary for driver=opnex (default: opnex)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPNEX_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPNEX_DISCORD_SMOKE_DRIVER\n" +
    "  OPNEX_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPNEX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPNEX_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPNEX_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPNEX_DISCORD_SMOKE_AGENT\n" +
    "  OPNEX_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPNEX_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPNEX_DISCORD_SMOKE_POLL_MS\n" +
    "  OPNEX_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPNEX_DISCORD_SMOKE_OPNEX_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.OPNEX_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.OPNEX_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "opnex"
        ? "opnex"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.OPNEX_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPNEX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPNEX_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPNEX_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.OPNEX_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.OPNEX_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.OPNEX_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPNEX_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPNEX_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPNEX_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const opnexBin =
    resolveArg("--opnex-bin") || process.env.OPNEX_DISCORD_SMOKE_OPNEX_BIN || "opnex";
...
async function opnexCliJson<T>(params: { opnexBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.opnexBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`opnex ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithOpnex(params: {
  opnexBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await opnexCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    opnexBin: params.opnexBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "opnex") {
    return await readMessagesWithOpnex({
      opnexBin: params.args.opnexBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `opnex-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("opnex message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "opnex"
            ? await readMessagesWithOpnex({
                opnexBin: args.opnexBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OPNEX.`,
        diagnostics: {
          bindingCandidates: [
            {
