...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.opnex) ? packageJson.opnex : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.opnex?.channel?.id) || left.name;
    const rightId = trimString(right.opnex?.channel?.id) || right.name;
