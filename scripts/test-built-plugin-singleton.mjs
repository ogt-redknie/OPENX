...
const { clearPluginCommands, getPluginCommandSpecs, loadOPNEXPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadOPNEXPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "opnex.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'opnex/plugin-sdk';",
...
const registry = loadOPNEXPlugins({
  ...
  env: {
    ...process.env,
    OPNEX_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
