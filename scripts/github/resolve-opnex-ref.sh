...
REMOTE_URL="${OPNEX_REF_REMOTE:-https://github.com/opnex/opnex.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-opnex-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves OPNEX branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve OPNEX ref: ${REF}" >&2
exit 1
