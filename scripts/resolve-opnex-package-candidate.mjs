...
const DEFAULT_OUTPUT_NAME = "opnex-current.tgz";
export const OPNEX_PACKAGE_SPEC_RE =
  /^opnex@(beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-beta\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-opnex-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateOPNEXPackageSpec(spec) {
  if (!OPNEX_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be opnex@beta, opnex@latest, or an exact OPNEX release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an OPNEX branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `opnex-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^opnex-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no OPNEX tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-opnex-for-docker.mjs",
        "--source-dir",
      ...
      validateOPNEXPackageSpec(options.packageSpec);
  ...
  await run("node", ["scripts/check-opnex-package-tarball.mjs", target]);
  ...
  if (pkg.name !== "opnex") {
    throw new Error(`package candidate must be named "opnex"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
