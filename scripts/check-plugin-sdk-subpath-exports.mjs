...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("opnex/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced opnex/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced opnex/plugin-sdk/<subpath> must exist in the public package exports.",
  );
