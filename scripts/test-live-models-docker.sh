...
if opnex_live_truthy "${OPNEX_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  OPNEX_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${OPNEX_LIVE_PROVIDERS:-}" || -n "${OPNEX_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      opnex_live_collect_auth_dirs_from_csv "${OPNEX_LIVE_PROVIDERS:-}"
      opnex_live_collect_auth_dirs_from_csv "${OPNEX_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      opnex_live_collect_auth_files_from_csv "${OPNEX_LIVE_PROVIDERS:-}"
      opnex_live_collect_auth_files_from_csv "${OPNEX_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${OPNEX_DOCKER_PROFILE_ENV_ONLY:-0}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e OPNEX_LIVE_TEST=1 \
  -e OPNEX_LIVE_MODELS="${OPNEX_LIVE_MODELS:-modern}" \
  -e OPNEX_LIVE_PROVIDERS="${OPNEX_LIVE_PROVIDERS:-}" \
  -e OPNEX_LIVE_MAX_MODELS="${OPNEX_LIVE_MAX_MODELS:-12}" \
  -e OPNEX_LIVE_MODEL_TIMEOUT_MS="${OPNEX_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e OPNEX_LIVE_REQUIRE_PROFILE_KEYS="${OPNEX_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e OPNEX_LIVE_GATEWAY_MODELS="${OPNEX_LIVE_GATEWAY_MODELS:-}" \
  -e OPNEX_LIVE_GATEWAY_PROVIDERS="${OPNEX_LIVE_GATEWAY_PROVIDERS:-}" \
  -e OPNEX_LIVE_GATEWAY_MAX_MODELS="${OPNEX_LIVE_GATEWAY_MAX_MODELS:-}")
