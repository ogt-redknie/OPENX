#!/usr/bin/env node
// Builds the OPNEX package artifact used by Docker E2E.
...
async function newestOPNEXTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed OPNEX tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building OPNEX package artifacts");
    await run("pnpm", ["build"], sourceDir);
  }

  console.error("==> Writing OPNEX package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing OPNEX package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestOPNEXTarball(outputDir, packOutput);
  ...
  console.error("==> Checking OPNEX package tarball");
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-opnex-package-tarball.mjs"), tarball],
    sourceDir,
  );
