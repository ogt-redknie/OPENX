#!/bin/bash
# Setup OPNEX Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'opnex-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OPNEX message: Send warning via OPNEX itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opnex-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opnex-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opnex-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opnex-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opnex-auth-monitor.timer"
echo ""
echo "Quick commands:"
