...
import { discoverOPNEXPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']opnex\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']opnex\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']opnex\/plugin-sdk\/channel-runtime["']/,
    label: "opnex/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']opnex\/plugin-sdk\/config-runtime["']/,
    label: "opnex/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']opnex\/plugin-sdk\/infra-runtime["']/,
    label: "opnex/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic opnex/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy opnex/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused opnex/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
