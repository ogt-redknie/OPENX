...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/opnex-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/opnex";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/opnex-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `opnex-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "OPNEX_CONFIG_PATH" &&
          key !== "OPNEX_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `OPNEX_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `OPNEX_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm opnex qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredOPNEXTmpDir(), `${plan.vmName}-qa-suite-`),
  );
