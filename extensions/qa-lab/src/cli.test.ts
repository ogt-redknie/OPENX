...
import type { QaRunnerCliContribution } from "opnex/plugin-sdk/qa-runner-runtime";
...
vi.mock("opnex/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "opnex", "qa", "aimock", "--port", "44080"]);
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "opnex", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "opnex", "qa", "telegram"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "opnex", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "opnex", "qa", "suite", "--allow-failures"]);
