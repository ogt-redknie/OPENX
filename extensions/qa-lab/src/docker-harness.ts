...
const QA_LAB_UI_OVERLAY_DIR = "/opt/opnex-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        OPNEX_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      OPNEX_CONFIG_PATH: /tmp/opnex/opnex.json
      ...
      OPNEX_PROFILE: ""
    volumes:
      - ./state:/opt/opnex-scaffold:ro
      - ${repoMount}:/opt/opnex-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/opnex/workspace /tmp/opnex/state && cp /opt/opnex-scaffold/opnex.json /tmp/opnex/opnex.json && cp -R /opt/opnex-scaffold/seed-workspace/. /tmp/opnex/workspace/ && ln -snf /opt/opnex-repo /tmp/opnex/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t opnex:qa-local-prebaked --build-arg OPNEX_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "opnex:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "opnex.json"),
  ];
