...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      __testing.resolveTelegramQaRuntimeEnv({
        OPNEX_QA_TELEGRAM_GROUP_ID: "-100123",
        OPNEX_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        OPNEX_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("OPNEX_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      __testing.resolveTelegramQaRuntimeEnv({
        OPNEX_QA_TELEGRAM_GROUP_ID: "qa-group",
        OPNEX_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        OPNEX_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("OPNEX_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies OPNEX_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
