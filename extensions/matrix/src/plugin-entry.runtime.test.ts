...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __opnexMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["opnex", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@opnex", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__opnexMatrixWrapperJitiOptions = options;",
...
function writeOPNEXPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "opnex.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeOPNEXAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedOPNEXBinFixture(
  fixtureRoot: string,
...
function writeCapturingSourceRuntimeWrapperFixture(fixtureRoot: string) {
  delete matrixWrapperGlobal.__opnexMatrixWrapperJitiOptions;
  writeOPNEXAliasFixture(fixtureRoot);
...
function expectSourcePluginSdkAliases(fixtureRoot: string) {
  expect(matrixWrapperGlobal.__opnexMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeOPNEXPackageFixture(fixtureRoot);
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__opnexMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "opnex/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__opnexMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted opnex package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__opnexMatrixWrapperJitiOptions;
  writeTrustedOPNEXBinFixture(fixtureRoot, "OPNEX.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeSourceRuntimeWrapperFixture(fixtureRoot);
  await importFixtureModule(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js");
