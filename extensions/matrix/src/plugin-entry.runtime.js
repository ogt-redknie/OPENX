...
const OPNEX_PLUGIN_SDK_PACKAGE_NAMES = [
  ["opnex", "plugin-sdk"].join("/"),
  ["@opnex", "plugin-sdk"].join("/"),
];
...
function hasTrustedOPNEXRootIndicator(packageRoot, packageJson) {
  ...
  const hasOPNEXBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("opnex")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.opnex === "string");
  const hasOPNEXEntrypoint = fs.existsSync(path.join(packageRoot, "opnex.mjs"));
  return hasCliEntryExport || hasOPNEXBin || hasOPNEXEntrypoint;
}

function findOPNEXPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "opnex" && hasTrustedOPNEXRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findOPNEXPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of OPNEX_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["opnex/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findOPNEXPackageRoot(moduleDir);
  if (location) {
