...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleOPNEXDevices.length > 0) {
      params.logger.warn(
        `matrix: stale OPNEX devices detected for ${params.auth.userId}: ${deviceHealth.staleOPNEXDevices.map((device) => device.deviceId).join(", ")}. Run 'opnex matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
