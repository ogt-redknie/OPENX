...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'opnex matrix verify status --verbose --account ${accountId}' and 'opnex matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another OPNEX runtime is using the same account. " +
    `Check 'opnex matrix verify status --verbose --account ${accountId}' and 'opnex matrix devices list --account ${accountId}'.`
  );
}
