...
} from "opnex/plugin-sdk/file-lock";
...
describe("Matrix IndexedDB persistence", () => {
  ...
  it("persists and restores database contents for the selected prefix", async () => {
    ...
    await seedDatabase({
      name: "opnex-matrix-test::matrix-sdk-crypto",
      storeName: "sessions",
    ...
    await persistIdbToDisk({
      snapshotPath,
      databasePrefix: "opnex-matrix-test",
    });
  ...
  it("serializes concurrent persist operations via file lock", async () => {
    ...
    await Promise.all([
      persistIdbToDisk({ snapshotPath, databasePrefix: "opnex-matrix-test" }),
      persistIdbToDisk({ snapshotPath, databasePrefix: "opnex-matrix-test" }),
    ]);
