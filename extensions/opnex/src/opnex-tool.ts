...
import {
  ...
  resolveOPNEXCwd,
  type OPNEXRunner,
  type OPNEXRunnerParams,
} from "./opnex-runner.js";
import {
  type ManagedOPNEXFlowResult,
  resumeManagedOPNEXFlow,
  runManagedOPNEXFlow,
} from "./opnex-taskflow.js";
...
function resolveManagedFlowToolResult(result: ManagedOPNEXFlowResult) {
  if (!result.ok) {
    throw result.error;
  }
...
export function createOPNEXTool(api: OPNEXPluginApi, options?: OPNEXToolOptions) {
  const runner = options?.runner ?? createEmbeddedOPNEXRunner();
  return {
    name: "opnex",
    label: "OPNEX Workflow",
    description:
      "Run OPNEX pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const cwd = resolveOPNEXCwd(params.cwd);
      const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 20_000;
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512_000;

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`opnex plugin runtime=${api.runtime.version}`);
      }

      const runnerParams: OPNEXRunnerParams = {
        action,
      ...
      if (action === "run") {
        ...
        if (flowParams) {
          return resolveManagedFlowToolResult(
            await resumeManagedOPNEXFlow({
              taskFlow: requireTaskFlowRuntime(taskFlow, "resume"),
              runner,
              runnerParams: runnerParams as OPNEXRunnerParams & {
                action: "resume";
                approve: boolean;
