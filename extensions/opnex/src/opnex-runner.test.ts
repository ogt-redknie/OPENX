...
import {
  createEmbeddedOPNEXRunner,
  loadEmbeddedToolRuntimeFromPackage,
  resolveOPNEXCwd,
} from "./opnex-runner.js";
...
describe("resolveOPNEXCwd", () => {
  it("defaults to the current working directory", () => {
    expect(resolveOPNEXCwd(undefined)).toBe(process.cwd());
  });

  it("keeps relative paths inside the repo root", () => {
    expect(resolveOPNEXCwd("extensions/opnex")).toBe(
      path.resolve(process.cwd(), "extensions/opnex"),
    );
  });
});

describe("createEmbeddedOPNEXRunner", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("runs inline pipelines through the embedded runtime", async () => {
    ...
    const runner = createEmbeddedOPNEXRunner({
  ...
  it("detects workflow files and parses argsJson", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-opnex-runner-"));
    const workflowPath = path.join(tempDir, "workflow.opnex");
  ...
  it("fails closed when the embedded runtime requests unsupported input", async () => {
    ...
    ).rejects.toThrow("OPNEX input requests are not supported by the OPNEX OPNEX tool yet");
  });
  ...
  it("loads the embedded runtime once per runner", async () => {
    ...
    const runner = createEmbeddedOPNEXRunner({ loadRuntime });
  ...
  it("deduplicates content-identical schema compilation in the installed OPNEX runtime", async () => {
    await loadEmbeddedToolRuntimeFromPackage();

    const corePath = requireForTest.resolve("opnex/core");
  ...
  it("falls back to the installed package core file when the core export is unavailable", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-opnex-package-"));
    const packageRoot = path.join(tempDir, "node_modules", "@clawdbot", "opnex");
