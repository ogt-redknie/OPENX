...
import { installOPNEXAjvCompileCache } from "./opnex-ajv-cache.js";

export type OPNEXEnvelope =
  | {
      ok: true;
...
export type OPNEXRunner = {
  run: (params: OPNEXRunnerParams) => Promise<OPNEXEnvelope>;
};
...
const opnexRequire = createRequire(import.meta.url);
...
function toEmbeddedToolRuntime(
  ...
  throw new Error(`${source} does not export OPNEX embedded runtime functions`);
}

function findOPNEXPackageRoot(resolvedEntryPath: string): string {
  let dir = path.dirname(resolvedEntryPath);
  while (true) {
    const packageJsonPath = path.join(dir, "package.json");
    try {
      const parsed = JSON.parse(readFileSync(packageJsonPath, "utf8")) as { name?: string };
      if (parsed.name === "opnex") {
        return dir;
      }
    } catch {
    ...
    if (parent === dir) {
      throw new Error(`Could not locate opnex package root from ${resolvedEntryPath}`);
    }
    dir = parent;
  }
}
...
export function resolveOPNEXCwd(cwdRaw: unknown): string {
...
function createLimitedSink(maxBytes: number, label: "stdout" | "stderr") {
  ...
  return new Writable({
    write(chunk, _encoding, callback) {
      ...
      if (bytes > maxBytes) {
        callback(new Error(`opnex ${label} exceeded maxStdoutBytes`));
        return;
      }
      callback();
    },
...
function normalizeEnvelope(envelope: EmbeddedToolEnvelope): OPNEXEnvelope {
  if (envelope.ok) {
    if (envelope.status === "needs_input") {
      return {
        ...
        error: {
          ...
          message: "OPNEX input requests are not supported by the OPNEX OPNEX tool yet",
        },
      };
    }
    return {
  ...
  return {
    ...
    error: {
      ...
      message: envelope.error?.message ?? "opnex runtime failed",
    },
  };
}

function throwOnErrorEnvelope(envelope: OPNEXEnvelope): Extract<OPNEXEnvelope, { ok: true }> {
  if (envelope.ok) {
    return envelope;
  }
...
async function resolveWorkflowFile(candidate: string, cwd: string) {
  ...
  if (![".opnex", ".yaml", ".yml", ".json"].includes(ext)) {
    throw new Error("Workflow file must end in .opnex, .yaml, .yml, or .json");
  }
  return resolved;
}
...
async function withTimeout<T>(
  ...
  return await new Promise<T>((resolve, reject) => {
    const onTimeout = () => {
      const error = new Error("opnex runtime timed out");
...
export async function loadEmbeddedToolRuntimeFromPackage(
  ...
  installOPNEXAjvCompileCache();

  const importModule =
    options.importModule ??
    (async (specifier: string) => (await import(specifier)) as Partial<EmbeddedToolRuntime>);
  const resolvePackageEntry =
    options.resolvePackageEntry ?? ((specifier: string) => opnexRequire.resolve(specifier));

  let coreLoadError: unknown;
  try {
    const coreSpecifier = ["@clawdbot", "opnex", "core"].join("/");
    return toEmbeddedToolRuntime(await importModule(coreSpecifier), "opnex/core");
  } catch (error) {
    coreLoadError = error;
  }
  ...
  try {
    const packageEntryPath = resolvePackageEntry("opnex");
    const packageRoot = findOPNEXPackageRoot(packageEntryPath);
  ...
  throw new Error("Failed to load the OPNEX embedded runtime", {
    cause: new AggregateError(
      [coreLoadError, fallbackLoadError],
      "Both OPNEX embedded runtime load paths failed",
    ),
  });
}

export function createEmbeddedOPNEXRunner(options?: {
