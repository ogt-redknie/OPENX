...
import type { OPNEXEnvelope, OPNEXRunner, OPNEXRunnerParams } from "./opnex-runner.js";
...
export type RunManagedOPNEXFlowParams = {
  taskFlow: BoundTaskFlow;
  runner: OPNEXRunner;
  runnerParams: OPNEXRunnerParams;
  controllerId: string;
  goal: string;
  stateJson?: JsonLike;
  currentStep?: string;
...
export type ResumeManagedOPNEXFlowParams = {
  taskFlow: BoundTaskFlow;
  runner: OPNEXRunner;
  runnerParams: OPNEXRunnerParams & {
    action: "resume";
...
export type ManagedOPNEXFlowResult =
  | {
      ok: true;
      envelope: OPNEXEnvelope;
      flow: FlowRecord;
...
function buildApprovalWaitState(envelope: Extract<OPNEXEnvelope, { ok: true }>): JsonLike {
  if (!envelope.requiresApproval) {
    return {
      kind: "opnex_approval",
      prompt: "",
      items: [],
    } satisfies OPNEXApprovalWaitState;
  }
  return {
    kind: "opnex_approval",
...
function buildEnvelopeError(envelope: Extract<OPNEXEnvelope, { ok: false }>) {
  return new Error(envelope.error.message);
}

export async function runManagedOPNEXFlow(
  params: RunManagedOPNEXFlowParams,
): Promise<ManagedOPNEXFlowResult> {
  const flow = params.taskFlow.createManaged({
    controllerId: params.controllerId,
    goal: params.goal,
    currentStep: params.currentStep ?? "run_opnex",
  ...
  try {
    ...
    const mutation = applyEnvelopeToFlow({
      ...
      flow,
      envelope,
      waitingStep: params.waitingStep ?? "await_opnex_approval",
    });
    if (!envelope.ok) {
      return {
        ok: false,
...
export async function resumeManagedOPNEXFlow(
  params: ResumeManagedOPNEXFlowParams,
): Promise<ManagedOPNEXFlowResult> {
  const resumed = params.taskFlow.resume({
    flowId: params.flowId,
    expectedRevision: params.expectedRevision,
    status: "running",
    currentStep: params.currentStep ?? "resume_opnex",
  });
