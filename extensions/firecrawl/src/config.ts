...
import { resolveSecretInputString, normalizeSecretInput } from "opnex/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<OPNEXConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OPNEXConfig): WebSearchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: OPNEXConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: OPNEXConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: OPNEXConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: OPNEXConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: OPNEXConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: OPNEXConfig, override?: number): number {
