    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex memory status", "Show index and provider status."],
          [
            "opnex memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["opnex memory status --deep", "Probe embedding provider readiness."],
          ["opnex memory index --force", "Force a full reindex."],
          ['opnex memory search "meeting notes"', "Quick search using positional query."],
          [
            'opnex memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `opnex memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "opnex memory rem-backfill --path ./memory",
          ...
          ["opnex memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.opnex.ai/cli/memory")}\n`,
    );
