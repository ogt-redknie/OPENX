...
import { formatMemoryDreamingDay } from "opnex/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "opnex/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "opnex-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*opnex-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*opnex-memory-promotion:([^\n]+?)\s*-->/gi);
