...
import { resolveMemoryRemDreamingConfig } from "opnex/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: OPNEXConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredOPNEXTmpDir(), "opnex-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: OPNEXConfig): string {
...
function resolveAgent(cfg: OPNEXConfig, agent?: string) {
...
function resolveAgentIds(cfg: OPNEXConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: opnex memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredOPNEXTmpDir(), "opnex-rem-backfill-"),
      );
      try {
