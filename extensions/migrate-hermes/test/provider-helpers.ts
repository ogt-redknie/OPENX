...
import type { MigrationProviderContext } from "opnex/plugin-sdk/plugin-entry";
...
export async function makeTempRoot() {
  const root = await fs.mkdtemp(
    path.join(resolvePreferredOPNEXTmpDir(), "opnex-migrate-hermes-"),
  );
  tempRoots.add(root);
  return root;
}
...
export function makeConfigRuntime(
  config: OPNEXConfig,
  onWrite?: (next: OPNEXConfig) => void,
): NonNullable<MigrationProviderContext["runtime"]> {
  const commitConfig = (next: OPNEXConfig) => {
    for (const key of Object.keys(config) as Array<keyof OPNEXConfig>) {
      delete config[key];
    }
  ...
  return {
    config: {
      ...
      mutateConfigFile: async ({
        ...
        afterWrite?: unknown;
        mutate: (draft: OPNEXConfig, context: unknown) => Promise<unknown> | void;
      }) => {
...
export function makeContext(params: {
  ...
  overwrite?: boolean;
  model?: NonNullable<NonNullable<OPNEXConfig["agents"]>["defaults"]>["model"];
  reportDir?: string;
