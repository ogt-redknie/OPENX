...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "opnex-test-web-logout-"));
    previousOAuthDir = process.env.OPNEX_OAUTH_DIR;
    process.env.OPNEX_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.OPNEX_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the OPNEX auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the OPNEX auth root", async () => {
