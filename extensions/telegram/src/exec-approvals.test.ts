...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<OPNEXConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<OPNEXConfig["channels"]>["telegram"]>>,
): OPNEXConfig {
  return {
    channels: {
      telegram: {
