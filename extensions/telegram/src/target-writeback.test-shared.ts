...
vi.mock("opnex/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("opnex/plugin-sdk/config-mutation")>(
    "opnex/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("opnex/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("opnex/plugin-sdk/cron-store-runtime")>(
    "opnex/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeConfigFile).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ expectedConfigPath: "/tmp/opnex.json" }),
      );
