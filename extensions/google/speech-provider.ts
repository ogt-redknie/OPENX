import { transcodeAudioBufferToOpus } from "opnex/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "opnex/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: OPNEXConfig): string | undefined {
...
function isOPNEXGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isOPNEXGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
