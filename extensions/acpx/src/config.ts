...
import { formatPluginConfigIssue } from "opnex/plugin-sdk/extension-shared";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "opnex-plugin-tools";
export const ACPX_OPNEX_TOOLS_MCP_SERVER_NAME = "opnex-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "opnex.plugin.json")) &&
...
function resolveAcpxPluginRootFromOPNEXLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common OPNEX layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromOPNEXLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveOPNEXRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const opnexRoot = resolveOPNEXRoot(pluginRoot);
  const distEntry = path.join(opnexRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(opnexRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
export function resolveOPNEXToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const opnexRoot = resolveOPNEXRoot(pluginRoot);
  const distEntry = path.join(opnexRoot, "dist", "mcp", "opnex-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(opnexRoot, "src", "mcp", "opnex-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.opnexToolsMcpBridge && resolved[ACPX_OPNEX_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_OPNEX_TOOLS_MCP_SERVER_NAME} is reserved when opnexToolsMcpBridge=true`,
    );
  }
  ...
  if (params.opnexToolsMcpBridge) {
    resolved[ACPX_OPNEX_TOOLS_MCP_SERVER_NAME] = resolveOPNEXToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
export function resolveAcpxPluginConfig(params: {
  ...
  const opnexToolsMcpBridge = normalized.opnexToolsMcpBridge === true;
