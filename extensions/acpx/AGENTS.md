...
The bundled ACPX extension is a thin OPNEX wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `opnex/acpx`, not in this extension.
...
Use this flow when OPNEX needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `opnex/acpx` repo first.
2. In OPNEX, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild OPNEX and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for OPNEX ACP validation.
...
If a change feels like shared ACP runtime behavior instead of OPNEX-specific glue, move it to `opnex/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
