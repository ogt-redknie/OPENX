...
const { stopOPNEXChromeMock } = vi.hoisted(() => ({
  stopOPNEXChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopOPNEXChrome: stopOPNEXChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["opnex", "user"]);
    ...
    expect(stopMap.opnex).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopOPNEXChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("opnex browser stop failed: Error: oops");
  });
});
