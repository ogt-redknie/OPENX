...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOPNEXTmpDir: vi.fn(() => "/tmp/opnex"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-opnex-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-opnex-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredOPNEXTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredOPNEXTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOPNEXTmpDir.mockReturnValue("/tmp/opnex");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".opnex-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOPNEXTmpDir.mockReturnValue("/tmp/opnex-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "opnex-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "opnex-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOPNEXTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "opnex-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "opnex-preferred", "downloads")}${path.sep}`),
    );
  });
