...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        opnex: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPNEX_GATEWAY_PORT;
    process.env.OPNEX_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPNEX_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPNEX_GATEWAY_PASSWORD;
