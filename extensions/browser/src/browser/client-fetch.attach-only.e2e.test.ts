...
import { clearRuntimeConfigSnapshot } from "opnex/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("opnex-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".opnex", "opnex.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to OPNEX");
      expect(message).toContain("Restarting the OPNEX gateway will not launch it");
      expect(message).not.toContain("Restart the OPNEX gateway");
