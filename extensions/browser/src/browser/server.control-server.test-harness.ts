...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OPNEX_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "opnex",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/opnex" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    opnex: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchOPNEXChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveOPNEXUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOPNEXChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.OPNEX_GATEWAY_PORT;
  process.env.OPNEX_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.OPNEX_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.OPNEX_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OPNEX_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.OPNEX_GATEWAY_PASSWORD;
  } else {
    process.env.OPNEX_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
