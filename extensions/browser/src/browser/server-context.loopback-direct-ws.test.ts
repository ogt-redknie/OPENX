...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const opnex = ctx.forProfile("opnex");

    const opened = await opnex.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await opnex.focusTab("T1");
    await opnex.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await opnex.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await opnex.focusTab("T2");
    await opnex.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(opnex.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(opnex.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(opnex.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
