import type { OPNEXPluginSecurityAuditContext } from "opnex/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "opnex/plugin-sdk/secret-input";
import { formatCliCommand } from "opnex/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "opnex/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: OPNEXPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("opnex security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.OPNEX_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.OPNEX_GATEWAY_PASSWORD) ||
