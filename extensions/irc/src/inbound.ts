...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OPNEXConfig);
  ...
  if (message.isGroup && commandGate.shouldBlock) {
    const { logInboundDrop } = await import("opnex/plugin-sdk/channel-inbound");
    logInboundDrop({
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OPNEXConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OPNEXConfig);
  ...
  const { dispatchInboundReplyWithBase } =
    await import("opnex/plugin-sdk/inbound-reply-dispatch");
