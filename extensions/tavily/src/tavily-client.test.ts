...
vi.mock("opnex/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: opnex", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "opnex" });
  });

  it("runTavilyExtract sends X-Client-Source: opnex", async () => {
