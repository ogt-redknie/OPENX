...
import { resolveUserPath } from "opnex/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.OPNEX_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.OPNEX_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
