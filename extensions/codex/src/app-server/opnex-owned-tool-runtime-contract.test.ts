...
import {
  ...
  installOPNEXOwnedToolHooks,
  mediaToolResult,
  resetOPNEXOwnedToolHooks,
  textToolResult,
...
describe("OPNEX-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetOPNEXOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installOPNEXOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installOPNEXOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installOPNEXOwnedToolHooks();
