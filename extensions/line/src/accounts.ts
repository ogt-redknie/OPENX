...
import { resolveAccountEntry } from "opnex/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: OPNEXConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: OPNEXConfig): string {
