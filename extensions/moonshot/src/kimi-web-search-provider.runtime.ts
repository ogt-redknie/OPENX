import { createProviderHttpError } from "opnex/plugin-sdk/provider-http";
...
function resolveKimiBaseUrl(kimi?: KimiConfig, opnexConfig?: OPNEXConfig): string {
  ...
  const moonshotBaseUrl = opnexConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: OPNEXConfig; searchConfig?: SearchConfigRecord },
