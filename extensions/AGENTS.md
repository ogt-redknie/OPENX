...
- Extension production code should import from `opnex/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `opnex.plugin.json` and the package
  `opnex` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `opnex/plugin-sdk/*`.
