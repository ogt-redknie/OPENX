...
- Start from `opnex wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `opnex wiki obsidian status` before shelling out, then prefer the dedicated helpers like `opnex wiki obsidian search`, `opnex wiki obsidian open`, `opnex wiki obsidian command`, and `opnex wiki obsidian daily`.
