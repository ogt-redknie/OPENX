...
const AGENT_DIGEST_PATH = ".opnex-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".opnex-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- opnex:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- opnex:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- opnex:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- opnex:wiki:${group.dir}:index:end -->`,
