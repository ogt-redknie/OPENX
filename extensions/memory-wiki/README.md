...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **OPNEX**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.opnex-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
opnex wiki doctor
opnex wiki init
opnex wiki ingest ./notes/alpha.md
opnex wiki compile
opnex wiki lint
opnex wiki search "alpha"
opnex wiki get entity.alpha --from 1 --lines 80

opnex wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

opnex wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

opnex wiki bridge import
opnex wiki unsafe-local import

opnex wiki obsidian status
opnex wiki obsidian search "alpha"
opnex wiki obsidian open syntheses/alpha-summary.md
opnex wiki obsidian command workspace:quick-switcher
opnex wiki obsidian daily
```
...
- The compiled agent digests in `.opnex-wiki/cache/agent-digest.json` and `.opnex-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
