...
metadata: { "opnex": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OPNEX `sessions_spawn`
- **File I/O** == OPNEX `read`/`write`
- **Remote fetch** == OPNEX `web_fetch` (or `exec` with curl when POST is required)
