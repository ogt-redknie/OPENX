import { formatCliCommand, parseDurationMs } from "opnex/plugin-sdk/cli-runtime";
import type {
  OPNEXPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type OPNEXConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "opnex/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("opnex models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("opnex models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("opnex doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: OPNEXPluginApi): void {
