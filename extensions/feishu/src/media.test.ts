...
vi.mock("opnex/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opnex/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
function expectPathIsolatedToTmpRoot(pathValue: string, key: string): void {
  ...
  const tmpRoot = path.resolve(resolvePreferredOPNEXTmpDir());
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  it("passes mediaLocalRoots as localRoots to loadWebMedia for local paths (#27884)", async () => {
    ...
    const roots = ["/allowed/workspace", "/tmp/opnex"];
    await sendMediaFeishu({
      cfg: emptyConfig,
      to: "user:ou_target",
