...
function ensureZalouserPluginEnabled(cfg: OPNEXConfig): OPNEXConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("opnex directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
      ...
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("opnex directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
