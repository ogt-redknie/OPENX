...
Designed to be called from workflow engines (for example, OPNEX via
`opnex.invoke --each`) without adding new OPNEX code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  OPNEX) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on OPNEX internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OPNEX extension (like `OPNEX`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opnex/extensions` as a standalone plugin directory.
