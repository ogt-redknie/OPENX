...
Read-only diff viewer plugin for **OPNEX** agents.
...
Set plugin-wide defaults in `~/.opnex/opnex.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/opnex",
        },
      },
    },
  },
...
Before:
OPNEX supports plugins.

After:
OPNEX supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/opnex-diffs`).
