...
import type { DiscordGuildEntry, OPNEXConfig } from "opnex/plugin-sdk/config-types";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "opnex/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "opnex/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: OPNEXConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: OPNEXConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: OPNEXConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: OPNEXConfig; accountId: string }) =>
    ...
    disable: (cfg: OPNEXConfig) => setSetupChannelEnabled(cfg, channel, false),
