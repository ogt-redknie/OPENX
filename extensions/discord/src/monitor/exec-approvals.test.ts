...
vi.mock("opnex/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("opnex/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<OPNEXConfig["channels"]>["discord"]>["execApprovals"],
): OPNEXConfig {
  return {
    channels: {
      discord: {
