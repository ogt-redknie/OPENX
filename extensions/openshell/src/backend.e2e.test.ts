...
import { createSandboxTestContext } from "opnex/plugin-sdk/test-fixtures";
import {
...
const OPNEX_OPENSHELL_E2E = process.env.OPNEX_E2E_OPENSHELL === "1";
const OPNEX_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const OPNEX_OPENSHELL_COMMAND =
  process.env.OPNEX_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "opnex-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPNEX_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: OPNEX_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(OPNEX_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(OPNEX_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-openshell-e2e-"));
      ...
      const gatewayName = `opnex-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `opnex-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/opnex-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: OPNEX_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("opnex-openshell-e2e");
