 ...
 * `opnex/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.opnex/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".opnex", "qqbot", ...subPaths);
}

/** Return a path under `~/.opnex/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.opnex/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under OPNEX's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".opnex", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.opnex/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.opnex/media`, OPNEX's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `opnex/src/media/local-roots.ts`). Using it as a
 ...
 */
export function getOPNEXMediaDir(): string {
  return path.join(getHomeDir(), ".opnex", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".opnex", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and OPNEX's shared `~/.opnex/media`
  ...
  const allowedRoots = [getOPNEXMediaDir(), getQQBotMediaPath()];
