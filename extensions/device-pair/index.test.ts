...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredOPNEXTmpDir: vi.fn(() => path.join(os.tmpdir(), "opnex-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredOPNEXTmpDir: pluginApiMocks.resolvePreferredOPNEXTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as OPNEXPluginApi["runtime"],
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredOPNEXTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredOPNEXTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the OPNEX iOS app:");
    ...
    expect(text).not.toContain("![OPNEX pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the OPNEX iOS app:");
