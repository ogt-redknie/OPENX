...
import { loginOpenAICodexOAuth } from "opnex/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "opnex/plugin-sdk/provider-usage";
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.opnex.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `opnex models auth login --provider openai-codex` or `opnex configure`.";
}
