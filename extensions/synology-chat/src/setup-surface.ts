...
function getChannelConfig(cfg: OPNEXConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: OPNEXConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: OPNEXConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: OPNEXConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL OPNEX uses to send replies back to Chat.",
      ],
