...
function readLegacyGrokFallbackAuth(cfg?: OPNEXConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: OPNEXConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function readLegacyGrokApiKey(cfg?: OPNEXConfig): string | undefined {
...
export function readPluginXaiWebSearchApiKey(cfg?: OPNEXConfig): string | undefined {
  const resolved = readPluginXaiWebSearchApiKeyResult(cfg);
  return resolved.status === "available" ? resolved.value : undefined;
}

export function resolveFallbackXaiAuth(cfg?: OPNEXConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: OPNEXConfig): string | undefined {
