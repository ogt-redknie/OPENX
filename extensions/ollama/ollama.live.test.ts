...
const LIVE = process.env.OPNEX_LIVE_TEST === "1" && process.env.OPNEX_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.OPNEX_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.OPNEX_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.OPNEX_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.OPNEX_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.OPNEX_LIVE_OLLAMA_WEB_SEARCH !== "0";
...
async function withTempOPNEXState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "opnex.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runOPNEXCli(args: string[], env: NodeJS.ProcessEnv) {
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "opnex-ollama-cli-output-"));
  ...
  try {
    const result = spawnSync(process.execPath, ["opnex.mjs", ...args], {
      cwd: process.cwd(),
      env,
      timeout: 90_000,
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  return {
    ...
    OPNEX_LIVE_TEST: "1",
    OPNEX_LIVE_OLLAMA: "1",
    OPNEX_LIVE_OLLAMA_WEB_SEARCH: "0",
    OPNEX_STATE_DIR: path.join(root, "state"),
    OPNEX_CONFIG_PATH: path.join(root, "opnex.json"),
    OPNEX_NO_RESPAWN: "1",
    OPNEX_TEST_FAST: "1",
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without PI model discovery", async () => {
    await withTempOPNEXState(async ({ root }) => {
      const result = await runOPNEXCli(
        [
          "infer",
          "model",
          "run",
