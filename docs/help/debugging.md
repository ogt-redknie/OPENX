...
This is handy when you need to toggle obscure settings without editing `opnex.json`.
...
OPNEX keeps `src/cli/debug-timing.ts` as a small helper for local
...
`opnex models list`, a temporary patch in
...
```bash
OPNEX_DEBUG_TIMING=1 pnpm opnex models list --all --provider moonshot
```
...
```bash
OPNEX_DEBUG_TIMING=json pnpm opnex models list --all --provider moonshot \
...
extension `package.json` and `opnex.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.opnex-dev` and
...
```bash
pnpm gateway:dev
OPNEX_PROFILE=dev opnex tui
```

If you don’t have a global install yet, run the CLI via `pnpm opnex ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `OPNEX_PROFILE=dev`
   - `OPNEX_STATE_DIR=~/.opnex-dev`
   - `OPNEX_CONFIG_PATH=~/.opnex-dev/opnex.json`
   - `OPNEX_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`OPNEX_SKIP_CHANNELS=1`).
...
```bash
OPNEX_PROFILE=dev opnex gateway --dev --reset
```
...
## Raw stream logging (OPNEX)

OPNEX can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.opnex/logs/raw-stream.jsonl
```
...
```bash
OPNEX_RAW_STREAM=1
OPNEX_RAW_STREAM_PATH=~/.opnex/logs/raw-stream.jsonl
```

Default file:

`~/.opnex/logs/raw-stream.jsonl`
