---
summary: "Get OPNEX installed and run your first chat in minutes."
read_when:
...
Install OPNEX, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.opnex/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.opnex/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run OPNEX as a service account or want custom paths:

- `OPNEX_HOME` — home directory for internal path resolution
- `OPNEX_STATE_DIR` — override the state directory
- `OPNEX_CONFIG_PATH` — override the config file path
