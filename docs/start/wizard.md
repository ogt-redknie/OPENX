...
CLI onboarding is the **recommended** way to set up OPNEX on macOS,
...
`opnex dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`opnex configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.opnex/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `opnex doctor` first.
</Note>
...
- CLI command reference: [`opnex onboard`](/cli/onboard)
