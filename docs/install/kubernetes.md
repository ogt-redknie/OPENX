---
summary: "Deploy OPNEX Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run OPNEX on a Kubernetes cluster
  - You want to test OPNEX in a Kubernetes environment
title: "Kubernetes"
---

# OPNEX on Kubernetes

A minimal starting point for running OPNEX on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

OPNEX is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/opnex 18789:18789 -n opnex
...
```bash
kubectl get secret opnex-secrets -n opnex -o jsonpath='{.data.OPNEX_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: opnex (configurable via OPNEX_NAMESPACE)
├── Deployment/opnex        # Single pod, init container + gateway
├── Service/opnex           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/opnex-config  # opnex.json + AGENTS.md
└── Secret/opnex-secrets    # Gateway token + API keys
```
...
Edit `opnex.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret opnex-secrets -n opnex \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/opnex -n opnex
```

### Custom namespace

```bash
OPNEX_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/opnex/opnex:latest # or pin to a specific version from https://github.com/opnex/opnex/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # opnex.json + AGENTS.md
