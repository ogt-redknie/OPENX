---
summary: "Host OPNEX on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up OPNEX on Oracle Cloud
  - Looking for free VPS hosting for OPNEX
  - Want 24/7 OPNEX on a small server
title: "Oracle Cloud"
---

Run a persistent OPNEX Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@opnex`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=opnex --reset` to re-authenticate.

**Gateway will not start** -- Run `opnex doctor --non-interactive` and check logs with `journalctl --user -u opnex-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep OPNEX up to date
