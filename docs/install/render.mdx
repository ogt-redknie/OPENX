---
summary: "Deploy OPNEX on Render with Infrastructure-as-Code"
read_when:
  - Deploying OPNEX to Render
...
Deploy OPNEX on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/opnex/opnex)
...
repository configures everything needed to run OPNEX:
...
services:
  - type: web
    ...
    envVars:
      - key: OPNEX_GATEWAY_PORT
        value: "8080"
      - key: OPNEX_STATE_DIR
        value: /data/.opnex
      - key: OPNEX_WORKSPACE_DIR
        value: /data/workspace
      - key: OPNEX_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means OPNEX state
...
`OPNEX_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original OPNEX repository, Render will not auto-deploy your OPNEX. To update it, run a manual Blueprint sync from the dashboard.
...
For OPNEX, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with OPNEX state plus any configured
...
- Missing `OPNEX_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `OPNEX_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `opnex backup create` in the Render shell.
