---
summary: "Install OPNEX — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs OPNEX, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://opnex.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want OPNEX and Node kept under a local prefix such as
`~/.opnex`, without depending on a system-wide Node install:

```bash
curl -fsSL https://opnex.ai/install-cli.sh | bash
```
...
`opnex update --channel dev` and `opnex update --channel stable`. See
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g opnex@latest
```
...
Or skip the link and use `pnpm opnex ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:opnex/opnex#main
```
...
```bash
opnex --version      # confirm the CLI is available
opnex doctor         # check for config issues
opnex gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `opnex onboard --install-daemon` or `opnex gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy OPNEX on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove OPNEX completely.
  </Card>
</CardGroup>

## Troubleshooting: `opnex` not found

If the install succeeded but `opnex` is not found in your terminal:
