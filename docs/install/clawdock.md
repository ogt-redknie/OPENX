---
summary: "ClawDock shell helpers for Docker-based OPNEX installs"
read_when:
  - You run OPNEX with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based OPNEX installs.
...
| `clawdock-cli <command>`  | Run OPNEX CLI commands in Docker           |
...
- `~/.opnex/.env` for env-backed provider keys and bot tokens
- `~/.opnex/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.opnex/opnex.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `opnex.json` quickly. It redacts `.env` values in its printed output.
