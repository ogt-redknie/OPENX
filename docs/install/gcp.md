---
summary: "Run OPNEX Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want OPNEX running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# OPNEX on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent OPNEX Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "OPNEX 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the OPNEX Gateway in Docker
- Persist `~/.opnex` + `~/.opnex/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.opnex` state includes `opnex.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-opnex-project --name="OPNEX Gateway"
    gcloud config set project my-opnex-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create opnex-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh opnex-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the OPNEX repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    OPNEX_IMAGE=opnex:latest
    OPNEX_GATEWAY_TOKEN=
    OPNEX_GATEWAY_BIND=lan
    OPNEX_GATEWAY_PORT=18789

    OPNEX_CONFIG_DIR=/home/$USER/.opnex
    OPNEX_WORKSPACE_DIR=/home/$USER/.opnex/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.opnex
    ```

    Leave `OPNEX_GATEWAY_TOKEN` blank unless you explicitly want to
    manage it through `.env`; OPNEX writes a random gateway token to
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `OPNEX_GATEWAY_TOKEN`.
    ...
    services:
      opnex-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - OPNEX_GATEWAY_BIND=${OPNEX_GATEWAY_BIND}
          - OPNEX_GATEWAY_PORT=${OPNEX_GATEWAY_PORT}
          - OPNEX_GATEWAY_TOKEN=${OPNEX_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${OPNEX_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`OPNEX_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm opnex-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh opnex-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop opnex-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type opnex-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start opnex-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create opnex-deploy \
     --display-name="OPNEX Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-opnex-project \
     --member="serviceAccount:opnex-deploy@my-opnex-project.iam.gserviceaccount.com" \
