---
summary: "Run OPNEX Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want OPNEX running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running OPNEX in Docker on Hetzner or a similar provider
title: "Hetzner"
---

# OPNEX on Hetzner (Docker, Production VPS Guide)

## Goal

Run a persistent OPNEX Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want “OPNEX 24/7 for ~$5”, this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.opnex/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.opnex
    ```
  ...
  <Step title="Configure environment variables">
    ...
    OPNEX_CONFIG_DIR=/root/.opnex
    OPNEX_WORKSPACE_DIR=/root/.opnex/workspace
...
- Infrastructure: [opnex-terraform-hetzner](https://github.com/andreesg/opnex-terraform-hetzner)
- Docker config: [opnex-docker-config](https://github.com/andreesg/opnex-docker-config)
