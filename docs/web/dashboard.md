...
- Re-open anytime: `opnex dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `OPNEX_GATEWAY_TOKEN`); `opnex dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `opnex dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `opnex dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `OPNEX_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `opnex status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `opnex config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `OPNEX_GATEWAY_TOKEN` in this shell, then rerun `opnex dashboard`
  - No shared secret configured: `opnex doctor --generate-gateway-token`
