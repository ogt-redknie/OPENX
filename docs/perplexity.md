...
OPNEX supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, OPNEX also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `opnex configure --section web`. It stores the key in
`~/.opnex/opnex.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.opnex/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
