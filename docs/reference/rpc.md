...
OPNEX integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OPNEX owns lifecycle when `channels.signal.autoStart=true`.
...
- OPNEX spawns `imsg rpc` as a child process (legacy iMessage integration).
