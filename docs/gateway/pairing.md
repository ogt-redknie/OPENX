...
opnex nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), OPNEX treats
...
Pairing state is stored under the Gateway state directory (default `~/.opnex`):

- `~/.opnex/nodes/paired.json`
- `~/.opnex/nodes/pending.json`

If you override `OPNEX_STATE_DIR`, the `nodes/` folder moves with it.
