...
- Default rolling log file is under `/tmp/opnex/` (one file per day): `opnex-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.opnex/opnex.json`:
  - `logging.file`
  - `logging.level`
...
OPNEX can mask sensitive tokens before log or transcript output leaves the
...
`opnex gateway` supports a per-gateway style switch:
