---
summary: "How OPNEX sandboxing works: modes, scopes, workspace access, and images"
title: "Sandboxing"
...
OPNEX can run **tools inside sandbox backends** to reduce blast radius. This is **optional** and controlled by configuration (`agents.defaults.sandbox` or `agents.list[].sandbox`). If sandboxing is off, tools run on the host. The Gateway stays on the host; tool execution runs in an isolated sandbox when enabled.
...
<AccordionGroup>
  <Accordion title="Sandboxed browser details">
    ...
    - By default, sandbox browser containers use a dedicated Docker network (`opnex-sandbox-browser`) instead of the global `bridge` network. Configure with `agents.defaults.sandbox.browser.network`.
    ...
    - noVNC observer access is password-protected by default; OPNEX emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Sandboxing is off by default. If you enable sandboxing and do not choose a backend, OPNEX uses the Docker backend. It executes tools and sandbox browsers locally via the Docker daemon socket (`/var/run/docker.sock`). Sandbox container isolation is determined by Docker namespaces.
...
If you deploy the OPNEX Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config requires host paths**: The `opnex.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.opnex/workspaces`), not the internal Gateway container path. When OPNEX asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS bridge parity (identical volume map)**: The OPNEX Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.opnex:/home/user/.opnex`).

If you map paths internally without absolute host parity, OPNEX natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.
</Warning>

### SSH backend

Use `backend: "ssh"` when you want OPNEX to sandbox `exec`, file tools, and media reads on an arbitrary SSH-accessible machine.
...
<AccordionGroup>
  <Accordion title="How it works">
    - OPNEX creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
    - On first use after create or recreate, OPNEX seeds that remote workspace from the local workspace once.
    ...
    - OPNEX does not sync remote changes back to the local workspace automatically.
  ...
  <Accordion title="Authentication material">
    ...
    - `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. OPNEX resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
  ...
  <Accordion title="Remote-canonical consequences">
    ...
    - Host-local edits made outside OPNEX after the seed step are not visible remotely until you recreate the sandbox.
    - `opnex sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
...
Use `backend: "openshell"` when you want OPNEX to sandbox tools in an OpenShell-managed remote environment. For the full setup guide, configuration reference, and workspace mode comparison, see the dedicated [OpenShell page](/gateway/openshell).
...
- `mirror` (default): local workspace stays canonical. OPNEX syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. OPNEX seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

<AccordionGroup>
  <Accordion title="Remote transport details">
    - OPNEX asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
<Tabs>
  <Tab title="mirror (local canonical)">
    ...
    - you edit files locally outside OPNEX and want those changes to show up in the sandbox automatically
  ...
  <Tab title="remote (OpenShell canonical)">
    ...
    - When the sandbox is first created, OPNEX seeds the remote workspace from the local workspace once.
    - After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
    - OPNEX does **not** sync remote changes back into the local workspace after exec.
    ...
    - If you edit files on the host outside OPNEX after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `opnex sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `opnex sandbox recreate` deletes the current runtime and lets OPNEX recreate it on next use
...
<Tabs>
  <Tab title="none (default)">
    Tools see a sandbox workspace under `~/.opnex/sandboxes`.
  </Tab>
  <Tab title="ro">
...
<Note>
**Skills note:** the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, OPNEX mirrors eligible skills into the sandbox workspace (`.../skills`) so they can be read. With `"rw"`, workspace skills are readable from `/workspace/skills`.
</Note>
...
- OPNEX blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- OPNEX also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. OPNEX normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `opnex-sandbox:bookworm-slim`
...
<Steps>
  <Step title="Build the default image">
    ...
    OPNEX does not silently substitute plain `debian:bookworm-slim` when `opnex-sandbox:bookworm-slim` is missing. Sandbox runs that target the default image fail fast with a build instruction until you run `scripts/sandbox-setup.sh`, because the bundled image carries `python3` for sandbox write/edit helpers.
  ...
  <Step title="Optional: build the common image">
    ...
    Then set `agents.defaults.sandbox.docker.image` to `opnex-sandbox-common:bookworm-slim`.
...
<AccordionGroup>
  <Accordion title="Sandbox browser Chromium defaults">
    ...
    - The three graphics hardening flags (`--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu`) are optional and are useful when containers lack GPU support. Set `OPNEX_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if your workload requires WebGL or other 3D/browser features.
    - `--disable-extensions` is enabled by default and can be disabled with `OPNEX_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
    - `--renderer-process-limit=2` is controlled by `OPNEX_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
For Docker gateway deployments, `scripts/docker/setup.sh` can bootstrap sandbox config. Set `OPNEX_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can override socket location with `OPNEX_DOCKER_SOCKET`. Full setup and env reference: [Docker](/install/docker#agent-sandbox).
...
- Use `opnex sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
