...
- `opnex gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `opnex doctor` reports no blocking config/service issues.
- `opnex channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.

## Split brain installs and newer config guard

Use this when a gateway service unexpectedly stops after an update, or logs show that one `opnex` binary is older than the version that last wrote `opnex.json`.

OPNEX stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer OPNEX, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which opnex
opnex --version
opnex gateway status --deep
opnex config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `opnex` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `opnex` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `OPNEX_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
opnex logs --follow
opnex models status
opnex config get agents.defaults.models
```
...
- OPNEX model runs fail only on normal agent turns
...
opnex infer model run --model <provider/model> --prompt "hi" --json
opnex logs --follow
```

Look for:

- direct tiny calls succeed, but OPNEX runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. OPNEX retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but OPNEX agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → OPNEX transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    2. Set `compat.supportsTools: false` for models/backends that cannot handle OPNEX's tool schema surface reliably.
    ...
    4. If tiny direct requests keep passing while OPNEX agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
opnex status
opnex channels status --probe
opnex pairing list --channel <channel> [--account <id>]
opnex config get channels
opnex logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `opnex config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `opnex devices list` then `opnex devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `opnex devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `opnex devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
opnex logs --follow
opnex doctor
opnex gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `opnex onboard --mode local` / `opnex setup` to restamp the expected local-mode config. If you are running OPNEX via Podman, the default config path is `~/.opnex/opnex.json`.
    ...
    - `System-level OPNEX gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `OPNEX_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `opnex doctor --fix` or `opnex gateway install --force`, then restart the gateway service.
...
Use this when the Gateway starts, but logs say it restored `opnex.json`.
...
- A timestamped `opnex.json.clobbered.*` file beside the active config
...
<AccordionGroup>
  <Accordion title="What happened">
    ...
    - OPNEX preserved the rejected payload as `.clobbered.*`.
    ...
    - If all validation issues were under `plugins.entries.<id>...`, OPNEX would not restore the whole file. Plugin-local failures stay loud while unrelated user settings remain in the active config.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an OPNEX-owned config write failed schema or clobber checks before commit.
  ...
  <Accordion title="Fix options">
    ...
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `opnex config set` or `config.patch`.
    3. Run `opnex config validate` before restarting.
...
Use this when `opnex gateway probe` reaches something, but still prints a warning block.
...
opnex cron status
opnex cron list
opnex cron runs --id <jobId> --limit 20
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so OPNEX skips the model call.
...
```bash
opnex nodes status
opnex nodes describe --node <idOrNameOrIp>
opnex approvals get --node <idOrNameOrIp>
opnex logs --follow
opnex status
```
...
opnex browser start --browser-profile opnex
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `opnex browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (OPNEX-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    opnex gateway status
    opnex config get gateway.mode
    opnex config get gateway.remote.url
    opnex config get gateway.auth.mode
    ```
