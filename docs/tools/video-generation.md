...
OPNEX agents can generate videos from text prompts, reference images, or
...
OPNEX treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    opnex config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
  <Step title="Ask the agent">
    > Generate a 5-second cinematic video of a friendly OPNEX surfing at sunset.
...
1. OPNEX submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, OPNEX wakes the same session with an internal completion event.
...
generation. Use `opnex tasks list` or `opnex tasks show <taskId>` to
...
Generated video files are saved under OPNEX-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, OPNEX
...
```bash
opnex tasks list
opnex tasks show <taskId>
opnex tasks cancel <taskId>
```
...
<Note>
Not all providers support all parameters. OPNEX normalizes duration to
...
OPNEX resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@opnex/byteplus-modelark`](https://www.npmjs.com/package/@opnex/byteplus-modelark)
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. OPNEX submits the
...
```bash
OPNEX_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- One-second OPNEX prompt.
- Per-provider operation cap from
  `OPNEX_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `OPNEX_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your OPNEX config:
...
```bash
opnex config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
