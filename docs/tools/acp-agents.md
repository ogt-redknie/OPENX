---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, OPNEX ACP, OpenCode) through the ACP backend"
read_when:
...
let OPNEX run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, OPNEX ACP, OpenCode, Gemini CLI, and other
...
directly to existing OPNEX channel conversations, use
[`opnex mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ OPNEX | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an OPNEX Gateway session _as_ an ACP server for an editor or client                   | [`opnex acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to OPNEX over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No OPNEX tools, no ACP controls, no harness runtime                                                                                                                               |

## Does this work out of the box?

Usually yes. Fresh installs ship the bundled `acpx` runtime plugin enabled
by default with a plugin-local pinned `acpx` binary that OPNEX probes
and self-repairs on startup. Run `/acp doctor` for a readiness check.

OPNEX only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. OPNEX owns routing,
...
OPNEX plugin tools and built-in OPNEX tools are **not** exposed to
...
| `opnex` | OPNEX Gateway bridge through `opnex acp` | Lets an ACP-aware harness talk back to an OPNEX Gateway session.                 |
| `pi`       | Pi/embedded OPNEX runtime                   | Used for OPNEX-native harness experiments.                                       |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but OPNEX
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the OPNEX session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from OPNEX's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    OPNEX dynamic tools still execute through OPNEX, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, OPNEX injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through OPNEX approvals. Codex `Stop` hooks are relayed to
    OPNEX `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    OPNEX picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    OPNEX config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an OPNEX agent
    is configured with `runtime.type="acp"`, OPNEX uses
...
plugin is enabled. Use **sub-agents** when you want OPNEX-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | OPNEX native sub-agent runtime  |
...
1. OPNEX ACP session control plane.
...
- **ACP session** — the durable Codex/Claude/Gemini runtime state OPNEX routes to.
...
spawned ACP session — no child thread, same chat surface. OPNEX keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; OPNEX returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnAcpSessions` is only required when OPNEX needs to create a child thread for `--thread auto|here` — not for `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, OPNEX inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations — `/acp ...` commands are handled by OPNEX even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - OPNEX binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, OPNEX returns a clear
...
- OPNEX ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, OPNEX inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, OPNEX uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, OPNEX may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize OPNEX Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise OPNEX/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What OPNEX sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. OPNEX agents get OPNEX's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_OPNEX_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in OPNEX where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. OPNEX converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see OPNEX-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, OPNEX uses an agent-to-agent (A2A) follow-up path
    ...
    OPNEX skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an OPNEX channel session key; OPNEX still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new OPNEX session you are creating, so `mode: "session"` still requires `thread: true`.
...
- OPNEX's sandbox policy does **not** wrap ACP harness execution.
- OPNEX still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced OPNEX-native work.
...
If no target resolves, OPNEX returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, OPNEX normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`. |
| `/acp set thinking <level>`  | runtime config key `thinking`        | For Codex ACP, OPNEX sends the corresponding `reasoning_effort` where the adapter supports one.                                                                             |
...
aliases), the plugin-tools and OPNEX-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | OPNEX is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| Harness sees `<<<BEGIN_OPNEX_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update OPNEX and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
