...
- allowlist entries live in host-local `~/.opnex/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `opnex approvals allowlist ...`).
- `opnex security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `opnex doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, OPNEX still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, OPNEX sends a followup `agent` turn to the same session.
...
- Matrix native prompts include `com.opnex.approval` custom event content on the first prompt
  event so OPNEX-aware Matrix clients can read structured approval state while stock clients
...
topics, OPNEX preserves the topic for the approval prompt and the post-approval follow-up.
