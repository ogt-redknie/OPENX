...
For session-backed agent runs, OPNEX starts music generation as a
...
<Note>
Not all providers support all parameters. OPNEX still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, OPNEX
...
- **Status lookup:** `opnex tasks list` or `opnex tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** OPNEX injects an internal completion event back
...
```bash
OPNEX_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
