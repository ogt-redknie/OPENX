...
OPNEX supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), OPNEX reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.opnex/.env`.

If you omit `baseUrl`, OPNEX defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, OPNEX defaults to `kimi-k2.6`.
