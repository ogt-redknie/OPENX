...
`opnex agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    opnex agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    opnex agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    opnex agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    opnex agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    opnex agent --agent ops --message "Generate report" \
...
opnex agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
