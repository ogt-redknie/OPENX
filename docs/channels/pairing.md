...
read_when:
  ...
  - Reviewing OPNEX security posture
title: "Pairing"
---

“Pairing” is OPNEX’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `opnex-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.opnex/credentials/`:
...
3. On your phone, open the OPNEX iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, OPNEX keeps the existing approval as-is and creates a fresh pending upgrade request. Use `opnex devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `opnex nodes pending|approve|reject|remove|rename`) is a
