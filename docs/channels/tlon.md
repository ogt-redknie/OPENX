...
Tlon is a decentralized messenger built on Urbit. OPNEX connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current OPNEX releases, so normal packaged
...
```bash
opnex plugins install @opnex/tlon
```

Local checkout (when running from a git repo):

```bash
opnex plugins install ./path/to/local/tlon-plugin
```
...
By default, OPNEX blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `opnex message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OPNEX replies in-thread.
