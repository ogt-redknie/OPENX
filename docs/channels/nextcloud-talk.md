...
Nextcloud Talk ships as a bundled plugin in current OPNEX releases, so
...
```bash
opnex plugins install @opnex/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opnex plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OPNEX" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
4. Configure OPNEX:
   ...
   ```bash
   opnex channels add --channel nextcloud-talk \
...
- Approve via:
  - `opnex pairing list nextcloud-talk`
  - `opnex pairing approve nextcloud-talk <CODE>`
