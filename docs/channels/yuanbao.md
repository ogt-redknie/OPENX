...
Tencent Yuanbao is Tencent's AI assistant platform. The OPNEX channel plugin
connects Yuanbao bots to OPNEX over WebSocket so they can interact with users
...
> **Requires OPNEX 2026.4.10 or above.** Run `opnex --version` to check. Upgrade with `opnex update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  opnex channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
opnex channels login --channel yuanbao
```
...
```bash
opnex pairing list yuanbao
opnex pairing approve yuanbao <CODE>
```
