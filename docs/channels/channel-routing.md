...
OPNEX routes replies **back to the channel where a message came from**. The
...
OPNEX infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, OPNEX still records inbound session metadata, but it
...
when a guarded path must not create a new OPNEX session. In that mode,
OPNEX may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when OPNEX would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.opnex/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.opnex`):

- `~/.opnex/agents/<agentId>/sessions/sessions.json`
