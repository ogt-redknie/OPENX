---
summary: "Synology Chat webhook setup and OPNEX config"
read_when:
  - Setting up Synology Chat with OPNEX
...
Synology Chat ships as a bundled plugin in current OPNEX releases, so normal
...
```bash
opnex plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged OPNEX releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `opnex onboard` now shows Synology Chat in the same channel setup list as `opnex channels add`.
   - Non-interactive setup: `opnex channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your OPNEX gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in OPNEX.
   - Guided: `opnex onboard`
   - Direct: `opnex channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- OPNEX accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `opnex pairing list synology-chat`
  - `opnex pairing approve synology-chat <CODE>`
...
```bash
opnex message send --channel synology-chat --target 123456 --text "Hello from OPNEX"
opnex message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before OPNEX forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. OPNEX now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached OPNEX
- `Rate limit exceeded`:
