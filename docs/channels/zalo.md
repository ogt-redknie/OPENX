...
Zalo ships as a bundled plugin in current OPNEX releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `opnex plugins install @opnex/zalo`
- Or from a source checkout: `opnex plugins install ./path/to/local/zalo-plugin`
...
This page reflects current OPNEX behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `opnex pairing list zalo`
  - `opnex pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in OPNEX.
...
- Example: `opnex message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `opnex channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `opnex logs --follow`
