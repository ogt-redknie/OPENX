...
read_when:
  - Setting up Zalo Personal for OPNEX
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside OPNEX.
...
Zalo Personal ships as a bundled plugin in current OPNEX releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `opnex plugins install @opnex/zalouser`
- Or from a source checkout: `opnex plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `opnex channels login --channel zalouser`
...
```bash
opnex directory self --channel zalouser
opnex directory peers list --channel zalouser --query "name"
opnex directory groups list --channel zalouser --query "work"
```
...
- `opnex pairing list zalouser`
- `opnex pairing approve zalouser <code>`
...
- On startup, OPNEX resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, OPNEX stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in OPNEX state. Example:
...
- OPNEX sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, OPNEX sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `opnex channels logout --channel zalouser && opnex channels login --channel zalouser`
...
- The channel now runs fully in OPNEX without external CLI binaries.
