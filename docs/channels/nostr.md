...
read_when:
  - You want OPNEX to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OPNEX to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current OPNEX releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`opnex onboard`) and `opnex channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
opnex plugins install @opnex/nostr
```

Use a local checkout (dev workflows):

```bash
opnex plugins install --link <path-to-local-nostr-plugin>
```
...
opnex channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "opnex@example.com",
        lud16: "opnex@example.com",
      },
    },
  },
}
