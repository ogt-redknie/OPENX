...
If the active memory plugin exposes shared recall artifacts, OPNEX can search
...
currently exposing public bridge inputs yet. Run `opnex wiki doctor` first,
...
`opnex wiki status`, `opnex wiki doctor`, and `opnex wiki bridge
...
opnex wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
