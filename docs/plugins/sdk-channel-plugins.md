---
summary: "Step-by-step guide to building a messaging channel plugin for OPNEX"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect OPNEX to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects OPNEX to a
...
Channel plugins do not need their own send/edit/react tools. OPNEX keeps one
...
Use `opnex/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `opnex/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `opnex/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `opnex/plugin-sdk/approval-auth-runtime`
- `opnex/plugin-sdk/approval-client-runtime`
- `opnex/plugin-sdk/approval-delivery-runtime`
- `opnex/plugin-sdk/approval-gateway-runtime`
- `opnex/plugin-sdk/approval-handler-adapter-runtime`
- `opnex/plugin-sdk/approval-handler-runtime`
- `opnex/plugin-sdk/approval-native-runtime`
- `opnex/plugin-sdk/approval-reply-runtime`
- `opnex/plugin-sdk/channel-runtime-context`

Likewise, prefer `opnex/plugin-sdk/setup-runtime`,
`opnex/plugin-sdk/setup-adapter-runtime`,
`opnex/plugin-sdk/reply-runtime`,
`opnex/plugin-sdk/reply-dispatch-runtime`,
`opnex/plugin-sdk/reply-reference`, and
`opnex/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `opnex/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `opnex/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `opnex/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `opnex.setupEntry` in
...
- use the broader `opnex/plugin-sdk/setup` seam only when you also need the
...
- `opnex/plugin-sdk/account-core`,
  `opnex/plugin-sdk/account-id`,
  `opnex/plugin-sdk/account-resolution`, and
  `opnex/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `opnex/plugin-sdk/inbound-envelope` and
  `opnex/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `opnex/plugin-sdk/messaging-targets` for target parsing/matching
- `opnex/plugin-sdk/outbound-media` and
  `opnex/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `opnex/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `opnex/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `opnex/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `opnex/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `opnex/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `opnex/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`opnex/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`opnex/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#opnex-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/opnex-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "opnex": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect OPNEX to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { OPNEXConfig } from "opnex/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so OPNEX
  ...
  <Step title="Add a setup entry">
    ...
    OPNEX loads this instead of the full entry when the channel is disabled
    ...
    `opnex/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    OPNEX. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to OPNEX.
...
├── package.json              # opnex.channel metadata
├── opnex.plugin.json      # Manifest with config schema
