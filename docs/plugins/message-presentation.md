...
Message presentation is OPNEX's shared contract for rich outbound chat UI.
...
```bash
opnex message send --channel slack \
  --target channel:C123 \
...
```bash
opnex message send --channel telegram \
...
Use helpers from `opnex/plugin-sdk/interactive-runtime` when bridging older
code:
