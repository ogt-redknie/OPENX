...
read_when:
  - You want an OPNEX agent to join a Google Meet call
  - You want an OPNEX agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for OPNEX — the plugin is explicit by design:
...
- Realtime voice can call back into the full OPNEX agent when deeper
...
```bash
opnex googlemeet setup --transport chrome-node --mode transcribe
```
...
join. Use `opnex googlemeet setup --json` for scripts or machine-readable
...
```bash
opnex googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
opnex googlemeet create --transport chrome-node --mode realtime
```

Create only the URL without joining:

```bash
opnex googlemeet create --no-join
```
...
- Browser fallback: used when OAuth credentials are absent. OPNEX uses the
  ...
  the OPNEX Chrome profile on the node to already be signed in to Google.
...
OPNEX's microphone/camera permission grant and avoid the Meet **Use
...
Local Chrome joins through the signed-in OPNEX browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by OPNEX. For
...
You do **not** need a full OPNEX Gateway or model API key inside a macOS VM
...
- Gateway host: OPNEX Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: OPNEX CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update OPNEX in the VM, then enable the bundled plugin there:

```bash
opnex plugins enable google-meet
```

Start the node host in the VM:

```bash
opnex node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
OPNEX_ALLOW_INSECURE_PRIVATE_WS=1 \
  opnex node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
OPNEX_ALLOW_INSECURE_PRIVATE_WS=1 \
  opnex node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
opnex node restart
```

`OPNEX_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`opnex.json` setting. `opnex node install` stores it in the LaunchAgent
...
```bash
opnex googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, OPNEX browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, OPNEX auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `opnex node run` in the VM,
  approve pairing, and make sure `opnex plugins enable google-meet` and
  `opnex plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses OPNEX
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. OPNEX
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by OPNEX; use separate virtual devices or Loopback-style routing
...
OPNEX does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with OPNEX, review BlackHole's
...
Chrome transport opens the Meet URL through OPNEX browser control and joins
as the signed-in OPNEX browser profile. On macOS, the plugin checks for
...
```bash
opnex googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
opnex googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local OPNEX audio
...
meeting; OPNEX does not discover those from the Meet page.
...
opnex plugins list | grep -E 'google-meet|voice-call'
opnex googlemeet setup
```
...
`OPNEX_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes OPNEX requests:
...
`meetings.space.readonly` lets OPNEX resolve Meet URLs/codes to spaces.
...
```bash
opnex googlemeet auth login --json
```
...
```bash
OPNEX_GOOGLE_MEET_CLIENT_ID="your-client-id" \
OPNEX_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
opnex googlemeet auth login --json
```
...
OPNEX_GOOGLE_MEET_CLIENT_ID="your-client-id" \
OPNEX_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
opnex googlemeet auth login --json --manual
```
...
support existed, rerun `opnex googlemeet auth login --json` so the refresh
...
```bash
opnex googlemeet doctor --oauth --json
```
...
```bash
opnex googlemeet doctor --oauth --create-space --json
opnex googlemeet create --no-join --json
```
...
```bash
opnex googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
opnex googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `opnex googlemeet auth login
...
- `OPNEX_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `OPNEX_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `OPNEX_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `OPNEX_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `OPNEX_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `OPNEX_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `OPNEX_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
opnex googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
opnex googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
opnex googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
opnex googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
opnex googlemeet latest --today
opnex googlemeet calendar-events --today --json
opnex googlemeet artifacts --event "Weekly sync"
opnex googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
opnex googlemeet latest --meeting https://meet.google.com/abc-defg-hij
opnex googlemeet artifacts --conference-record conferenceRecords/abc123 --json
opnex googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Write a readable report:

```bash
opnex googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
opnex googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
opnex googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
opnex googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
OPNEX_LIVE_TEST=1 \
OPNEX_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
pnpm test:live -- extensions/google-meet/google-meet.live.test.ts
```

Live smoke environment:

- `OPNEX_LIVE_TEST=1` enables guarded live tests.
- `OPNEX_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `OPNEX_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `OPNEX_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `OPNEX_GOOGLE_MEET_CLIENT_SECRET`,
  `OPNEX_GOOGLE_MEET_ACCESS_TOKEN`, and
  `OPNEX_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `OPNEX_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the OPNEX browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the OPNEX browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, OPNEX reports `manualActionRequired: true` or a
...
- `chrome.guestName: "OPNEX Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through OPNEX browser automation on `chrome-node`
...
- `realtime.instructions`: brief spoken replies, with
  `opnex_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional OPNEX agent id for
  `opnex_agent_consult`; defaults to `main`
...
VM. In both cases the realtime model and `opnex_agent_consult` run on the
...
OPNEX tools, it can call `opnex_agent_consult`.

The consult tool runs the regular OPNEX agent behind the scenes with recent
...
Meet lane should consult a dedicated OPNEX agent workspace, model defaults,
...
```bash
opnex googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
opnex plugins list | grep google-meet
opnex googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; OPNEX
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, OPNEX
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `OPNEX_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `opnex googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the OPNEX Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. OPNEX should click **Use microphone** or, for
...
utterance but OPNEX did not see new output bytes reach the Chrome audio
bridge.
...
```bash
opnex googlemeet recover-tab
opnex googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: OPNEX owns the
