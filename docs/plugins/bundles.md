---
summary: "Install and use Codex, Claude, and Cursor bundles as OPNEX plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how OPNEX maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

OPNEX can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
OPNEX maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native OPNEX plugins. Native plugins run
...
of requiring authors to rewrite them as native OPNEX plugins, OPNEX
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    opnex plugins install ./my-bundle

    # Archive
    opnex plugins install ./my-bundle.tgz
...
## What OPNEX maps from bundles

Not every bundle feature runs in OPNEX today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal OPNEX skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | OPNEX-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal OPNEX skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal OPNEX skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal OPNEX hook-pack
...
- OPNEX merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- OPNEX exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, OPNEX uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in OPNEX config. `opnex mcp set` and `opnex doctor --fix` normalize the common alias.
...
OPNEX registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- OPNEX sanitizes shell override keys before applying them
...
- OPNEX loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `opnex plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but OPNEX does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit OPNEX best when they use skill roots and OPNEX-style
...
OPNEX checks for native plugin format first:

1. `opnex.plugin.json` or valid `package.json` with `opnex.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, OPNEX uses the native path. This prevents
dual-format packages from being partially installed as bundles.

## Runtime dependencies and cleanup

- Bundled plugin runtime dependencies ship inside the OPNEX package under
  `dist/*`. OPNEX does **not** run `npm install` at startup for bundled
...
- OPNEX does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `opnex plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. OPNEX does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    OPNEX hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
