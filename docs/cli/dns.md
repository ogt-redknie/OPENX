---
summary: "CLI reference for `opnex dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: opnex.internal)
title: "DNS"
---
...
```bash
opnex dns setup
opnex dns setup --domain opnex.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `opnex.internal`)
...
- zone file path
- current tailnet IPs
- recommended `opnex.json` discovery config
...
- If `--domain` is omitted, OPNEX uses `discovery.wideArea.domain` from config.
