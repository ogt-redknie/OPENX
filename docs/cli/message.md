---
summary: "CLI reference for `opnex message` (send + channel actions)"
read_when:
...
```
opnex message <subcommand> [flags]
```
...
- `opnex message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, OPNEX will attempt a live directory lookup when the provider supports it.
...
- `opnex message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, OPNEX does not force a `default` account SecretRef scope
...
```
opnex message send --channel discord \
...
```bash
opnex message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
opnex message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
opnex message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
opnex message poll --channel msteams \
...
```
opnex message react --channel slack \
...
```
opnex message react --channel signal \
...
```
opnex message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
opnex message send --channel telegram --target @mychat \
