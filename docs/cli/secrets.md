---
summary: "CLI reference for `opnex secrets` (reload, audit, configure, apply)"
read_when:
...
Use `opnex secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
opnex secrets reload
opnex secrets reload --json
opnex secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan OPNEX state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `opnex.json` refs)
...
opnex secrets audit
opnex secrets audit --check
opnex secrets audit --json
opnex secrets audit --allow-exec
```
...
```bash
opnex secrets configure
opnex secrets configure --plan-out /tmp/opnex-secrets-plan.json
opnex secrets configure --apply --yes
opnex secrets configure --providers-only
opnex secrets configure --skip-provider-setup
opnex secrets configure --agent ops
opnex secrets configure --json
```
...
- `configure` targets secret-bearing fields in `opnex.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, OPNEX fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
opnex secrets apply --from /tmp/opnex-secrets-plan.json --json
```
...
- `opnex.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.opnex/.env` known secret keys whose values were migrated
