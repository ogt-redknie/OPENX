---
summary: "Expose OPNEX channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to OPNEX-backed channels
  - Running `opnex mcp serve`
  - Managing OPNEX-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`opnex mcp` has two jobs:

- run OPNEX as an MCP server with `opnex mcp serve`
- manage OPNEX-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is OPNEX acting as an MCP server
- `list` / `show` / `set` / `unset` is OPNEX acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`opnex acp`](/cli/acp) when OPNEX should host a coding harness session itself and route that runtime through ACP.

## OPNEX as an MCP server

This is the `opnex mcp serve` path.

### When to use `serve`

Use `opnex mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to OPNEX-backed channel conversations
- you already have a local or remote OPNEX Gateway with routed sessions
- you want one MCP server that works across OPNEX's channel backends instead of running separate per-channel bridges

Use [`opnex acp`](/cli/acp) instead when OPNEX should host the coding runtime itself and keep the agent session inside OPNEX.

### How it works

`opnex mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote OPNEX Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `opnex mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the OPNEX Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `opnex agent` and `opnex infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by OPNEX (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when OPNEX already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    opnex mcp serve --url wss://gateway-host:18789 --token-file ~/.opnex/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    opnex mcp serve --url wss://gateway-host:18789 --password-file ~/.opnex/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    opnex mcp serve --verbose
    opnex mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the OPNEX equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying OPNEX channel configuration
...
OPNEX ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `opnex mcp serve`
...
## OPNEX as an MCP client registry

This is the `opnex mcp list`, `show`, `set`, and `unset` path.

These commands do not expose OPNEX over MCP. They manage OPNEX-owned MCP server definitions under `mcp.servers` in OPNEX config.

Those saved definitions are for runtimes that OPNEX launches or configures later, such as embedded Pi and other runtime adapters. OPNEX stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write OPNEX config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes OPNEX `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

### Saved MCP server definitions

OPNEX also stores a lightweight MCP server registry in config for surfaces that want OPNEX-managed MCP definitions.

Commands:

- `opnex mcp list`
- `opnex mcp show [name]`
- `opnex mcp set <name> <json>`
- `opnex mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `opnex mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
opnex mcp list
opnex mcp show context7 --json
opnex mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
opnex mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
opnex mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, OPNEX uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

OPNEX config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `opnex mcp set` and repaired by `opnex doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
