---
summary: "CLI reference for `opnex qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
opnex qr
opnex qr --setup-code-only
opnex qr --json
opnex qr --remote
opnex qr --url wss://gateway.example/ws
```
...
- With `--remote`, OPNEX requires either `gateway.remote.url` or
