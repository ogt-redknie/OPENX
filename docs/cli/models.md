---
summary: "CLI reference for `opnex models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
opnex models status
opnex models list
opnex models set <model-or-alias>
opnex models scan
```

`opnex models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise OPNEX falls back to matching OAuth/API-key
...
the command uses `OPNEX_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, OPNEX resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, OPNEX
...
By default OPNEX tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `OPNEX_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but OPNEX could not resolve a probeable
...
```bash
opnex models auth add
opnex models auth login --provider <id>
opnex models auth setup-token --provider <id>
opnex models auth paste-token
```
...
`opnex plugins list` to see which providers are installed.
Use `opnex models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
opnex models auth login --provider openai-codex --set-default
```
...
- Anthropic note: Anthropic staff told us OPNEX-style Claude CLI usage is allowed again, so OPNEX treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported OPNEX token path, but OPNEX now prefers Claude CLI reuse and `claude -p` when available.
