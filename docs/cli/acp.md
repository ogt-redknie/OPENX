...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an OPNEX Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`opnex acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to OPNEX channel
conversations instead of hosting an ACP harness session, use
[`opnex mcp serve`](/cli/mcp) instead.
...
- OPNEX acts as an ACP server
- an IDE or ACP client connects to OPNEX
- OPNEX forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where OPNEX runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to OPNEX: use `opnex acp`
- OPNEX should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the OPNEX gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
opnex acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
opnex acp --url wss://gateway-host:18789 --token-file ~/.opnex/gateway.token

# Attach to an existing session key
opnex acp --session agent:main:main

# Attach by label (must already exist)
opnex acp --session-label "support inbox"

# Reset the session key before the first prompt
opnex acp --session agent:main:main --reset-session
```
...
opnex acp client --server-args --url wss://gateway-host:18789 --token-file ~/.opnex/gateway.token

# Override the server command (default: opnex)
opnex acp client --server "node" --server-args opnex.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run OPNEX through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an OPNEX Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `opnex acp` over stdio.

Example config (persisted):

```bash
opnex config set gateway.remote.url wss://gateway-host:18789
opnex config set gateway.remote.token <token>
```
...
```bash
opnex acp --session agent:main:main
opnex acp --session agent:design:main
opnex acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see OPNEX plugin tools or selected
...
[OPNEX tools MCP bridge](/tools/acp-agents-setup#opnex-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
OPNEX bot over ACP, use `acpx` with its built-in `opnex` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx opnex` at `opnex acp`.
3. Target the OPNEX session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default OPNEX ACP session
acpx opnex exec "Summarize the active OPNEX session state."

# Persistent named session for follow-up turns
acpx opnex sessions ensure --name codex-bridge
acpx opnex -s codex-bridge --cwd /path/to/repo \
  "Ask my OPNEX work agent for recent context relevant to this repo."
```

If you want `acpx opnex` to target a specific Gateway and session key every
time, override the `opnex` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "opnex": {
      "command": "env OPNEX_HIDE_BANNER=1 OPNEX_SUPPRESS_NOTES=1 opnex acp --url ws://127.0.0.1:18789 --token-file ~/.opnex/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local OPNEX checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPNEX_HIDE_BANNER=1 OPNEX_SUPPRESS_NOTES=1 node opnex.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an OPNEX agent without scraping a terminal.
...
In Zed, open the Agent panel and select “OPNEX ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`OPNEX_GATEWAY_TOKEN`, `OPNEX_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`OPNEX_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `OPNEX_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `opnex acp client` sets `OPNEX_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `opnex`).
