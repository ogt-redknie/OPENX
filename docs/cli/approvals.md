---
summary: "CLI reference for `opnex approvals` and `opnex exec-policy`"
read_when:
...
`opnex exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.opnex/exec-approvals.json`

Examples:

```bash
opnex exec-policy show
opnex exec-policy show --json

opnex exec-policy preset yolo
opnex exec-policy preset cautious --json

opnex exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `opnex exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `opnex approvals set --gateway`
or `opnex approvals set --node <id|name|ip>`.

## Common commands

```bash
opnex approvals get
opnex approvals get --node <id|name|ip>
opnex approvals get --gateway
```

`opnex approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
opnex approvals set --file ./exec-approvals.json
opnex approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
opnex approvals set --node <id|name|ip> --file ./exec-approvals.json
opnex approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested OPNEX policy aligned, also set:
...
```bash
opnex approvals allowlist add "~/Projects/**/bin/rg"
opnex approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
opnex approvals allowlist add --agent "*" "/usr/bin/uname"

opnex approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `opnex nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.opnex/exec-approvals.json`.
