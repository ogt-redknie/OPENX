---
summary: "CLI reference for `opnex memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
opnex memory status --fix
opnex memory index --force
opnex memory search "meeting notes"
opnex memory search --query "deployment" --max-results 20
opnex memory promote --limit 10 --min-score 0.75
opnex memory promote --apply
opnex memory promote --json --min-recall-count 0 --min-unique-queries 0
...
opnex memory status --json
opnex memory status --deep --index
opnex memory status --deep --index --verbose
opnex memory status --agent main
opnex memory index --agent main --verbose
```
...
```bash
opnex memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `opnex cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
opnex memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
opnex memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
