...
read_when:
  ...
  - You encounter Task Flow or opnex tasks flow in release notes or docs
...
3. Use [OPNEX](/tools/OPNEX) for deterministic steps, approval gates, and resume tokens.
...
opnex cron add \
  ...
  --message "Run the market-intel OPNEX workflow. Verify source freshness before summarizing." \
  --announce \
  --channel slack \
...
- Browser availability and profile choice, for example `opnex` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
- API credentials and quota for each source.
- Network reachability for required endpoints.
- Required tools enabled for the agent, such as `OPNEX`, `browser`, and `llm-task`.
...
For reusable team or community workflows, package the CLI, `.opnex` files, and any setup notes as a skill or plugin and publish it through [ClawHub](/tools/clawhub). Keep workflow-specific guardrails in that package unless the plugin API is missing a needed generic capability.
...
`opnex tasks flow cancel` sets a sticky cancel intent on the flow. Active tasks within the flow are cancelled, and no new steps are started. The cancel intent persists across restarts, so a cancelled flow stays cancelled even if the gateway restarts before all child tasks have terminated.
...
| Command                           | Description                                   |
| --------------------------------- | --------------------------------------------- |
| `opnex tasks flow list`        | Shows tracked flows with status and sync mode |
| `opnex tasks flow show <id>`   | Inspect one flow by flow id or lookup key     |
| `opnex tasks flow cancel <id>` | Cancel a running flow and its active tasks    |

## How flows relate to tasks

Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `opnex tasks` to inspect individual task records and `opnex tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) — the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) — CLI command reference for `opnex tasks flow`
