---
summary: "Run OPNEX on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for OPNEX
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the OPNEX Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/opnex-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `OPNEX_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `OPNEX_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/opnex-compile-cache`
...
For the standard `opnex onboard --install-daemon` path, edit the user unit:
...
`opnex-gateway.service` via `sudo systemctl edit opnex-gateway.service`.
