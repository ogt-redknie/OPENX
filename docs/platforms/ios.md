...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `opnex.internal.`), **or**
...
Run `opnex devices list` again before approval.
...
```bash
opnex nodes status
opnex gateway call node.list --params "{}"
```
...
- `OPNEX_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine OPNEX iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight OPNEX builds.
...
```bash
export OPNEX_APNS_TEAM_ID="TEAMID"
export OPNEX_APNS_KEY_ID="KEYID"
export OPNEX_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.opnex/credentials/apns
chmod 700 ~/.opnex/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.opnex/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.opnex/credentials/apns/AuthKey_KEYID.p8
export OPNEX_APNS_PRIVATE_KEY_PATH="$HOME/.opnex/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_opnex-gw._tcp` on `local.` and, when configured, the same
...
`opnex.internal.`) and Tailscale split DNS.
...
```bash
opnex nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__opnex__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__opnex__/canvas/` and `/__opnex__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through OPNEX node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through OPNEX by invoking node
...
```bash
opnex nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__opnex; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
opnex nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `opnex devices list` and approve manually.
