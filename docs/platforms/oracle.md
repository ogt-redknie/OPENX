---
summary: "OPNEX on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up OPNEX on Oracle Cloud
  - Looking for low-cost VPS hosting for OPNEX
  - Want 24/7 OPNEX on a small server
title: "Oracle Cloud (platform)"
---

# OPNEX on Oracle Cloud (OCI)

## Goal

Run a persistent OPNEX Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for OPNEX (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname opnex
...
sudo tailscale up --ssh --hostname=opnex
```

This enables Tailscale SSH, so you can connect via `ssh opnex` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@opnex` (or use the Tailscale IP).
...
opnex config set gateway.bind loopback

# Require auth for the Gateway + Control UI
opnex config set gateway.auth.mode token
opnex doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
opnex config set gateway.tailscale.mode serve
opnex config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart opnex-gateway.service
```
...
# Check daemon status
systemctl --user status opnex-gateway.service
...
```
https://opnex.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `opnex security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.opnex`
- **Security audit:** `opnex security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@opnex
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=opnex --reset
```
...
journalctl --user -u opnex-gateway.service -n 50
```
...
- `~/.opnex/` — `opnex.json`, per-agent `auth-profiles.json`, channel/provider state, and session data
- `~/.opnex/workspace/` — workspace (SOUL.md, memory, artifacts)
