...
read_when:
  - Installing OPNEX on Windows
  - Choosing between native Windows and WSL2
  - Looking for Windows companion app status
title: "Windows"
---

OPNEX supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `opnex --version`, `opnex doctor`, and `opnex plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
opnex agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `opnex onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `opnex onboard --non-interactive --install-daemon` and `opnex gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, OPNEX falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, OPNEX now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
opnex onboard --non-interactive --skip-health
opnex gateway run
```
...
### 2) Install the OPNEX gateway user service
...
systemctl --user status opnex-gateway.service --no-pager
```
...
pnpm build
pnpm ui:build
pnpm opnex onboard --install-daemon
```
