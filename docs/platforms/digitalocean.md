---
summary: "OPNEX on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OPNEX on DigitalOcean
  - Looking for cheap VPS hosting for OPNEX
title: "DigitalOcean (platform)"
---

# OPNEX on DigitalOcean

## Goal

Run a persistent OPNEX Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
opnex config set gateway.bind tailnet
opnex gateway restart
```
...
- `~/.opnex/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u opnex-gateway.service --no-pager -n 50
```
