---
summary: "OPNEX on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OPNEX on a Raspberry Pi
  - Running OPNEX on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (platform)"
---

# OPNEX on Raspberry Pi

## Goal

Run a persistent, always-on OPNEX Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u opnex-gateway.service -f
```

## 9) Access the OPNEX Dashboard
...
```bash
ssh user@gateway-host 'opnex dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/opnex-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/opnex-compile-cache
mkdir -p /var/tmp/opnex-compile-cache
export OPNEX_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPNEX_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running OPNEX, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit opnex-gateway.service
```

```ini
[Service]
Environment=OPNEX_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/opnex-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep OPNEX state/cache on SSD-backed storage to avoid SD-card
...
Most OPNEX features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled opnex-gateway.service

# Enable if not
systemctl --user enable opnex-gateway.service
...
```bash
# Check logs
journalctl --user -u opnex-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/opnex  # if using hackable install
npm run build
systemctl --user restart opnex-gateway.service
```
