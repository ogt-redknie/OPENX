...
service via the external `opnex` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.opnex.gateway`
  (or `ai.opnex.<profile>` when using `--profile`/`OPNEX_PROFILE`; legacy `com.opnex.*` is supported).
