...
- Same path: run the app from a fixed location (for OPNEX, `dist/OPNEX.app`).
...
```bash
sudo tccutil reset Accessibility ai.opnex.mac
sudo tccutil reset ScreenCapture ai.opnex.mac
...
Workaround: move files into the OPNEX workspace (`~/.opnex/workspace`) if you want to avoid per-folder grants.
