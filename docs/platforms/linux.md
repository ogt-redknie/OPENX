...
2. `npm i -g opnex@latest`
3. `opnex onboard --install-daemon`
...
OPNEX installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `opnex gateway install` and
`opnex onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/opnex-gateway[-<profile>].service`:

```
[Unit]
Description=OPNEX Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. OPNEX therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, OPNEX starts the child through a short
...
- OPNEX-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `OPNEX_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
