...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OPNEX
...
Running OPNEX via Node with `tsx` fails at startup with:

```
[opnex] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opnex status` and `gateway:watch` worked.
