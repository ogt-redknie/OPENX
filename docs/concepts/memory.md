---
summary: "How OPNEX remembers things across sessions"
...
OPNEX remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.opnex/workspace`).
...
<Info>
OPNEX auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, OPNEX
...
```bash
opnex memory rem-backfill --rollback
opnex memory rem-backfill --rollback-short-term
```

## CLI

```bash
opnex memory status          # Check index status and provider
opnex memory search "query"  # Search from the command line
opnex memory index --force   # Rebuild the index
```
