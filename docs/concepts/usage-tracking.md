...
- `/usage cost` in chats: local cost summary aggregated from OPNEX session logs.
- CLI: `opnex status --usage` prints a full per-provider breakdown.
- CLI: `opnex channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. OPNEX treats
  ...
  quota, so OPNEX inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, OPNEX prefers the
...
can supply plugin-specific usage auth logic; otherwise OPNEX falls back to
