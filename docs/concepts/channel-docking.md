---
summary: "Move one OPNEX session's reply route between linked chat channels"
...
Channel docking is call forwarding for one OPNEX session.
...
Alice can message OPNEX on Telegram and Discord:
...
OPNEX keeps the current session context and changes the reply route:
...
4. Keep the same OPNEX session, but receive future replies in Discord.
