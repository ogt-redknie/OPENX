...
`node-llama-cpp` runtime package next to OPNEX, then point `local.modelPath`
at a GGUF file:
...
OPNEX indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.opnex/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `opnex memory index --force`
...
**Memory search disabled?** Check `opnex memory status`. If no provider is
...
**Stale results?** Run `opnex memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** OPNEX falls back to in-process cosine similarity
