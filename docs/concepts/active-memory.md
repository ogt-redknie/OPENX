...
Paste this into `opnex.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, OPNEX can show:
...
`config.promptOverride` replaces the default Active Memory prompt. OPNEX
...
5. Verify memory search itself works with `opnex memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, OPNEX auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `opnex memory status --deep` to inspect the memory-search backend
      and index health.
