...
alongside OPNEX. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, OPNEX falls back to the
...
OPNEX creates a self-contained QMD home under
`~/.opnex/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- OPNEX creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so OPNEX skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, OPNEX retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, OPNEX groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, OPNEX falls back to the builtin SQLite engine.
  ...
  `opnex memory status` and one-shot CLI probes still recheck QMD directly.
...
OPNEX keeps the QMD search path compatible with both current and older QMD
installs.

On startup, OPNEX checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, OPNEX searches all
...
Older QMD builds only accept one collection filter. When OPNEX detects one
...
process, so you can tune QMD globally without adding new OPNEX config:
...
collection under `~/.opnex/agents/<id>/qmd/sessions/`.
...
When scope denies a search, OPNEX logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If OPNEX
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but OPNEX still reports
...
with `opnex memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs OPNEX uses.

**Many QMD subprocesses during search?** Update QMD if possible. OPNEX uses
...
`memory.qmd.searchMode = "search"`. OPNEX treats that mode as lexical-only,
...
the next startup reconciliation. OPNEX recreates stale managed collections
...
OPNEX's builtin symlink rules. Keep temporary monorepo checkouts under
