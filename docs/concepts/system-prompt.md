---
summary: "What the OPNEX system prompt contains and how it is assembled"
read_when:
...
OPNEX builds a custom system prompt for every agent run. The prompt is **OPNEX-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by OPNEX and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full OPNEX-owned prompt. The provider runtime can:
...
- **OPNEX Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to OPNEX docs (repo or npm package) and when to read them.
...
OPNEX can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **OPNEX
...
For channel auto-reply runs, OPNEX can omit the generic **Silent Replies**
...
occurs, OPNEX can inject a warning block in Project Context; control this with
...
When eligible skills exist, OPNEX injects a compact **available skills list**
...
points to the local OPNEX docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.opnex.ai](https://docs.opnex.ai).

The same section also includes the OPNEX source location. Git checkouts expose the local
...
consult docs first for OPNEX behavior, commands, configuration, or architecture, and to
run `opnex status` itself when possible (asking the user only when it lacks access).
