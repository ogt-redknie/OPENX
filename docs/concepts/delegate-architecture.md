---
summary: "Delegate architecture: running OPNEX as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run OPNEX as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an OPNEX agent that:
...
OPNEX's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of OPNEX's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.opnex/workspace-delegate",
  tools: {
...
- Cron run history: `~/.opnex/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.opnex/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through OPNEX's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
opnex agents add delegate
```

This creates:

- Workspace: `~/.opnex/workspace-delegate`
- State: `~/.opnex/agents/delegate/agent`
- Sessions: `~/.opnex/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.opnex/workspace" },
      {
        id: "delegate",
        workspace: "~/.opnex/workspace-delegate",
        tools: {
...
~/.opnex/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.opnex/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.opnex/workspace-org",
        agentDir: "~/.opnex/agents/org-assistant/agent",
...
recall view. OPNEX redacts credential/token-like text, truncates long
