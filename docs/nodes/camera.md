...
OPNEX supports **camera capture** for agent workflows:
...
```bash
opnex nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
opnex nodes camera snap --node <id> --facing front
opnex nodes camera clip --node <id> --duration 3000
opnex nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`opnex.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `opnex` CLI to invoke camera commands on the macOS node.

Examples:

```bash
opnex nodes camera list --node <id>            # list camera ids
opnex nodes camera snap --node <id>            # prints MEDIA:<path>
opnex nodes camera snap --node <id> --max-width 1280
opnex nodes camera snap --node <id> --delay-ms 2000
opnex nodes camera snap --node <id> --device-id <id>
opnex nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
opnex nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
opnex nodes camera clip --node <id> --device-id <id>
opnex nodes camera clip --node <id> --no-audio
```

Notes:

- `opnex nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
opnex nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
