...
```bash
opnex nodes describe --node <idOrNameOrIp>
opnex nodes canvas snapshot --node <idOrNameOrIp>
opnex logs --follow
```
...
opnex nodes status
opnex approvals get --node <idOrNameOrIp>
opnex approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`opnex approvals get --node ...`), not in the gateway pairing record.
