---
summary: "Use Xiaomi MiMo models with OPNEX"
read_when:
  - You want Xiaomi MiMo models in OPNEX
  - You need XIAOMI_API_KEY setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. OPNEX uses the Xiaomi
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    opnex onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    opnex onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    opnex models list --provider xiaomi
    ```
  </Step>
</Steps>
...
targets such as Feishu and Telegram, OPNEX transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.opnex/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.opnex/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
