...
read_when:
  - You want to use Volcano Engine or Doubao models with OPNEX
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    opnex onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    opnex models list --provider volcengine
    opnex models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, OPNEX asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `opnex onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, OPNEX falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.opnex/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running OPNEX as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about OPNEX setup.
  </Card>
</CardGroup>
