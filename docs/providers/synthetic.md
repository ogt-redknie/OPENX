---
summary: "Use Synthetic's Anthropic-compatible API in OPNEX"
read_when:
...
OPNEX registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    opnex onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
OPNEX's Anthropic client appends `/v1` to the base URL automatically, so use
...
`opnex models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that OPNEX appends `/v1` automatically.
