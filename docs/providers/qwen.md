---
summary: "Use Qwen Cloud via OPNEX's bundled qwen provider"
read_when:
  - You want to use Qwen with OPNEX
...
See [Issue #49557](https://github.com/opnex/opnex/issues/49557) for
background.

</Warning>

OPNEX now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        opnex onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        opnex onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        opnex models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        opnex onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        opnex onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
OPNEX currently ships this bundled Qwen catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the bundled provider maps OPNEX
...
<AccordionGroup>
  ...
  <Accordion title="Video generation details">
    For video generation, OPNEX maps the configured Qwen region to the matching
