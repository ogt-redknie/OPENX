...
read_when:
  - You want to use Vercel AI Gateway with OPNEX
...
<Tip>
OPNEX auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    opnex onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    opnex models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
OPNEX accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. OPNEX resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the OPNEX Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.opnex/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
