---
summary: "Use xAI Grok models in OPNEX"
read_when:
  - You want to use Grok models in OPNEX
  - You are configuring xAI auth or model ids
title: "xAI"
---

OPNEX ships a bundled `xai` provider plugin for Grok models.
...
<Steps>
  ...
  <Step title="Set your API key">
    ...
    ```bash
    opnex onboard --auth-choice xai-api-key
    ```
...
<Note>
OPNEX uses the xAI Responses API as the bundled xAI transport. The same
...
OPNEX includes these xAI model families out of the box:
...
The bundled plugin maps xAI's current public API surface onto OPNEX's shared
...
| xAI capability             | OPNEX surface                          | Status                                                              |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
...
| Streaming TTS              | —                                         | Not exposed; OPNEX's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                 |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                 |
| Realtime voice             | —                                         | Not exposed yet; different session/WebSocket contract               |
| Files / batches            | Generic model API compatibility only      | Not a first-class OPNEX tool                                     |

<Note>
OPNEX uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different OPNEX contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    opnex config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    OPNEX can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    OPNEX asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. OPNEX forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    OPNEX uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the OPNEX speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through OPNEX's
    ...
    transcription request. Prompt hints are accepted by the shared OPNEX
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an OPNEX tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an OPNEX tool for
  ...
  <Accordion title="Known limits">
    ...
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      ...
      surface than the standard OPNEX xAI transport.
    - xAI Realtime voice is not registered as an OPNEX provider yet. It
  ...
  <Accordion title="Advanced notes">
    - OPNEX applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as OPNEX
      tools. OPNEX enables the specific xAI built-in it needs inside each tool
...
OPNEX_LIVE_TEST=1 OPNEX_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
OPNEX_LIVE_TEST=1 OPNEX_LIVE_TEST_QUIET=1 OPNEX_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through OPNEX's
