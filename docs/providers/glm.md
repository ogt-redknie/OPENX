---
summary: "GLM model family overview + how to use it in OPNEX"
read_when:
  - You want GLM models in OPNEX
...
GLM is a **model family** (not a company) available through the Z.AI platform. In OPNEX, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    opnex config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets OPNEX detect the matching Z.AI endpoint from the key and
...
OPNEX currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, OPNEX inspects the key format
