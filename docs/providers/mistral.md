---
summary: "Use Mistral models and Voxtral transcription with OPNEX"
read_when:
  - You want to use Mistral models in OPNEX
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

OPNEX supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    opnex onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    opnex onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    opnex models list --provider mistral
    ```
  </Step>
</Steps>
...
OPNEX currently ships this bundled Mistral catalog:
...
<Note>
OPNEX defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    OPNEX maps the session **thinking** level to Mistral's API:

    | OPNEX thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
