---
summary: "Use Venice AI privacy-focused models in OPNEX"
read_when:
  - You want privacy-focused inference in OPNEX
...
<Steps>
  ...
  <Step title="Configure OPNEX">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        opnex onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    opnex agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, OPNEX shows all available Venice models. Pick based on your needs:
...
```bash
opnex models set venice/kimi-k2-5
opnex models set venice/claude-opus-4-6
```

List all available models:

```bash
opnex models list | grep venice
```

You can also run `opnex configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, OPNEX fills the required DeepSeek V4
...
OPNEX keeps that provider-specific replay fix separate from the native
...
OPNEX automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opnex agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
opnex agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
opnex agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
opnex agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
opnex agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `opnex models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
