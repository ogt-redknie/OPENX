...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://opnex.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://opnex.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://opnex.ai/abc-123", baseHref)).toBe(
      "blob:https://opnex.ai/abc-123",
    );
  });
