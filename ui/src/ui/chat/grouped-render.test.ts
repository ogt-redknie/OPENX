...
vi.mock("../views/agents-utils.ts", () => {
  ...
  return {
    assistantAvatarFallbackUrl: () => "/opnex-molty.png",
    agentLogoUrl: () => "/opnex-logo.svg",
...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__opnex__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("opnex:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript and content image variants", () => {
    const renderUserMedia = (message: unknown) => {
      ...
      renderGroupedMessage(container, message, "user", {
        ...
        localMediaPreviewRoots: ["/tmp/opnex"],
      });
      return container;
    };
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/opnex/user-upload.png",
      timestamp: Date.now(),
    });
    expect(
      container.querySelector<HTMLImageElement>(".chat-message-image")?.getAttribute("src"),
    ).toBe(
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Fuser-upload.png&token=session-token",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/opnex/first.png", "/tmp/opnex/second.jpg"],
    ...
    expect(
      ...
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ffirst.png&token=session-token",
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Fsecond.jpg&token=session-token",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/__opnex__/media/user-upload.pdf",
    ...
    expect(documentLink?.getAttribute("href")).toBe("/__opnex__/media/user-upload.pdf");
  });
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      ...
      expect(headers.get("x-opnex-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ftest+image.png&token=session-token&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ftest+image.png&token=session-token",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ftest-doc.pdf&token=session-token",
    );
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ftest+image.png&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/opnex/__opnex__/assistant-media?source=%2Ftmp%2Fopnex%2Ftest+image.png&token=fresh-token&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(container.querySelector(".chat-message-image")).not.toBeNull();
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__opnex__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__opnex__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/opnex/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/opnex/__opnex__/assistant-media?source=%2FC%3A%2Ftmp%2Fopnex%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/opnex/__opnex__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/opnex/__opnex__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__opnex__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__opnex__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__opnex__/cap/cap_123/__opnex__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__opnex__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__opnex__/canvas/documents/cv_inline_default/index.html",
    );
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__opnex__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
