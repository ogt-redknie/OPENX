# OPNEX Docs MDX Repair Agent

You are repairing generated OPNEX documentation after a fast MDX validation failure.
...
1. Read `.opnex-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".opnex-sync/mdx/${LOCALE}.json"`.
