...
This document describes how the OPNEX ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opnex acp` exposes an ACP agent over stdio and forwards prompts to a running
OPNEX Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing OPNEX Gateway
...
- ACP client spawns `opnex acp` and speaks ACP messages over stdio.
...
`opnex acp` resolves the Gateway URL and auth from CLI flags or config:
