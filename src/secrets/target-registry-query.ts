...
let compiledSecretTargetRegistryState: {
  ...
  opnexCompiledSecretTargets: CompiledTargetRegistryEntry[];
  opnexTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreOPNEXTargetState: {
  knownTargetIds: Set<string>;
  opnexCompiledSecretTargets: CompiledTargetRegistryEntry[];
  opnexTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelOPNEXTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const opnexCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "opnex.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    opnexCompiledSecretTargets,
    opnexTargetsById: buildConfigTargetIdIndex(opnexCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreOPNEXTargetState() {
  if (compiledCoreOPNEXTargetState) {
    return compiledCoreOPNEXTargetState;
  }
  const opnexCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "opnex.json")
    .map(compileTargetRegistryEntry);
  compiledCoreOPNEXTargetState = {
    knownTargetIds: new Set(opnexCompiledSecretTargets.map((entry) => entry.id)),
    opnexCompiledSecretTargets,
    opnexTargetsById: buildConfigTargetIdIndex(opnexCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(opnexCompiledSecretTargets),
  };
  return compiledCoreOPNEXTargetState;
}

function getCompiledBundledChannelOPNEXTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelOPNEXTargets.has(normalizedChannelId)) {
    return compiledBundledChannelOPNEXTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "opnex.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelOPNEXTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreOPNEXTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreOPNEXTargetState().opnexCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelOPNEXTargets(explicitBundledChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().opnexCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreOPNEXTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreOPNEXTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.opnexCompiledSecretTargets,
    entriesById: registryState.opnexTargetsById,
  });
