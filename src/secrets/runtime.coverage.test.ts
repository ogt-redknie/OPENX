...
const DEBUG_COVERAGE_BATCHES = process.env.OPNEX_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_OPNEX_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForOPNEXTarget(
  config: OPNEXConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every opnex.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "opnex.json" &&
        !PLUGIN_OWNED_OPNEX_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("opnex.json", batch);
      const config = {} as OPNEXConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `OPNEX_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForOPNEXTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `OPNEX_AUTH_SECRET_TARGET_${entry.id}`;
