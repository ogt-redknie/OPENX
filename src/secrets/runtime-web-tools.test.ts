...
function setConfiguredFetchProviderKey(configTarget: OPNEXConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: OPNEXConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: OPNEXConfig, provider: ProviderUnderTest): unknown {
