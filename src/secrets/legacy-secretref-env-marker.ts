...
function toCandidate(
  ...
  defaults: NonNullable<OPNEXConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: OPNEXConfig): {
  config: OPNEXConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as OPNEXConfig & Record<string, unknown>;
