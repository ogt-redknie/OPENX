...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("opnex-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".opnex", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".opnex", "agents", "ops", "agent");
