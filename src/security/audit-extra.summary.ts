...
function summarizeGroupPolicy(cfg: OPNEXConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: OPNEXConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OPNEXConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OPNEXConfig): boolean {
...
function isBrowserEnabled(cfg: OPNEXConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: OPNEXConfig): SecurityAuditFinding[] {
