...
type CollectDangerousConfigFlags = (cfg: OPNEXConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.OPNEX_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.OPNEX_GATEWAY_PASSWORD);
