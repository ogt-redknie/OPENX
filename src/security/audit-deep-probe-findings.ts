...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("opnex status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("opnex security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set OPNEX_GATEWAY_TOKEN/OPNEX_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("opnex security audit --deep")}".`,
    });
  }
  return findings;
}
