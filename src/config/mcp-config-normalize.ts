...
export type OPNEXMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_OPNEX_TRANSPORT: Record<string, OPNEXMcpHttpTransport | "stdio"> = {
...
export function resolveOPNEXMcpTransportAlias(
  value: unknown,
): OPNEXMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_OPNEX_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_OPNEX_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveOPNEXMcpTransportAlias(next.type);
