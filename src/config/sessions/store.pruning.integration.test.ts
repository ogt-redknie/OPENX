...
// Keep integration tests deterministic: never read a real opnex.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "opnex-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.OPNEX_SESSION_CACHE_TTL_MS;
    process.env.OPNEX_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.OPNEX_SESSION_CACHE_TTL_MS;
    } else {
      process.env.OPNEX_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"opnex-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"opnex-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
