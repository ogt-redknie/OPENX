...
import { resetConfigRuntimeState, type OPNEXConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        OPNEX_DISABLE_PLUGIN_DISCOVERY_CACHE: undefined,
        OPNEX_DISABLE_PLUGIN_MANIFEST_CACHE: undefined,
        OPNEX_PLUGIN_CATALOG_PATHS: undefined,
        OPNEX_MPM_CATALOG_PATHS: undefined,
        OPNEX_PLUGIN_DISCOVERY_CACHE_MS: undefined,
        OPNEX_PLUGIN_MANIFEST_CACHE_MS: undefined,
        OPNEX_LOAD_SHELL_ENV: undefined,
        OPNEX_DEFER_SHELL_ENV_FALLBACK: undefined,
        OPNEX_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeOPNEXConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.OPNEX_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected OPNEX_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeOPNEXConfig(home, config);
...
export function buildWebSearchProviderConfig(params: {
  provider: NonNullable<
    NonNullable<NonNullable<NonNullable<OPNEXConfig["tools"]>["web"]>["search"]>["provider"]
  >;
  enabled?: boolean;
