...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("opnex-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "opnex-sandbox-browser-global" },
      agentBrowser: { network: "opnex-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("opnex-sandbox-browser-agent");
  });
