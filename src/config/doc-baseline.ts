...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.OPNEX_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveOPNEXPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    OPNEX_STATE_DIR: path.join(os.tmpdir(), "opnex-config-doc-baseline-state"),
    OPNEX_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
