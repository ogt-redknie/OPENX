...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPNEX_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-opnex",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-opnex#general");
  });
  ...
  it("derives session transcripts dir from OPNEX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPNEX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPNEX_STATE_DIR differs from stored paths", () => {
