...
function collectConfigEnvVarsByTarget(cfg?: OPNEXConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OPNEXConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: OPNEXConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: OPNEXConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.OPNEX_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
