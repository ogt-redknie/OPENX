...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.opnex) || !isRecord(entry.opnex.channel)) {
      continue;
    }
    const channel = entry.opnex.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: OPNEXConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: OPNEXConfig, pluginId: string) => boolean;
