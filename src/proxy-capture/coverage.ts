...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[opnex proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[opnex proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`opnex proxy coverage\` for details.`,
  );
}
