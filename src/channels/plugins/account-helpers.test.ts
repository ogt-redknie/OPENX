...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): OPNEXConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as OPNEXConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: OPNEXConfig) => string[];
  input: OPNEXConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: OPNEXConfig, expected: string) {
