...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OPNEXConfig, policy: ChannelAccessPolicy) => OPNEXConfig;
  resolveAllowlist: (params: { cfg: OPNEXConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OPNEXConfig; resolved: TResolved }) => OPNEXConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OPNEXConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OPNEXConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OPNEXConfig, policy: ChannelAccessPolicy): OPNEXConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OPNEXConfig, policy: ChannelAccessPolicy): OPNEXConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OPNEXConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OPNEXConfig; resolved: string[] }) => {
      calls.push("apply");
