...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: OPNEXConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: OPNEXConfig }>(
  collector: WarningCollector<{ cfg: OPNEXConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: OPNEXConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: OPNEXConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: OPNEXConfig },
>(
  projectCfg: (cfg: OPNEXConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: OPNEXConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: OPNEXConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: OPNEXConfig },
>(params: {
  providerConfigPresent: (cfg: OPNEXConfig) => boolean;
