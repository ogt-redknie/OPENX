...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when OPNEX_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".opnex", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when OPNEX_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", OPNEX_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".opnex-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers OPNEX_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        OPNEX_PROFILE: "rescue",
        OPNEX_STATE_DIR: "C:\\State\\opnex",
      },
      expected: path.join("C:\\State\\opnex", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPNEX_PROFILE: "default" },
      expected: path.join("/home/test", ".opnex", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem OPNEX Gateway",
          "cd /d C:\\Projects\\opnex",
          "set NODE_ENV=production",
          "set OPNEX_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\opnex",
          environment: {
            NODE_ENV: "production",
            OPNEX_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\opnex\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\OPNEX Share\\node.exe" "\\\\fileserver\\OPNEX Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\OPNEX Share\\node.exe",
            "\\\\fileserver\\OPNEX Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from OPNEX_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ OPNEX_STATE_DIR: path.join(tmpDir, "custom-state") }),
