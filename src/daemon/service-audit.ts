...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds OPNEX_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `opnex gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OPNEX_GATEWAY_TOKEN does not match gateway.auth.token in opnex.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.OPNEX_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.OPNEX_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `opnex gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
