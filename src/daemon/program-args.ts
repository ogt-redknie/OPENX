...
export const OPNEX_WRAPPER_ENV_KEY = "OPNEX_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/opnex -> .pnpm/opnex@X.Y.Z/...
...
export async function resolveOPNEXWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${OPNEX_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveOPNEXWrapperPath(params.wrapperPath);
  if (wrapperPath) {
