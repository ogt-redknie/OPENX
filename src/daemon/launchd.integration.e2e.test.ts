...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opnex-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPNEX_LAUNCHD_LABEL: `ai.opnex.launchd-int-${testId}`,
      OPNEX_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
