...
describe("resolveGatewayProgramArguments", () => {
  it("prefers index.js over legacy entry.js when both exist in the same dist directory", async () => {
    const entryPath = path.resolve("/opt/opnex/dist/entry.js");
    const indexPath = path.resolve("/opt/opnex/dist/index.js");
  ...
  it("keeps entry.js when index.js is missing", async () => {
    const entryPath = path.resolve("/opt/opnex/dist/entry.js");
    const indexPath = path.resolve("/opt/opnex/dist/index.js");
    const indexMjsPath = path.resolve("/opt/opnex/dist/index.mjs");
    process.argv = ["node", entryPath];
    fsMocks.realpath.mockResolvedValue(entryPath);
  ...
  it("uses realpath-resolved dist entry when running via npx shim", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/opnex");
    const entryPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/opnex/dist/entry.js");
  ...
  it("prefers symlinked path over realpath for stable service config", async () => {
    // Simulates pnpm global install where node_modules/opnex is a symlink
    // to .pnpm/opnex@X.Y.Z/node_modules/opnex
    const symlinkPath = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/opnex/dist/entry.js",
    );
    const realpathResolved = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/opnex@2026.1.21-2/node_modules/opnex/dist/entry.js",
    );
    ...
    expect(result.programArguments[1]).toBe(
      path.resolve("/Users/test/Library/pnpm/global/5/node_modules/opnex/dist/index.js"),
    );
  ...
  it("falls back to node_modules package dist when .bin path is not resolved", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/opnex");
    const indexPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/opnex/dist/index.js");
    process.argv = ["node", argv1];
  ...
  it("rejects a non-executable wrapper file", async () => {
    ...
    ).rejects.toThrow("OPNEX_WRAPPER must point to an executable file");
  });
});
