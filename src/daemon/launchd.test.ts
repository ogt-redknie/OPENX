...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.opnex.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", OPNEX_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.opnex/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.opnex/service-env/ai.opnex.gateway.env";
    const wrapperPath = "/Users/test/.opnex/service-env/ai.opnex.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.opnex/service-env")).toBe(0o700);
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.opnex.gateway</string>",
  ...
  it("stops LaunchAgent by disabling relaunch before stopping the process", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.opnex.gateway`;
    expect(state.launchctlCalls).toContainEqual(["disable", serviceId]);
    expect(state.launchctlCalls).toContainEqual(["stop", "ai.opnex.gateway"]);
  ...
  it("treats already-unloaded services as successfully stopped without bootout fallback", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual([
      "disable",
      `${typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501"}/ai.opnex.gateway`,
    ]);
    expect(state.launchctlCalls.some((call) => call[0] === "bootout")).toBe(false);
    expect(output).toContain("Stopped LaunchAgent");
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.opnex.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when OPNEX_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.opnex.gateway.plist",
    },
    {
      name: "uses profile-specific label when OPNEX_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", OPNEX_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.opnex.jbphoenix.plist",
    },
    {
      name: "prefers OPNEX_LAUNCHD_LABEL over OPNEX_PROFILE",
      env: {
        HOME: "/Users/test",
        OPNEX_PROFILE: "jbphoenix",
        OPNEX_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from OPNEX_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        OPNEX_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty OPNEX_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        OPNEX_PROFILE: "myprofile",
        OPNEX_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.opnex.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        OPNEX_LAUNCHD_LABEL: "../evil/label",
      }),
