...
function hasMeaningfulConfig(config: OPNEXConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.OPNEX_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh OPNEX setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: OPNEXConfig) => Promise<OPNEXConfig>;
}): Promise<void> {
  const [
  ...
  await params.prompter.outro("Migration complete. Run `opnex doctor` next.");
}
