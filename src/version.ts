...
declare const __OPNEX_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "opnex";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["OPNEX_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["OPNEX_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["OPNEX_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.OPNEX_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current OPNEX version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __OPNEX_VERSION__ === "string" ? __OPNEX_VERSION__ : undefined,
  bundledVersion: process.env.OPNEX_BUNDLED_VERSION,
});
