...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.OPNEX_BASE_ENV).toBe("base");
    expect(resolved.OPNEX_TEST_ENV).toBe("ok");
    expect(resolved.OPNEX_TO_REMOVE).toBeUndefined();
    expect(resolved.OPNEX_CLI).toBe(OPNEX_CLI_ENV_VALUE);
  });
