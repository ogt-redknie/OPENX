...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/opnex-bundle-mcp-gemini",
    ...
    expect(prepared.env?.OPNEX_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["opnex"]);
    expect(raw.mcpServers?.opnex?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.opnex?.headers?.Authorization).toBe("Bearer loopback-token-123");
