...
const LIVE = isLiveTestEnabled(["OPNEX_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "opnex-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("opnex_live_probe", "OPNEX Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an OPNEX-configured streamable-http server", async () => {
    const geminiCommand = process.env.OPNEX_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("opnexLiveProbe");
