...
const OPNEX_CLAUDE_PLUGIN_NAME = "opnex-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredOPNEXTmpDir(), "opnex-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, OPNEX_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: OPNEX_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped OPNEX skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
