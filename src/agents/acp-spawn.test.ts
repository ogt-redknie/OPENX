...
function createDefaultSpawnConfig(): OPNEXConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: OPNEXConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects OPNEX config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
    ...
    expect(result).toHaveProperty("error", expect.stringContaining("OPNEX config agent"));
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps OPNEX ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
