...
import { createOPNEXTools } from "./opnex-tools.js";
...
function resolveExecConfig(params: { cfg?: OPNEXConfig; agentId?: string }) {
...
export function createOPNEXCodingTools(options?: {
  agentId?: string;
  ...
  const base = (createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === "read") {
      ...
      const wrapped = createOPNEXReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
