...
const ensureOPNEXModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureOPNEXModelsJson: (...args: Parameters<typeof ensureOPNEXModelsJsonMock>) =>
        ensureOPNEXModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("opnex-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureOPNEXModelsJsonMock.mockReset();
  ensureOPNEXModelsJsonMock.mockResolvedValue({ wrote: false });
