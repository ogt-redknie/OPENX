...
const GOOGLE_LIVE_SINGLE_KEY = "OPNEX_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPNEX_LIVE_ANTHROPIC_KEY",
    listVar: "OPNEX_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPNEX_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPNEX_LIVE_${base}_KEY`;
