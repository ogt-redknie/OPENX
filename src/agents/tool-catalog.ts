...
function buildCoreToolGroupMap() {
  ...
  const opnexTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOPNEXGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:opnex": opnexTools,
