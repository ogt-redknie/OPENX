...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveOPNEXAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveOPNEXAgentDir()`
    // nullish branch. With OPNEX_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-opnex-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-path-ensure-"));
    process.env.OPNEX_STATE_DIR = stateDir;
  });
