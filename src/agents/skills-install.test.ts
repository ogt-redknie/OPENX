...
import { resolveOPNEXMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"opnex":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("opnex-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["OPNEX_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("OPNEX_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an OPNEX-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["OPNEX_STATE_DIR", "OPNEX_CONFIG_PATH"]);
    try {
      process.env.OPNEX_STATE_DIR = "/tmp/untrusted-state";
      process.env.OPNEX_CONFIG_PATH = "/tmp/untrusted-config/opnex.json";
