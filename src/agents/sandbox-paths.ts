...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const opnexTmpDir = path.resolve(resolvePreferredOPNEXTmpDir());
  if (!isPathInside(opnexTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: opnexTmpDir });
  return resolved;
}
