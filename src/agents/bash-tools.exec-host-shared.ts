...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.opnex/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "opnex doctor" and "opnex approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
