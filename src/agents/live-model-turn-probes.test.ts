...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "OPNEX_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { OPNEX_LIVE_MODEL_IMAGE_PROBE: "false" },
        "OPNEX_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { OPNEX_LIVE_MODEL_IMAGE_PROBE: "1" },
        "OPNEX_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
