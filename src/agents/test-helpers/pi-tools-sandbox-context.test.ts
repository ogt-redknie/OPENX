...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("opnex-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("opnex-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("opnex-sbx-");
