...
function resolveAttachmentLimits(config: OPNEXConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".opnex", "attachments");
  const relDir = path.posix.join(".opnex", "attachments", attachmentId);
