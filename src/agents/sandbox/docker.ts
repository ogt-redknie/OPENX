...
import { markOPNEXExecEnv } from "../../infra/opnex-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; OPNEX will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opnex.sandbox=1");
  args.push("--label", `opnex.sessionKey=${params.scopeKey}`);
  args.push("--label", `opnex.createdAtMs=${createdAtMs}`);
  args.push("--label", `opnex.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `opnex.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markOPNEXExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "opnex.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opnex sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opnex sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opnex sandbox recreate --all");
}
