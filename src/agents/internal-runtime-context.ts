export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_OPNEX_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_OPNEX_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[OPNEX_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[OPNEX_INTERNAL_CONTEXT_END]]";

export const OPNEX_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const OPNEX_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "OPNEX runtime context for the immediately preceding user message.";
export const OPNEX_RUNTIME_EVENT_HEADER = "OPNEX runtime event.";
export const OPNEX_RUNTIME_CONTEXT_CUSTOM_TYPE = "opnex.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["OPNEX runtime context (internal):", OPNEX_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === OPNEX_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === OPNEX_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === OPNEX_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(`${OPNEX_RUNTIME_EVENT_HEADER}\n${OPNEX_RUNTIME_CONTEXT_NOTICE}`)
  );
}

export function isOPNEXRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === OPNEX_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isOPNEXRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isOPNEXRuntimeContextCustomMessage(message));
}
