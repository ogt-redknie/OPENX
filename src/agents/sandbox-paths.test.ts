...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-managed-media-"));
...
async function withOutsideHardlinkInOPNEXTmp<T>(
  params: {
    opnexTmpDir: string;
  ...
  const hardlinkPath = path.join(params.opnexTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.opnexTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.opnexTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.opnexTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const opnexTmpDir = resolvePreferredOPNEXTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred OPNEX tmp root",
      media: path.join(opnexTmpDir, "image.png"),
      expected: path.join(opnexTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred OPNEX tmp root",
      media: pathToFileURL(path.join(opnexTmpDir, "photo.png")).href,
      expected: path.join(opnexTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred OPNEX tmp root",
      media: path.join(opnexTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(opnexTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(opnexTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside opnex tmp root",
      media: path.join(os.tmpdir(), "outside-opnex", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked OPNEX tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(opnexTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(opnexTmpDir, { recursive: true });
      const symlinkPath = path.join(opnexTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked OPNEX tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOPNEXTmp(
      {
        opnexTmpDir,
  ...
  it("rejects symlinked OPNEX tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOPNEXTmp(
