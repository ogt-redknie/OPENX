...
export function listAgentEntries(cfg: OPNEXConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OPNEXConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OPNEXConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
