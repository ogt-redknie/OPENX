...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when OPNEX_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const opnexHome = path.join(tmpDir, "opnex-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(opnexHome, { recursive: true });
    ...
    process.env.OPNEX_HOME = opnexHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(opnexHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
