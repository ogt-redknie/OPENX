...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/opnex/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./opnex-plugin-tools.js", () => ({
  resolveOPNEXPluginToolsForOptions: () => [],
}));

vi.mock("./opnex-tools.nodes-workspace-guard.js", () => ({
...
describe("createOPNEXTools TTS config wiring", () => {
  beforeEach(() => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createOPNEXTools } = await import("./opnex-tools.js");
...
describe("createOPNEXTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });

  it("passes preserved channel delivery context into the cron tool", async () => {
    const { createOPNEXTools } = await import("./opnex-tools.js");
