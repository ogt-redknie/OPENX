...
import { isOPNEXOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `opnex security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as OPNEXConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as OPNEXConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isOPNEXOwnerOnlyCoreToolName("gateway"),
    description:
