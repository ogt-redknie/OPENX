...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    opnex: process.env.OPNEX_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.opnex === undefined) {
      delete process.env.OPNEX_GATEWAY_TOKEN;
    } else {
      process.env.OPNEX_GATEWAY_TOKEN = envSnapshot.opnex;
    }
  });
  ...
  it("uses OPNEX_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPNEX_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
