...
function asOPNEXConfig(value: unknown): OPNEXConfig | undefined {
  return value !== null && typeof value === "object" && !Array.isArray(value)
    ? (value as OPNEXConfig)
    : undefined;
}
...
export function buildAgentRuntimeDeliveryPlan(
  ...
  const config = asOPNEXConfig(params.config);
  return {
    ...
    prompt: {
      ...
      resolveSystemPromptContribution(context) {
        return resolveProviderSystemPromptContribution({
          ...
          context: {
            ...context,
            config: asOPNEXConfig(context.config),
          },
        });
      },
    },
