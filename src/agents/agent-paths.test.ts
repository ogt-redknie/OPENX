...
describe("resolveOPNEXAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveOPNEXAgentDir();
  ...
  it("honors OPNEX_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          OPNEX_STATE_DIR: undefined,
          OPNEX_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOPNEXAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when OPNEX_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers OPNEX_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          OPNEX_STATE_DIR: undefined,
          OPNEX_AGENT_DIR: primaryOverride,
