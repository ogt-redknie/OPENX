...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("opnex-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("opnex-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("opnex-tilde-test-mkdir-");
  ...
  it("ignores OPNEX_HOME for write operations", async () => {
    const opnexHome = await createTempDir("opnex-home-override-", os.tmpdir());
    const dir = await createTempDir("opnex-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("OPNEX_HOME", opnexHome);

    createHostWorkspaceWriteTool(opnexHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(opnexHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores OPNEX_HOME for mkdir operations", async () => {
    const opnexHome = await createTempDir("opnex-home-override-", os.tmpdir());
    const dir = await createTempDir("opnex-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(opnexHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores OPNEX_HOME for readFile operations", async () => {
    const opnexHome = await createTempDir("opnex-home-override-", os.tmpdir());
    const dir = await createTempDir("opnex-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(opnexHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores OPNEX_HOME for access operations", async () => {
    const opnexHome = await createTempDir("opnex-home-override-", os.tmpdir());
    const dir = await createTempDir("opnex-tilde-test-edit-");
