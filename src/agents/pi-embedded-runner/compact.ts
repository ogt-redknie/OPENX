...
export async function compactEmbeddedPiSessionDirect(
  ...
  await ensureOPNEXModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createOPNEXCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createOPNEXCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact OPNEX-managed registrations so custom tools survive startup.
