...
function isLikelyOPNEXCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "opnex" ||
    entryBasename === "opnex.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real OPNEX CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureOPNEXModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyOPNEXCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): OPNEXConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureOPNEXModelsJson(cfg);
    } catch {
