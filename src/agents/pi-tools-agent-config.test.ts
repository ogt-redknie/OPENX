...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OPNEXConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OPNEXConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OPNEXConfig["agents"]>["list"]>[number]["tools"];
  }): OPNEXConfig {
    return {
      tools: params.tools,
      agents: {
