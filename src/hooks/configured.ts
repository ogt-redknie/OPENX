...
export function hasConfiguredInternalHooks(config: OPNEXConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: OPNEXConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
