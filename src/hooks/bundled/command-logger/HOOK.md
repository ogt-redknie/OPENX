...
homepage: https://docs.opnex.ai/automation/hooks#command-logger
metadata:
  {
    "opnex":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opnex/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opnex/logs/commands.log ~/.opnex/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opnex`:
   ```
   /home/username/.opnex/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
