...
This directory contains hooks that ship with OPNEX. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.opnex/workspace`)
...
homepage: https://docs.opnex.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.opnex/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.opnex/opnex.json`:
...
2. Restart gateway: `pkill -9 -f 'opnex.*gateway' && pnpm opnex gateway`
3. Enable the hook: `opnex hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opnex.ai/automation/hooks
