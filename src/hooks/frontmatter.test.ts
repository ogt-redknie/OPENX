...
`;
    ...
    expect(parsed.opnex.emoji).toBe("💾");
    expect(parsed.opnex.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.opnex.emoji).toBe("📝");
    expect(parsed.opnex.events).toEqual(["command"]);
    expect(parsed.opnex.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opnex.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opnex": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opnex": {"events": ["test"]}}');
  });
...
describe("resolveOPNEXMetadata", () => {
  it("extracts opnex metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOPNEXMetadata(frontmatter);
  ...
  it("returns undefined when opnex key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opnex: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OPNEX" },
            { id: "npm", kind: "npm", package: "@opnex/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opnex/hook");
  });
    ...
    "opnex":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OPNEX" }],
      },
  }
---
...
`;
    ...
    const opnex = resolveOPNEXMetadata(frontmatter);
    expect(opnex).toBeDefined();
    expect(opnex?.emoji).toBe("💾");
    expect(opnex?.events).toEqual(["command:new", "command:reset"]);
    expect(opnex?.requires?.config).toEqual(["workspace.dir"]);
    expect(opnex?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const opnex = resolveOPNEXMetadata(frontmatter);
    expect(opnex?.emoji).toBe("disk");
    expect(opnex?.events).toEqual(["command:new"]);
  });
});
