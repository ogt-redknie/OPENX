...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "opnex-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.OPNEX_BUNDLED_HOOKS_DIR;
    process.env.OPNEX_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.OPNEX_BUNDLED_HOOKS_DIR;
    } else {
      process.env.OPNEX_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".opnex", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): OPNEXConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("opnex-plugin");
  ...
  it("does not treat Claude hooks.json bundles as OPNEX hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".opnex", "extensions", "claude-bundle");
