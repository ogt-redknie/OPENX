...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "opnex-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "opnex-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "opnex-bundled");
      const plugin = makeHookEntry("shared", "opnex-plugin");
      const managed = makeHookEntry("shared", "opnex-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("opnex-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "opnex-managed");
      const workspace = makeHookEntry("shared", "opnex-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "opnex-workspace");
      const second = makeHookEntry("shared", "opnex-workspace");
      second.hook.handlerPath = "/tmp/opnex-workspace/shared/handler-2.js";
