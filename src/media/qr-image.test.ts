...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "opnex-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode-tui", async () => {
    await expect(renderQrPngBase64("opnex", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(renderPngBase64).toHaveBeenCalledWith("opnex", {
      margin: 2,
      scale: 8,
    });
  });
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("opnex", { scale: 8.9, marginModules: 2.9 });
    expect(renderPngBase64).toHaveBeenCalledWith("opnex", {
      margin: 2,
      scale: 8,
    });
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("opnex", { scale, marginModules })).rejects.toThrow(message);
  ...
  it("formats QR PNG data URLs", async () => {
    ...
    await expect(renderQrPngDataUrl("opnex")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("opnex", {
      tmpRoot,
      dirPrefix: "pair-",
