...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.opnex.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.opnex.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("skips gateway service install when a system OPNEX gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "opnex-gateway.service",
        detail: "unit: /etc/systemd/system/opnex-gateway.service",
        scope: "system",
        marker: "opnex",
        legacy: false,
      },
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining("System-level OPNEX gateway service detected"),
      "Gateway",
    );
  });
