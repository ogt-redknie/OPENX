...
beforeAll(() => {
  envSnapshot = captureEnv(["OPNEX_PROFILE"]);
  process.env.OPNEX_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByOPNEX: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/opnex/pnpm-lock.yaml",
        markerPath: "/tmp/opnex/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.OPNEX_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opnex.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opnex.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.OPNEX_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/opnex-root.js", () => ({
  resolveOPNEXPackageRoot: vi.fn().mockResolvedValue("/tmp/opnex"),
  resolveOPNEXPackageRootSync: vi.fn(() => "/tmp/opnex"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("opnex status --all") ||
          line.includes("opnex --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("opnex --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("OPNEX_GATEWAY_TOKEN", "abcd1234", async () => {
