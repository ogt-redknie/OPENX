...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("opnex security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("opnex security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.opnex.ai/faq",
    "Troubleshooting: https://docs.opnex.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("opnex status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("opnex logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("opnex node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("opnex status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("opnex gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`opnex devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("opnex devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("opnex devices list")}`),
  ];
}
