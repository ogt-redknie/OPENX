...
export const resolveOPNEXPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadOPNEXPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("opnex/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveOPNEXPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.OPNEX_UPDATE_IN_PROGRESS;
  process.env.OPNEX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-doctor-state-"));
