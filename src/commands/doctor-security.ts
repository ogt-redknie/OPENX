...
import type { OPNEXConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: OPNEXConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: OPNEXConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "OPNEX default (full)";
  const defaultRequestedAskSource = "OPNEX default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("opnex approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: OPNEXConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: OPNEXConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opnex security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.opnex/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("opnex approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opnex configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opnex config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opnex doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opnex config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('opnex config set session.dmScope "per-channel-peer"') +
