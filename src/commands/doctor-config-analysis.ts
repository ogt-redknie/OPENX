...
import { OPNEXSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: OPNEXConfig): {
  config: OPNEXConfig;
  removed: string[];
} {
  const parsed = OPNEXSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: OPNEXConfig): void {
