...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: OPNEXConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => OPNEXConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: OPNEXConfig, sourceConfig: OPNEXConfig) => void>(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opnex-agent-" });
}

function mockConfig(home: string, storePath: string): OPNEXConfig {
  const cfg = {
    agents: {
      defaults: {
