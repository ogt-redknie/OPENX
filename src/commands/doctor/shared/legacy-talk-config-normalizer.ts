...
export function normalizeLegacyTalkConfig(cfg: OPNEXConfig, changes: string[]): OPNEXConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as OPNEXConfig["talk"]) ?? {};
