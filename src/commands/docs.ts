...
const SEARCH_TOOL = "https://docs.opnex.ai/mcp.SearchOPNEX";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opnex.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opnex docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opnex.ai/");
      runtime.log(`Search: ${formatCliCommand('opnex docs "your query"')}`);
    }
    return;
  }
