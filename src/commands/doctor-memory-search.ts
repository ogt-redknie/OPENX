...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("opnex doctor --fix")} or ${formatCliCommand("opnex memory status --fix")}`
    : `Verify: ${formatCliCommand("opnex memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: OPNEXConfig): Promise<void> {
  try {
 ...
 * Runs as part of `opnex doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("opnex config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("opnex config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("opnex memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`opnex config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("opnex configure --section model")}`,
        `- To disable: ${formatCliCommand("opnex config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("opnex memory status --deep")}`,
      ].join("\n"),
      "Memory search",
