...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("opnex tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("opnex tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: OPNEXConfig) {
