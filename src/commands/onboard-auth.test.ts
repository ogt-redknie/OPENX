...
vi.mock("../agents/agent-paths.js", () => ({
  resolveOPNEXAgentDir: () => process.env.OPNEX_AGENT_DIR ?? "/tmp/opnex-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.OPNEX_STATE_DIR ?? "/tmp/opnex-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.OPNEX_AGENT_DIR ?? "/tmp/opnex-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPNEX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opnex-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-oauth-sync-"));
    ...
    process.env.OPNEX_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-oauth-nosync-"));
    process.env.OPNEX_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPNEX_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-oauth-external-"));
    process.env.OPNEX_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPNEX_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("opnex-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("opnex-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to OPNEX_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opnex-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
