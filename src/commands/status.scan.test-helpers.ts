...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/opnex-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = OPNEXConfig>(
  overrides: T = {} as T,
): OPNEXConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): OPNEXConfig {
