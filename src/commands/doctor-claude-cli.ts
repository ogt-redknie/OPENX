...
function usesClaudeCliModelSelection(cfg: OPNEXConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: OPNEXConfig): boolean {
...
function resolveClaudeCliCommand(cfg: OPNEXConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; OPNEX will create it on first run).`;
  }
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- OPNEX auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- OPNEX auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- OPNEX auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
