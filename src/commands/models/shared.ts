...
export async function loadValidConfigOrThrow(): Promise<OPNEXConfig> {
...
export async function updateConfig(
  mutator: (cfg: OPNEXConfig) => OPNEXConfig,
): Promise<OPNEXConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: OPNEXConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OPNEXConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opnex agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
