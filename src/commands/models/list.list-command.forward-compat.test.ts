...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    ensureOPNEXModelsJson: vi.fn(),
    ensureAuthProfileStore: vi.fn(),
    resolveOPNEXAgentDir: vi.fn(),
...
function resetMocks() {
  ...
  mocks.ensureOPNEXModelsJson.mockResolvedValue({ wrote: false });
  mocks.ensureAuthProfileStore.mockReturnValue({ version: 1, profiles: {}, order: {} });
  mocks.resolveOPNEXAgentDir.mockReturnValue("/tmp/opnex-agent");
...
describe("modelsListCommand forward-compat", () => {
  ...
  describe("--all catalog supplementation", () => {
    it("uses the provider catalog fast path for Codex provider lists", async () => {
      ...
      expect(mocks.ensureOPNEXModelsJson).not.toHaveBeenCalled();
      expect(mocks.loadModelRegistry).not.toHaveBeenCalled();
