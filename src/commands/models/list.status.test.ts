...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOPNEXAgentDir: vi.fn().mockReturnValue("/tmp/opnex-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opnex-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/opnex-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/opnex-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/opnex-dev/opnex.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOPNEXAgentDir: mocks.resolveOPNEXAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/opnex-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOPNEXAgentDir).toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/opnex-dev/opnex.json");
    expect(payload.auth.storePath).toBe("/tmp/opnex-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/opnex-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/opnex-agent-custom");
