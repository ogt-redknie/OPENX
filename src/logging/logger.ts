...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredOPNEXTmpDir() : POSIX_OPNEX_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "opnex.log")
    : `${POSIX_OPNEX_TMP_DIR}/opnex.log`;
}
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: OPNEXConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.OPNEX_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[opnex] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
