...
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<OPNEXConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: OPNEXConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: OPNEXConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: OPNEXConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: OPNEXConfig, result: OPNEXConfig): OPNEXConfig {
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with opnex configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "OPNEX will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.opnex.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — OPNEX will store a reference instead of the API key.",
