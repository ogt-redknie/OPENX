...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "OPNEX_ENV_TEST_A";
    const keyB = "OPNEX_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "OPNEX_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "OPNEX_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "OPNEX_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "OPNEX_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "OPNEX_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "OPNEX_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "opnex-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousOPNEXHome = process.env.OPNEX_HOME;
    const previousStateDir = process.env.OPNEX_STATE_DIR;
    const previousBundledDir = process.env.OPNEX_BUNDLED_PLUGINS_DIR;
    process.env.OPNEX_HOME = "/srv/opnex-home";
    process.env.OPNEX_STATE_DIR = "/srv/opnex-state";
    process.env.OPNEX_BUNDLED_PLUGINS_DIR = "/srv/opnex-bundled";
    ...
    try {
      ...
      expect(env.OPNEX_HOME).toBeUndefined();
      expect(env.OPNEX_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.OPNEX_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("OPNEX_HOME", previousOPNEXHome);
      restoreEnvKey("OPNEX_STATE_DIR", previousStateDir);
      restoreEnvKey("OPNEX_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { OPNEX_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processOPNEXHome: process.env.OPNEX_HOME,
          processBundledDir: process.env.OPNEX_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.OPNEX_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processOPNEXHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.OPNEX_HOME).toBe("/srv/opnex-home");
    } finally {
      restoreEnvKey("OPNEX_HOME", previousOPNEXHome);
    }
  });
