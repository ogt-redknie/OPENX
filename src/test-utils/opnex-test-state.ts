...
export type OPNEXTestStateLayout = "home" | "state-only" | "split";

export type OPNEXTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "gateway-loopback"
...
export type OPNEXTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: OPNEXTestStateLayout;
  scenario?: OPNEXTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "opnex-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "opnex.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "opnex.json"),
...
function scenarioConfig(options: OPNEXTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "opnex-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: OPNEXTestStateOptions): Record<string, string | undefined> {
...
export async function createOPNEXTestState(
  options: OPNEXTestStateOptions = {},
): Promise<OPNEXTestState> {
...
export async function withOPNEXTestState<T>(
  options: OPNEXTestStateOptions,
  fn: (state: OPNEXTestState) => Promise<T>,
): Promise<T> {
  const state = await createOPNEXTestState(options);
  try {
