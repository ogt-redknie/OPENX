...
${formatHelpExamples([
  ['opnex agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opnex agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opnex agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opnex agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opnex agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'opnex agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opnex.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opnex.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['opnex agents set-identity --agent main --name "OPNEX" --emoji "🦞"', "Set name + emoji."],
  ["opnex agents set-identity --agent main --avatar avatars/opnex.png", "Set avatar path."],
  [
    "opnex agents set-identity --workspace ~/.opnex/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "opnex agents set-identity --identity-file ~/.opnex/workspace/IDENTITY.md --agent main",
