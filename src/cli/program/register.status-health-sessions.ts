...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex status", "Show channel health + session summary."],
          ["opnex status --all", "Full diagnosis (read-only)."],
          ["opnex status --json", "Machine-readable output."],
          ["opnex status --usage", "Show model provider usage/quota snapshots."],
          [
            "opnex status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["opnex status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.opnex.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.opnex.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex sessions", "List all sessions."],
          ["opnex sessions --agent work", "List sessions for one agent."],
          ["opnex sessions --all-agents", "Aggregate sessions across agents."],
          ["opnex sessions --active 120", "Only last 2 hours."],
          ["opnex sessions --json", "Machine-readable output."],
          ["opnex sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.opnex.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "opnex sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["opnex sessions cleanup --enforce", "Apply maintenance now."],
          ["opnex sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["opnex sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "opnex sessions cleanup --enforce --store ./tmp/sessions.json",
