...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/opnex-root")).toEqual([
      path.join("/tmp/opnex-root", "dist", "index.js"),
      path.join("/tmp/opnex-root", "dist", "index.mjs"),
      path.join("/tmp/opnex-root", "dist", "entry.js"),
      path.join("/tmp/opnex-root", "dist", "entry.mjs"),
    ]);
  });
});
