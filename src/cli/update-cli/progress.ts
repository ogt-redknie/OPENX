...
const STEP_LABELS: Record<string, string> = {
  ...
  "opnex doctor entry": "Checking doctor entrypoint",
  "opnex doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g opnex@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g opnex@latest --omit=optional");
  }
