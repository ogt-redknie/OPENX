...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.opnex${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OPNEX_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.OPNEX_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.OPNEX_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.OPNEX_CONFIG_PATH)) {
    env.OPNEX_CONFIG_PATH = path.join(stateDir, "opnex.json");
  }

  if (profile === "dev" && !env.OPNEX_GATEWAY_PORT?.trim()) {
    env.OPNEX_GATEWAY_PORT = "19001";
  }
}
