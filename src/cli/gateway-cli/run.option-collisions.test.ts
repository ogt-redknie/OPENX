...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/opnex-stability.json",
  path: "/tmp/opnex-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/opnex-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/opnex-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/opnex-test-missing-config.json",
  resolveStateDir: () => "/tmp",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/opnex-control-ui";
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.OPNEX_SERVICE_MARKER;
    process.env.OPNEX_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        ...
        process.env.OPNEX_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.OPNEX_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.OPNEX_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `opnex onboard --mode local` or `opnex setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("restores last-known-good config before startup when the effective config is invalid", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/opnex-test-missing-config.json",
      config: {},
      parsed: null,
    ...
    expect(gatewayLogMessages).toContain(
      "gateway: restored invalid effective config from last-known-good backup: /tmp/opnex-test-missing-config.json",
    );
    expect(startGatewayServer).toHaveBeenCalledWith(
      19170,
      expect.objectContaining({
