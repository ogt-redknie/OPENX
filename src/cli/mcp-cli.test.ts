...
const serveOPNEXChannelMcp = mocks.serveOPNEXChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveOPNEXChannelMcp: mocks.serveOPNEXChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("opnex-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("opnex-cli-mcp-home-", async () => {
      ...
      expect(serveOPNEXChannelMcp).toHaveBeenCalledWith({
