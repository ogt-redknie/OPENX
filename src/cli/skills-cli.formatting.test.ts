...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "opnex-bundled-skills-test-"));
    process.env.OPNEX_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
