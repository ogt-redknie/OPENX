...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as OPNEXConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: OPNEXConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/opnex-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: OPNEXConfig) =>
    invokeMock<
      [OPNEXConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: OPNEXConfig, pluginId: string) =>
    invokeMock<[OPNEXConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  resolveStateDir.mockReturnValue("/tmp/opnex-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: OPNEXConfig) => ({ config: cfg })) as (
    ...args: unknown[]
  ) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: OPNEXConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: OPNEXConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
