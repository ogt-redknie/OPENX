...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: { wrapperPath?: string; env?: Record<string, string | undefined> }) => {
    ...
    if (params?.wrapperPath || params?.env?.OPNEX_WRAPPER) {
      environment.OPNEX_WRAPPER = params.wrapperPath ?? params.env?.OPNEX_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/opnex.json" },
  })),
}));
...
function mockResolvedGatewayTokenSecretRef() {
  ...
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:OPNEX_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${OPNEX_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds OPNEX_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["opnex", "gateway", "run"],
      environment: {
        OPNEX_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(actionState.warnings).toContain(
      "Gateway service OPNEX_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["opnex", "gateway", "run"],
      environment: {
        OPNEX_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/opnex-doppler", "gateway", "run"],
      environment: {
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/opnex-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service OPNEX_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when OPNEX_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["opnex", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["opnex", "gateway", "run"],
      environment: {
        OPNEX_STATE_DIR: "/tmp/opnex-doctor-manual",
        OPNEX_CONFIG_PATH: "/tmp/opnex-doctor-manual/opnex.json",
        OPNEX_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.OPNEX_STATE_DIR).toBeUndefined();
      expect(env.OPNEX_CONFIG_PATH).toBeUndefined();
