...
describe("proxy cli runtime", () => {
  const envKeys = [
    "OPNEX_DEBUG_PROXY_DB_PATH",
    "OPNEX_DEBUG_PROXY_BLOB_DIR",
    "OPNEX_DEBUG_PROXY_CERT_DIR",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "opnex-proxy-cli-runtime-"));
    ...
    process.env.OPNEX_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.OPNEX_DEBUG_PROXY_ENABLED;
    delete process.env.OPNEX_DEBUG_PROXY_SESSION_ID;
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.OPNEX_DEBUG_PROXY_DB_PATH!,
      process.env.OPNEX_DEBUG_PROXY_BLOB_DIR!,
    );
