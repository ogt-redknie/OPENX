...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update OPNEX and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["opnex update", "Update a source checkout (git)"],
        ["opnex update --channel beta", "Switch to beta channel (git + npm)"],
        ["opnex update --channel dev", "Switch to dev channel (git + npm)"],
        ["opnex update --tag beta", "One-off update to a dist-tag or version"],
        ["opnex update --tag main", "One-off package install from GitHub main"],
        ["opnex update --dry-run", "Preview actions without changing anything"],
        ["opnex update --no-restart", "Update without restarting the service"],
        ["opnex update --json", "Output result as JSON"],
        ["opnex update --yes", "Non-interactive (accept downgrade prompts)"],
        ["opnex update wizard", "Interactive update wizard"],
        ["opnex --update", "Shorthand for opnex update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run opnex update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.opnex.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex update status", "Show channel + version status."],
          ["opnex update status --json", "JSON output."],
          ["opnex update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.opnex.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
