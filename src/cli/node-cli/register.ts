    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "opnex node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["opnex node status", "Check node host service status."],
          ["opnex node install", "Install the node host service."],
          ["opnex node start", "Start the installed node host service."],
          ["opnex node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.opnex.ai/cli/node")}\n`,
    );
