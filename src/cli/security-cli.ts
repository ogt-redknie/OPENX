...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opnex security audit", "Run a local security audit."],
          ["opnex security audit --deep", "Include best-effort live Gateway probe checks."],
          ["opnex security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "opnex security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["opnex security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["opnex security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.opnex.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("OPNEX security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("opnex security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("opnex security audit --fix")}`));
        if (!fixResult) {
