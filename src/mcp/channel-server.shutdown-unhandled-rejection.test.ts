...
vi.mock("./channel-bridge.js", () => ({
  OPNEXChannelBridge: class MockOPNEXChannelBridge {
...
describe("serveOPNEXChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveOPNEXChannelMcp } = await import("./channel-server.js");

    const servePromise = serveOPNEXChannelMcp({ verbose: false });
