...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open OPNEX to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "opnex.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPNEX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPNEX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OPNEX_APNS_TEAM_ID and OPNEX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPNEX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPNEX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPNEX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPNEX_APNS_PRIVATE_KEY_P8 or OPNEX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPNEX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
