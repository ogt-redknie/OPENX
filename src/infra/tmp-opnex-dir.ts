...
export const POSIX_OPNEX_TMP_DIR = "/tmp/opnex";

type ResolvePreferredOPNEXTmpDirOptions = {
...
export function resolvePreferredOPNEXTmpDir(
  options: ResolvePreferredOPNEXTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "opnex" : `opnex-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[opnex] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback OPNEX temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback OPNEX temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_OPNEX_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_OPNEX_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_OPNEX_TMP_DIR)) {
      return POSIX_OPNEX_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_OPNEX_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_OPNEX_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_OPNEX_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_OPNEX_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_OPNEX_TMP_DIR;
