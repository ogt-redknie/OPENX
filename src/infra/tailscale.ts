...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.OPNEX_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OPNEX. You can keep running the web gateway without it: \`${formatCliCommand("opnex gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
