...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerOPNEXRestart() uses schtasks to restart the gateway.
...
export function triggerOPNEXRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.OPNEX_SYSTEMD_UNIT,
      process.env.OPNEX_PROFILE,
    );
  ...
  const label =
    process.env.OPNEX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPNEX_PROFILE);
