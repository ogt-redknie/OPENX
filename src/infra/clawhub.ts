...
// Keep aligned with @opnex/plugin-package-contract ExternalPluginCompatibility.
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.OPNEX_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.OPNEX_CLAWHUB_TOKEN) ||
