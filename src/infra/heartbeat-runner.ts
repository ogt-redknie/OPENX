...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OPNEXConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: OPNEXConfig) {
...
function resolveHeartbeatAgents(cfg: OPNEXConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OPNEXConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OPNEXConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: OPNEXConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: OPNEXConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OPNEXConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
