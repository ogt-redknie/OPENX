...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "OPNEX_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "OPNEX_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "OPNEX_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "OPNEX_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.opnex/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.opnex/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".opnex");
        process.env.OPNEX_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "opnex", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPNEX_STATE_DIR=./evil-state",
            "OPNEX_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.OPNEX_STATE_DIR).toBe(stateDir);
        expect(process.env.OPNEX_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPNEX_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "OPNEX_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "OPNEX_LIVE_GEMINI_KEY=sk-gemini-live",
            "OPNEX_LIVE_OPENAI_KEY=sk-openai-live",
            "OPNEX_GATEWAY_TOKEN=attacker-token",
            "OPNEX_GATEWAY_PASSWORD=attacker-password",
            "OPNEX_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks OPNEX_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.OPNEX_STATE_DIR).toBeUndefined();
        expect(process.env.OPNEX_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (OPNEX_AGENT_DIR, OPNEX_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, OPNEX_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "OPNEX_AGENT_DIR=./evil-agent",
            `OPNEX_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "OPNEX_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );
        ...
        expect(process.env.OPNEX_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.OPNEX_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks OPNEX_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "OPNEX_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.OPNEX_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.OPNEX_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "OPNEX_PINNED_PYTHON=./attacker-python",
            "OPNEX_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.OPNEX_PINNED_PYTHON;
        delete process.env.OPNEX_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.OPNEX_PINNED_PYTHON).toBeUndefined();
        expect(process.env.OPNEX_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPNEX_PINNED_PYTHON=/trusted/python",
            "OPNEX_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.OPNEX_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.OPNEX_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPNEX_LIVE_GEMINI_KEY=sk-gemini-live",
            "OPNEX_LIVE_OPENAI_KEY=sk-openai-live",
            "OPNEX_GATEWAY_TOKEN=trusted-token",
            "OPNEX_GATEWAY_PASSWORD=trusted-password",
            "OPNEX_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.OPNEX_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.OPNEX_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.OPNEX_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.OPNEX_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.OPNEX_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.OPNEX_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.OPNEX_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "OPNEX_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when OPNEX_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.OPNEX_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "opnex", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.OPNEX_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "OPNEX_WORKSPACE_DIR",
          "OPNEX_MDNS_HOSTNAME",
          "OPNEX_SESSION_CACHE_TTL_MS",
          "OPNEX_UPDATE_PACKAGE_SPEC",
          "OPNEX_GATEWAY_PORT",
          "OPNEX_GATEWAY_URL",
          "OPNEX_CLAWHUB_URL",
          "CLAWHUB_URL",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("opnex/opnex");
