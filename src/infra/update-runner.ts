...
const DEFAULT_PACKAGE_NAME = "opnex";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "opnex-update-preflight-";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["opnex", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "opnex.mjs");
      ...
      const doctorStep = await runStep(
        step("opnex doctor", doctorArgv, gitRoot, { OPNEX_UPDATE_IN_PROGRESS: "1" }),
      );
