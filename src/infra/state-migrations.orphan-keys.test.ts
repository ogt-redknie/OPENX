...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".opnex");
...
function sharedMainOpsConfig(sharedStorePath: string): OPNEXConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: OPNEXConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { OPNEX_STATE_DIR: stateDir };
