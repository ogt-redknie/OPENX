...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OPNEXConfig | null = null;
  ...
  get(key: string, cfg: OPNEXConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OPNEXConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OPNEXConfig): void {
