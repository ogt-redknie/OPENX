...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.OPNEX_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-current-bindings-"));
    process.env.OPNEX_STATE_DIR = testStateDir;
