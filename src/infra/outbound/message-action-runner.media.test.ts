...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred OPNEX tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideOPNEX = path.join(os.tmpdir(), "outside-opnex", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
