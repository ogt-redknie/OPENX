...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__opnex_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "opnex-root");
...
function setPackageRoot(root: string, name = "opnex") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./opnex-root.js").resolveOPNEXPackageRootSync,
  asyncResolver: typeof import("./opnex-root.js").resolveOPNEXPackageRoot,
  opts: Parameters<typeof import("./opnex-root.js").resolveOPNEXPackageRootSync>[0],
...
vi.mock("./opnex-root.fs.runtime.js", () => ({
  opnexRootFsSync: mockFsModule(),
  opnexRootFs: mockFsPromisesModule(),
}));

describe("resolveOPNEXPackageRoot", () => {
  let resolveOPNEXPackageRoot: typeof import("./opnex-root.js").resolveOPNEXPackageRoot;
  let resolveOPNEXPackageRootSync: typeof import("./opnex-root.js").resolveOPNEXPackageRootSync;

  beforeAll(async () => {
    ({ resolveOPNEXPackageRoot, resolveOPNEXPackageRootSync } =
      await import("./opnex-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "opnex");
        const pkgRoot = path.join(project, "node_modules", "opnex");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "opnex");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "opnex.mjs")));
    ...
    {
      name: "falls through from a non-opnex moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-opnex");
    ...
    {
      name: "returns null for non-opnex package roots",
      setup: () => {
        const pkgRoot = fx("not-opnex");
        setPackageRoot(pkgRoot, "not-opnex");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "opnex.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "opnex");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveOPNEXPackageRootSync,
      resolveOPNEXPackageRoot,
      opts,
      expected,
    );
  });
