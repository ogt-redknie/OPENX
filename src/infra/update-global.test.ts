...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["OPNEX_UPDATE_PACKAGE_SPEC"]);
    process.env.OPNEX_UPDATE_PACKAGE_SPEC = "file:/tmp/opnex.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "opnex", tag: "latest" })).toBe(
      "file:/tmp/opnex.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "opnex",
        tag: "beta",
        env: { OPNEX_UPDATE_PACKAGE_SPEC: "opnex@next" },
      }),
    ).toBe("opnex@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "opnex"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "opnex", tag: "main" })).toBe(
      OPNEX_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "opnex",
        tag: "github:opnex/opnex#feature/my-branch",
      }),
    ).toBe("github:opnex/opnex#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "opnex",
        tag: "https://example.com/opnex-main.tgz",
      }),
    ).toBe("https://example.com/opnex-main.tgz");
  });
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:opnex/opnex#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/opnex-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/opnex-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:opnex/opnex#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "opnex-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "opnex");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "opnex"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "opnex"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "opnex"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "opnex"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "opnex-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "opnex@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "opnex@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "opnex@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "opnex@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "opnex-update-npm-missing-bin-" }, async (base) => {
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    ...
    try {
      await withTempDir({ prefix: "opnex-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "opnex");
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "opnex@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "opnex@latest",
    ...
    expect(globalInstallArgs("pnpm", "opnex@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "opnex@latest",
    ]);
    expect(globalInstallArgs("bun", "opnex@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "opnex@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "opnex@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "opnex@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "opnex@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "opnex@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "opnex@latest"]);
  });

  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "opnex@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "opnex@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/opnex/lib/node_modules")).toEqual({
      prefix: "/opt/opnex",
      globalRoot: "/opt/opnex/lib/node_modules",
      binDir: "/opt/opnex/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "opnex-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".opnex-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".opnex-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".opnex-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "opnex"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".opnex-123", ".opnex-456"],
      });
      await expect(fs.stat(path.join(root, "opnex"))).resolves.toBeDefined();
      await expect(fs.stat(path.join(root, ".opnex-file"))).resolves.toBeDefined();
    });
  });

  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "opnex-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("ignores bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "opnex-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".opnex-install-stage", ".opnex-install-stage-retry"]) {
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "opnex-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "opnex-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "opnex-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "opnex-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "matrix", "@opnex/matrix");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "opnex-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "matrix", "@opnex/matrix");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "opnex-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@opnex/qa-lab");
        await writePackageDistInventory(packageRoot);
