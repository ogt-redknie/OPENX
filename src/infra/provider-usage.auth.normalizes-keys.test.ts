...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "opnex-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".opnex");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".opnex", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      OPNEX_STATE_DIR: path.join(home, ".opnex"),
      ...env,
    };
