...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, OPNEX's ring-zero setup helper.",
  "Turn the user's request into exactly one safe OPNEX Crestodian command.",
  ...
  "Use the provided OPNEX docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `OPNEX docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `OPNEX source: ${
