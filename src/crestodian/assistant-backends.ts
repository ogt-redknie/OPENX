...
export type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => OPNEXConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): OPNEXConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): OPNEXConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
