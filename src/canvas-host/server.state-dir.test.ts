...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPNEX_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("opnex-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OPNEX Canvas");
      } finally {
        await handler.close();
      }
    });
