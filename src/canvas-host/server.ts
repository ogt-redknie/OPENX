...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.opnexCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.opnexCanvasA2UIAction &&
        typeof window.opnexCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.opnexSendUserAction === "function";
  ...
  window.addEventListener("opnex:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OPNEX node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.opnexSendUserAction === "function"
        ? window.opnexSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPNEX_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPNEX_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OPNEX Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
