...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPNEX_HOME.
  delete process.env.OPNEX_HOME;
  process.env.OPNEX_STATE_DIR = path.join(base, ".opnex");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "opnex-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opnex", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
