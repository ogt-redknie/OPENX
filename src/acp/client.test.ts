...
const createTempDir = () => tempDirs.make("opnex-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPNEX_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "opnex",
    });

    expect(env.OPNEX_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("opnex");
  });

  it("overrides pre-existing OPNEX_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPNEX_SHELL: "wrong",
    });
    expect(env.OPNEX_SHELL).toBe("acp-client");
  ...
  it("preserves OPNEX_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OPNEX_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OPNEX_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OPNEX bridge", () => {
    ...
    expect(env.OPNEX_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.OPNEX_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("OPNEX_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "opnex", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "opnex", "dist", "entry.js");
    const shimPath = path.join(dir, "opnex.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\opnex\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/opnex-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/opnex-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/opnex-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
