...
const { createOPNEXCodingToolsMock } = vi.hoisted(() => ({
  createOPNEXCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createOPNEXCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(vi.mocked(createOPNEXCodingTools)).toHaveBeenCalledWith(
