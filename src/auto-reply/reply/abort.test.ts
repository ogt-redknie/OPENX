...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop opnex!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@opnex_bot", { botUsername: "opnex_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@opnex_bot", { botUsername: "opnex_bot" })).toBe(true);
