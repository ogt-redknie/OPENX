...
const { buildStatusReplyMock, createOPNEXToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createOPNEXToolsMock: vi.fn(),
...
vi.mock("../../agents/opnex-tools.runtime.js", () => ({
  createOPNEXTools: (...args: unknown[]) => createOPNEXToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createOPNEXToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
