...
type HomeEnvSnapshot = {
  ...
  OPNEX_STATE_DIR: string | undefined;
  OPNEX_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    OPNEX_AGENT_DIR: process.env.OPNEX_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".opnex", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPNEX_STATE_DIR = path.join(home, ".opnex");
    process.env.OPNEX_AGENT_DIR = path.join(home, ".opnex", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".opnex", "agent");
