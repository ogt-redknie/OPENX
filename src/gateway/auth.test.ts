...
describe("gateway auth", () => {
  ...
  it("resolves token/password from OPNEX gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          OPNEX_GATEWAY_TOKEN: "env-token",
          OPNEX_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides OPNEX_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        OPNEX_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
