...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "opnex-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".opnex", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["OPNEX_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "opnex");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.OPNEX_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".opnex");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "opnex-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-wizard-home-"));
      process.env.HOME = tempHome;
      process.env.OPNEX_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".opnex", "opnex.json");
      const bundledPluginsDir = path.join(tempHome, "opnex-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.OPNEX_STATE_DIR = path.join(tempHome, ".opnex");
      process.env.OPNEX_CONFIG_PATH = configPath;
      process.env.OPNEX_SKIP_CHANNELS = "1";
