...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listOPNEXPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listOPNEXPluginManifestMetadata: (
      params?: Parameters<typeof actual.listOPNEXPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listOPNEXPluginManifestMetadata(params);
      return actual.listOPNEXPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listOPNEXPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as OPNEXConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listOPNEXPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/opnex.plugin.json",
    ...overrides,
  };
}
