...
const { createOPNEXTools } = await import("../agents/opnex-tools.js");
...
type SessionSendTool = ReturnType<typeof createOPNEXTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOPNEXTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPNEX_GATEWAY_PORT", "OPNEX_GATEWAY_TOKEN"]);
  ...
  process.env.OPNEX_GATEWAY_PORT = String(gatewayPort);
  process.env.OPNEX_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPNEX_CONFIG_PATH missing in gateway test environment");
      }
