...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "opnex-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.OPNEX_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as OPNEXConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "opnex-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPNEX_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
