...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers OPNEX_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        OPNEX_HANDSHAKE_TIMEOUT_MS: "75",
        OPNEX_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        OPNEX_HANDSHAKE_TIMEOUT_MS: "",
        OPNEX_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS;
    try {
      process.env.OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      if (original === undefined) {
        delete process.env.OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.OPNEX_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
    }
  });
});
