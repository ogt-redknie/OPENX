...
export function resolveHooksConfig(cfg: OPNEXConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OPNEXConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-opnex-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-opnex-idempotency-key"]) ||
