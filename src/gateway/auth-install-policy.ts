...
type GatewayInstallAuthMode = NonNullable<NonNullable<OPNEXConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: OPNEXConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.OPNEX_GATEWAY_PASSWORD) ||
