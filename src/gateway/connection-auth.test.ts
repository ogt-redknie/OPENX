...
function cfg(input: Partial<OPNEXConfig>): OPNEXConfig {
  return input as OPNEXConfig;
}
...
const DEFAULT_ENV = {
  OPNEX_GATEWAY_TOKEN: "env-token",
  OPNEX_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when OPNEX env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when OPNEX env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when OPNEX env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
