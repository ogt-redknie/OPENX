...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.OPNEX_CONFIG_PATH) {
    configPaths.add(process.env.OPNEX_CONFIG_PATH);
  }
  if (process.env.OPNEX_STATE_DIR) {
    configPaths.add(path.join(process.env.OPNEX_STATE_DIR, "opnex.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPNEX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "opnex-test-no-bundled-extensions")
    : "opnex-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".opnex-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".opnex-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".opnex-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>opnex-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__opnexTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__opnexTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["OPNEX_GATEWAY_TOKEN"]);
  const prev = process.env.OPNEX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPNEX_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.OPNEX_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
