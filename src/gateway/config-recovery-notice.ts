...
export function formatConfigRecoveryNotice(params: {
  ...
  const configName = path.basename(params.configPath) || "opnex.json";
  return [
    `Config recovery warning: OPNEX restored ${configName} from the last-known-good backup during ${params.phase} (${params.reason}).`,
