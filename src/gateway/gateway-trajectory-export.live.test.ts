...
const CODEX_HARNESS_LIVE = process.env.OPNEX_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.OPNEX_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["OPNEX_TRAJECTORY", "OPNEX_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-opnex-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.OPNEX_TRAJECTORY = "1";
      process.env.OPNEX_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".opnex", "trajectory-exports", "bundle");
