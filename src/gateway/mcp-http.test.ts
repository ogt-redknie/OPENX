  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-opnex-account-id": "work",
        "x-opnex-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-opnex-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.opnex?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.opnex?.headers?.Authorization).toBe(
      "Bearer ${OPNEX_MCP_TOKEN}",
    );
    expect(config.mcpServers?.opnex?.headers?.["x-opnex-message-channel"]).toBe(
      "${OPNEX_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.opnex?.headers?.["x-opnex-sender-is-owner"]).toBeUndefined();
  });
});
