...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("opnex doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with OPNEX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with OPNEX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.OPNEX_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://opnex-gateway.ai:18789",
      onConnectError,
    });
