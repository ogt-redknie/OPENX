...
function gatewayTokenConfig(config: OPNEXConfig): OPNEXConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: OPNEXConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/opnex-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: OPNEXConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.OPNEX_SKIP_CHANNELS;
  const previousSkipProviders = process.env.OPNEX_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.OPNEX_SKIP_CHANNELS;
    } else {
      process.env.OPNEX_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.OPNEX_SKIP_PROVIDERS;
    } else {
      process.env.OPNEX_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
