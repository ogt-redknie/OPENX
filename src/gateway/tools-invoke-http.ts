...
function resolveMemoryToolDisableReasons(cfg: OPNEXConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-opnex-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-opnex-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-opnex-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-opnex-thread-id"));
