...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before OPNEX consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether OPNEX should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume OPNEX's bundle
   ...
   *
   * Use this when the backend should consume a canonical OPNEX auth profile
   ...
   * `profile-only` treats the selected OPNEX auth profile as the sole auth
