...
const APPROVALS_PATH = "~/.opnex/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "opnex-app-server:thread:",
  "opnex-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("opnex.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("opnex.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to OPNEX instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
