...
import type { OPNEXPluginService, OPNEXPluginServiceContext } from "./types.js";
...
function expectServiceLogger(ctx: OPNEXPluginServiceContext) {
...
function expectServiceContexts(
  contexts: OPNEXPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: OPNEXPluginServiceContext[] = [];
  ...
  it("grants internal diagnostics only to bundled diagnostics exporter services", async () => {
    ...
    const prometheusContexts: OPNEXPluginServiceContext[] = [];
    ...
    const untrustedContexts: OPNEXPluginServiceContext[] = [];
