...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "opnex", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".opnex", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".opnex", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "opnex-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".opnex", "extensions"),
      },
    });
  });
});
