 ...
 */
export type OPNEXPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<OPNEXConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). OPNEX uses this to expose
 ...
 *
 * Runs after OPNEX resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common OPNEX auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and OPNEX wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before OPNEX builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. OPNEX no
...
/**
 * Provider-owned stream wrapper hook after OPNEX applies its generic
 ...
 *
 * Return `true` or `false` to override OPNEX's built-in provider cache TTL
 ...
 *
 * Runs only after OPNEX exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that OPNEX should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after OPNEX loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, OPNEX falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * OPNEX calls this only from async model resolution paths. After it
   ...
   *
   * Runs after OPNEX performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before OPNEX registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic OPNEX wrappers.
   *
   ...
   *
   * Called after OPNEX resolves the raw configured credential but before the
   ...
   * hint than OPNEX's generic auth-store guidance.
   */
   ...
   * OPNEX's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when OPNEX should surface a provider-specific hint for
   ...
   * merging. OPNEX deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. OPNEX stores the
   ...
   * without replacing the full OPNEX-owned system prompt.
   */
   ...
   *
   * Runs after OPNEX's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep OPNEX's default fallback policy.
   */
   ...
   * rewrites after OPNEX has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before OPNEX handles
   ...
   *
   * OPNEX uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `opnex doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `opnex doctor` should offer.
   *
   ...
   *
   * OPNEX calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep OPNEX's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * OPNEX falls back to that stored profile.
   */
...
export type OPNEXPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active OPNEX session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current OPNEX configuration */
  config: OPNEXConfig;
 ...
 */
export type OPNEXPluginCommandDefinition = {
...
export type OPNEXPluginHttpRouteAuth = "gateway" | "plugin";
export type OPNEXPluginHttpRouteMatch = "exact" | "prefix";
export type OPNEXPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type OPNEXPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type OPNEXPluginHttpRouteParams = {
  path: string;
  handler: OPNEXPluginHttpRouteHandler;
  auth: OPNEXPluginHttpRouteAuth;
  match?: OPNEXPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: OPNEXPluginGatewayRuntimeScopeSurface;
  replaceExisting?: boolean;
};

export type OPNEXPluginCliContext = {
  program: Command;
  config: OPNEXConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type OPNEXPluginCliRegistrar = (ctx: OPNEXPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want OPNEX to keep a plugin command lazy-loaded while still
 ...
 */
export type OPNEXPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type OPNEXPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type OPNEXPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type OPNEXPluginSecurityAuditContext = {
  config: OPNEXConfig;
...
export type OPNEXPluginSecurityAuditCollector = (
  ctx: OPNEXPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type OPNEXGatewayDiscoveryAdvertiseContext = {
...
export type OPNEXGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: OPNEXGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type OPNEXPluginServiceContext = {
  config: OPNEXConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type OPNEXPluginService = {
  id: string;
  start: (ctx: OPNEXPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: OPNEXPluginServiceContext) => void | Promise<void>;
};

export type OPNEXPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type OPNEXPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: OPNEXPluginReloadRegistration;
  nodeHostCommands?: OPNEXPluginNodeHostCommand[];
  securityAuditCollectors?: OPNEXPluginSecurityAuditCollector[];
  register?: (api: OPNEXPluginApi) => void;
  activate?: (api: OPNEXPluginApi) => void;
};

export type OPNEXPluginModule = OPNEXPluginDefinition | ((api: OPNEXPluginApi) => void);
...
export type PluginConfigMigration = (config: OPNEXConfig) =>
  | {
      config: OPNEXConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `opnex migrate`. */
...
export type OPNEXPluginApi = {
  ...
  registerTool: (
    ...
    opts?: OPNEXPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: OPNEXPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: OPNEXPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: OPNEXPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, OPNEX can keep
  ...
  registerReload: (registration: OPNEXPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: OPNEXPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: OPNEXPluginSecurityAuditCollector) => void;
  registerService: (service: OPNEXPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: OPNEXGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `opnex migrate` (migration capability). */
