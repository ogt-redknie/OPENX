...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".opnex", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "OPNEX_HOME", "OPNEX_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("opnex-bundle-commands-home-");
      const workspaceDir = await createTempDir("opnex-bundle-commands-workspace-");
