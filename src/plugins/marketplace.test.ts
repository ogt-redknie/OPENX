...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("opnex-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("opnex-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("opnex-marketplace-test-", async (homeDir) => {
      const opnexHome = path.join(homeDir, "opnex-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, OPNEX_HOME: opnexHome },
