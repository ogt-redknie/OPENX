...
import type {
  ...
  OPNEXPluginApi as CoreOPNEXPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "opnex/plugin-sdk/core";
import * as providerEntrySdk from "opnex/plugin-sdk/provider-entry";
...
import type {
  ...
  OPNEXPluginApi as SharedOPNEXPluginApi,
...
import type { OPNEXPluginApi } from "../types.js";
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']opnex\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreOPNEXPluginApi>().toMatchTypeOf<OPNEXPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreOPNEXPluginApi>().toMatchTypeOf<SharedOPNEXPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("opnex/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "opnex/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "opnex/plugin-sdk/global-singleton",
    );
    const textRuntimeSdk = await importResolvedPluginSdkSubpath("opnex/plugin-sdk/text-runtime");
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("opnex/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "opnex/plugin-sdk/channel-lifecycle",
    );
    ...
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "opnex/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`opnex/plugin-sdk/${id}`));
    }
