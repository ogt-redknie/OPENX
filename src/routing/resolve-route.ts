...
function listAgents(cfg: OPNEXConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OPNEXConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OPNEXConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OPNEXConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OPNEXConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OPNEXConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OPNEXConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OPNEXConfig,
  {
    bindingsRef: OPNEXConfig["bindings"];
    agentsRef: OPNEXConfig["agents"];
    sessionRef: OPNEXConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OPNEXConfig): Map<string, ResolvedAgentRoute> {
