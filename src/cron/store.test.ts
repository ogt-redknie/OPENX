...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opnex-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPNEX_HOME for tilde expansion", () => {
    vi.stubEnv("OPNEX_HOME", "/srv/opnex-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/opnex-home", "cron", "jobs.json"));
  });
});
